...
{
	if (!CheckAttribute(rCharacter, "index"))
		return;
...
void Ship_FireAction()
{
	...
	if (IsCompanion(rCharacter))
	{
		...
		{
			if (HasSubStr(rCharacter, "PsHero_"))
				PChar.PGG_EnemyPP = "1";
...
float Ship_MastDamage()
{
	...
	if (sShip == "PRINCE" || sShip == "OXFORD" || sShip == "RESOLUTION" || sShip == "MORDAUNT")
		return fDamage * MastMulti * 0.8; //для хрупких кораблей сделать жирнее мачты
...
void Ship_Add2Sea(int iCharacterIndex, bool bFromCoast, string sFantomType)
{
	...
	{ // NK 04-09-16 clears timer attribute, -21 for updatetime
...
void Ship_OnBortReloaded()
{
	...
	if (sBort == "cannonf")
	{
		aCharacter.Tmp.BortsReloaded.cannonf = true;
	}
	if (sBort == "cannonb")
	{
		aCharacter.Tmp.BortsReloaded.cannonb = true;
	}
}
...
void Ship_BortReloadEvent()
{
	...
	if (sti(aCharacter.Tmp.BortsReloaded.Event))
	{
		...
		bool bFront = false;
		bool bBack = false;
		if (CheckAttribute(aCharacter, "Tmp.BortsReloaded.cannonl"))
			bLeft = sti(aCharacter.Tmp.BortsReloaded.cannonl) == true;
		if (CheckAttribute(aCharacter, "Tmp.BortsReloaded.cannonr"))
			bRight = sti(aCharacter.Tmp.BortsReloaded.cannonr) == true;
		if (CheckAttribute(aCharacter, "Tmp.BortsReloaded.cannonf"))
			bFront = sti(aCharacter.Tmp.BortsReloaded.cannonf) == true;
		if (CheckAttribute(aCharacter, "Tmp.BortsReloaded.cannonb"))
			bBack = sti(aCharacter.Tmp.BortsReloaded.cannonb) == true;
		...
		else
		{
			...
			if (bLeft)
			{
				PlayVoice("interface\_GunReadyL.wav");
			}
			if (bRight)
			{
				PlayVoice("interface\_GunReadyR.wav");
			}
			// if (bFront) { PlayVoice("interface\_Gun_FB_Ready.wav"); }
			if (bBack)
			{
				PlayVoice("interface\_Gun_FB_Ready.wav");
			}
		}
...
void Ship_CheckSituation()
{
	...
	if (!bIsCompanion) //fix 171204 boal Не нужно наших с толку сбивать
	{
		...
		if (CheckAttribute(rCharacter, "fortDefender") && sti(rCharacter.fortDefender) == 1 && bSeaActive && rCharacter.SeaAI.Task == "3")
		{
			...
			{
				if (sti(rCharacter.Ship.LastBallCharacter) != -1)
					attckChar = &characters[sti(rCharacter.Ship.LastBallCharacter)];
				else
					attckChar = CharacterFromId(fortChar.Ship.LastBallCharacter);
					...
					if (sti(rCharacter.nation) != PIRATE || bBettaTestMode) //boal 030804 нужно чтоб при тесте флаг с пиратами мирил
					{
						...
						if (rCharacter.id == "MQPirate")
						{
							...
							if (Ship_GetDistance2D(GetMainCharacter(), rCharacter) < 150 && !CheckAttribute(pchar, "GenQuest.DestroyPirate.WasFirstDeside"))
							{
								...
								{
									if ((GetCrewQuantity(pchar) / GetCrewQuantity(rCharacter)) < 1.34)
										shipCrew = true;
								}
								...
								if (iQty == 1)
								{
									if (shipHealth || shipClass || shipCrew)
										fight = true;
								}
								if (iQty == 2)
								{
									if (shipClass && shipCrew || shipHealth)
										fight = true;
								}
								if (iQty == 3)
								{
...
void Ship_CheckFlagEnemy(ref rCharacter)
{
	...
	{ // смотрим флаг у ГГ и узнаем в нем...
		...
		if (losechance > winchance)
		{
			...
			if (!CheckAttribute(pchar, "questTemp.stels.sea"))
				pchar.questTemp.stels.sea = GetDataDay() - 1;
...
void Ship_ActivateFirePlace()
{
	...
	if (!bAbordageStarted) // fix
	{
		...
		if (arCharacter.index == pchar.index)
			PlayVoice("Kopcapkz\SoundFX\SBVF_" + rand(3) + ".mp3"); // LEO: Оповещение о пожаре на корабле
	}
}
...
void Ship_BranderDetonate()
{
	...
	if (!FindEntity(&arShipObjects[0], "ship"))
	{
		return;
	} // can't be
	while (iBranderNumShips < 64 && FindEntityNext(&arShipObjects[iBranderNumShips]))
	{
		iBranderNumShips++;
	}
...
void Ship_ApplyHullHitpoints(ref rOurCharacter, float fHP, int iKillStatus, int iKillerCharacterIndex)
{
	...
	if (sti(rOurCharacter.TmpPerks.ShipDefenseProfessional))
		fMinus = 0.35;
	if (CheckAttribute(&RealShips[sti(rOurCharacter.Ship.Type)], "Tuning.HullSpecial"))
		fMinus = fMinus + 0.35;
...
void Ship_ApplyHullHitpointsWithCannon(ref rOurCharacter, float fHP, int iKillStatus, int iKillerCharacterIndex)
{
	...
	if (iKillerCharacterIndex >= 0)
	{
		...
		if (sti(Characters[iKillerCharacterIndex].TmpPerks.CannonProfessional))
			fPlus = 0.3;
		if (CheckAttribute(&RealShips[sti(Characters[iKillerCharacterIndex].Ship.Type)], "Tuning.CannonsSpecial"))
			fPlus -= 0.2;
		//fAccuracy = stf(characters[iKillerCharacterIndex].TmpSkill.Accuracy);
	}
...
void Ship_HullHitEvent()
{
	...
	if (rBallCharacter == GetMainCharacter() && CheckAttribute(rOurCharacter, "fortDefender"))
		pchar.point = "1";
	//<---Lipsar для ИИ сторожей
	...
	float fCrewDamage;
	if (CheckAttribute(RealShips[sti(rOurCharacter.Ship.Type)], "Tuning.HighBort") && iBallType == GOOD_GRAPES)
		fCrewDamage = (stf(rBall.DamageCrew) * 0.75) * fCannonDamageMultiply * AIShip_isPerksUse(rBallCharacter.TmpPerks.CrewDamageUp, 1.0, 1.15);
	else
	...
	else
	{
		...
		if (CheckAttribute(RealShips[sti(rOurCharacter.Ship.Type)], "Tuning.HighBort") && iBallType != GOOD_GRAPES)
			fHP *= 1.35; // LEO: (fHP *= 1.25;) - этого мало оказалось
		Ship_ApplyHullHitpointsWithCannon(rOurCharacter, fHP, KILL_BY_BALL, iBallCharacterIndex);
		// boal  check skill -->
		if (!isOurCompanion && IsCompanion(rBallCharacter))
		{
...
bool Ship_CheckMorale(int chridx, bool checkNow)
{
	...
	if (CheckAttribute(rCharacter, "surrendered"))
		return false; // NK surrender 05-04-20
	...
	if (rCharacter.ship.type == SHIP_FORT)
		return false;

	if (!checkNow)
	{
		//check occasionally
		int timecheck = CHANGE_MORALE_EVERY if (chridx == GetMainCharacterIndex())
		{
	...
	if (!CheckAttribute(rCharacter, "Ship.Crew.Morale"))
	{
		rCharacter.Ship.Crew.Morale = 45;
	}
	if (!CheckAttribute(rCharacter, "seatime.basecrewmorale"))
	{
		rCharacter.seatime.basecrewmorale = rCharacter.Ship.Crew.Morale;
	}

	int CrewQuantity = GetCrewQuantity(rCharacter);
	if (!CheckAttribute(rCharacter, "seatime.basecrewquantity"))
	{
		rCharacter.seatime.basecrewquantity = CrewQuantity;
	}
	...
	float tempmorale = stf(rCharacter.ship.crew.morale);
	if (CheckAttribute(rCharacter, "seatime.tempmorale"))
		tempmorale = stf(rCharacter.seatime.tempmorale); //i.e. a float morale
	...
	int lasthit = -1;
	if (CheckAttribute(rCharacter, "seatime.lasthit"))
		lasthit = sti(rCharacter.seatime.lasthit); //last time we were hit
	int lastfired = -1;
	if (CheckAttribute(rCharacter, "seatime.lastfired"))
		lastfired = sti(rCharacter.seatime.lastfired); // last time we fired or reloaded
	...
	if (enemydistance <= RANGE_TO_ENEMY)
		combatlevel = COMBATLEVEL_ENEMYHERE;
	if (lastfired != -1)
	{
		if ((currenttime - lastfired) < TIME_SINCE_GUNS)
			combatlevel = COMBATLEVEL_GUNS;
	}
	if (lasthit != -1)
	{
		if ((currenttime - lasthit) < TIME_SINCE_HIT)
			combatlevel = COMBATLEVEL_HIT;
	}
	...
	if (combatlevel == COMBATLEVEL_NONE)
	{
		...
		{
			if (tempmorale > basemorale)
				tempmorale -= MORALE_AMT_COMEBACK;
			else
				tempmorale += MORALE_AMT_COMEBACK;
		}
	}
	...
	{
		if (tempmorale < fMaxTempMorale)
			tempmorale += COMBATLEVEL_ENEMYHERE * 2.5;
	}
	if (tempmorale > MORALE_MAX)
		tempmorale = MORALE_MAX;
	if (tempmorale < MORALE_MIN)
		tempmorale = MORALE_MIN;
	...
	{
		ftmult *= 1.5;
		surrMarg += 2.0; //ftmult was 2.0 and surrMarg was 2.5 //#20200307-03
	}
	//Nation mod
	...
	if (HPp > 40.0 && fdist <= enemydistance && strengthrat < 0.8)
		return false;
	...
	if (strengthrat > 1.0) // they're stronger
	{
		...
		if (SP < 40.0)
			surmorale *= Bring2Range(5.0, 1.0, 0.1, 7.071, sqrt(SP)); //panic if can't get away
	}
	else
	{
		surmorale *= 0.3;
...
float GetDistDeltaToNearestEnemy(int idx)
{
	...
	{
		float fTimeDelta = fNow - stf(chr.seatime.nearestenemy.lastcheck) if (fTimeDelta > 1)
		{
			//calc distance
...
void AddKillMorale(ref rKillerCharacter, ref rDead)
{
	...
	{
		if (CheckAttribute(rKillerCharacter, "seatime.enemydead"))
			tmped = stf(rKillerCharacter.seatime.enemydead);
	...
	{
		if (Ships[s] < 0 || Ships[s] == iDeadCharacterIndex)
			continue; // KK
		...
		{
			tmped = 0;
			if (CheckAttribute(schr, "seatime.enemydead"))
				tmped = stf(schr.seatime.enemydead);
...
int GetSeaTime()
{
	...
	if (!CheckAttribute(pchar, "seatime"))
		return 0;
...
bool Ship_Check_Surrender(int chridx, float fSurrMult)
{
	...
	{
		//do a morale calc
		if (!Ship_CheckMorale(chridx, true))
			return false; //Boyer fix for when Ship_CheckMorale does not set .seatime.tempmorale #20170318-45
	}
	float tempmorale = rCharacter.seatime.tempmorale;
	//#20200307-03
	if (tempmorale < 0.05)
	...
	if (surch < 0.1)
		return false; //just don't surrender if less then 10 percent chance
...
void Ship_Neutral(int chridx, string sGroup)
{
	...
	if (CheckAttribute(chr, "relation"))
	{
		chr.oldrelation = "";
		aref arRel;
		makearef(arRel, chr.relation);
		aref arOldRel;
		makearef(arOldRel, chr.oldrelation);
	...
	if (CheckAttribute(pchar, "seaAI.update.forts"))
	{
		...
		for (i = 0; i < num; i++)
		{
			...
			//#20181116-03
			if (GetRelation(nMainCharacterIndex, iFortidx) == RELATION_ENEMY)
				continue;
...
void FlagPerkForCapturedShip(ref refChar)
{
	...
	if (!CheckAttribute(refChar, "nation"))
		return;
