...
float Cannon_DamageEvent()
{
	...
	if (CheckCharacterPerk(aCharacter, "SeaWolf") && rand(2) == 0)
		return 0.0;
	if (IsEquipCharacterByArtefact(aCharacter, "talisman3"))
		return 0.0;
	if (CheckAttribute(&RealShips[sti(aCharacter.Ship.Type)], "Tuning.CannonsSpecial") && rand(9) < 4)
		return 0.0;
	ref rCannon = GetCannonByType(sti(aCharacter.Ship.Cannons.Type));
...
int GetBortIntactCannonsNum(ref rCharacter, string sBort, int iNumCannonsOnBort)
{
	...
	if (!CheckAttribute(rCharacter, "Ship.Cannons.Borts." + sBort_real + ".damages") ||
		!CheckAttribute(rCharacter, "Ship.Cannons.Borts." + sBort + ".damages"))
