...
void Box_OnLoadLocation(ref _location)
{
	...
	for (i = 1; i < MAX_HANDLED_BOXES; i++)
	{
		...
		if (!needRespawn)
		{
			...
			{
				if (CheckAttribute(_location, locatorName + ".notouch"))
				{
					DeleteAttribute(_location, locatorName + ".notouch");
					needRespawn = true;
					_location.(locatorName) = Items_MakeTime(GetTime(), GetDataDay(), GetDataMonth(), GetDataYear());
					continue;
				}
			...
			{
				if (_location.id == "Caiman_StoreHouse" || _location.id == "Reefs_chapter")
					continue;
				//clear items in box
				if (CheckAttribute(_location, locatorName))
				{
