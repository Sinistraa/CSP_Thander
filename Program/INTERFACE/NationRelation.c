...
void InitInterface(string iniName)
{
	...
	if (CheckOfficersPerk(pchar, "SeaDogProfessional"))
	{
		...
		if (!CheckAttribute(pchar, "SeaDogProfessionalSwitch"))
			pchar.SeaDogProfessionalSwitch = 1;
		if (pchar.SeaDogProfessionalSwitch == 1)
			SendMessage(&GameInterface, "lslll", MSG_INTERFACE_MSG_TO_NODE, "SeaDogProfessional_Switch", 2, 1, true);
		else
			SendMessage(&GameInterface, "lslll", MSG_INTERFACE_MSG_TO_NODE, "SeaDogProfessional_Switch", 2, 1, false);
	}
	else
		SetNodeUsing("SeaDogProfessional_Switch", false);
}
...
void FlagsProcess()
{
	...
	if (CheckAttribute(pchar, "DisableChangeFlagMode") || IsCompanion(CharacterFromID("Flint")))
