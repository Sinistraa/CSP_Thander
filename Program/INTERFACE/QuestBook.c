...
void InitInterface(string iniName)
{
	...
	if (InterfaceStates.AltFont == "0")
		SendMessage(&GameInterface, "ls", MSG_INTERFACE_INIT, "RESOURCE\INI\INTERFACES\questbook_alt.ini");
	else
...
void XI_SetQuestData(bool qtitle)
{
	...
	if (InterfaceStates.AltFont == "0")
		XI_SetQuestTitles("QUEST_TITLE_ALT", arefTmp, 0);
	else
		XI_SetQuestTitles("QUEST_TITLE", arefTmp, 0);
	curQuestTop = 0;

	if (InterfaceStates.AltFont == "0")
		SetNodeUsing("QUEST_TITLE_ALT", qtitle);
	else
		SetNodeUsing("QUEST_TITLE", qtitle);
	switch (sti(InterfaceStates.AltFont))
	{
	case 0:
		SetNodeUsing("QUEST_TEXT_ALT_NORMAL", !qtitle);
		break;
	case 1:
		SetNodeUsing("QUEST_TEXT", !qtitle);
		break;
	case 2:
		SetNodeUsing("QUEST_TEXT_ALT", !qtitle);
		break;
	}
	...
	{
		if (InterfaceStates.AltFont == "0")
			SetCurrentNode("QUEST_TITLE_ALT");
		else
...
void HideEncWindows()
{
	...
	if (InterfaceStates.AltFont == "0")
		SetNodeUsing("QUEST_TITLE_ALT", true);
	else
...
void ShowShipsWindow()
{
	...
	if (InterfaceStates.AltFont == "0")
		SetNodeUsing("QUEST_TITLE_ALT", false);
	else
...
void HideQuests()
{
	...
	{
	case 0:
		SetNodeUsing("QUEST_TEXT_ALT_NORMAL", false);
		break;
	case 1:
		SetNodeUsing("QUEST_TEXT", false);
		break;
	case 2:
		SetNodeUsing("QUEST_TEXT_ALT", false);
		break;
	}
...
void QuestTopChange()
{
	if (GetSelectable("QUEST_TITLE") || GetSelectable("QUEST_TITLE_ALT"))
	{
		...
		if (newTop != curQuestTop)
		{
			curQuestTop = newTop;
			if (InterfaceStates.AltFont == "0")
				XI_SetQuestTitles("QUEST_TITLE_ALT", arefTmp, curQuestTop);
			else
...
void ProcScrollPosChange()
{
	...
	else
	{
		...
		{
		case 0:
			SendMessage(&GameInterface, "lslf", MSG_INTERFACE_MSG_TO_NODE, "QUEST_TEXT_ALT_NORMAL", 1, newPos);
			break;
		case 1:
			SendMessage(&GameInterface, "lslf", MSG_INTERFACE_MSG_TO_NODE, "QUEST_TEXT", 1, newPos);
			break;
		case 2:
			SendMessage(&GameInterface, "lslf", MSG_INTERFACE_MSG_TO_NODE, "QUEST_TEXT_ALT", 1, newPos);
			break;
		}
	}
}
...
void ProcScrollChange()
{
	...
	if (InterfaceStates.AltFont == "0")
	{
		if (GetSelectable("QUEST_TITLE_ALT"))
			controlNode = "QUEST_TITLE_ALT";
	}
	else
	{
	...
	case 0:
		if (GetSelectable("QUEST_TEXT_ALT_NORMAL"))
			controlNode = "QUEST_TEXT_ALT_NORMAL";
		break;
	case 1:
		if (GetSelectable("QUEST_TEXT"))
...
void ShowPGGInfo()
{
	...
	{ // нет ПГГ в списке
		...
		string texturedata;
		if (IsCharacterPerkOn(chrefspp, "Grunt"))
			texturedata = "INTERFACES\Sith\Char_Master.tga";
		if (IsCharacterPerkOn(chrefspp, "Trader"))
			texturedata = "INTERFACES\Sith\Char_Merchant.tga";
		if (IsCharacterPerkOn(chrefspp, "Fencer"))
			texturedata = "INTERFACES\Sith\Char_Corsair.tga";
		if (IsCharacterPerkOn(chrefspp, "Adventurer"))
			texturedata = "INTERFACES\Sith\Char_Adventurer.tga";
		if (IsCharacterPerkOn(chrefspp, "Buccaneer"))
			texturedata = "INTERFACES\Sith\Char_Inquisitor.tga";
		if (IsCharacterPerkOn(chrefspp, "Agent"))
			texturedata = "INTERFACES\Sith\Char_SecretAgent.tga";
		if (IsCharacterPerkOn(chrefspp, "SeaWolf"))
			texturedata = "INTERFACES\Sith\Char_SeaWolf.tga";
		SetNewPicture("CHARACTER_PROFESSION", texturedata);
		string portpic;
		switch (GameInterface.(CurTable).(CurRow).td6.icon.image)
		{
		case "France":
			portpic = "loading\enc_fra.tga";
			break;
		case "England":
			portpic = "loading\enc_eng.tga";
			break;
		case "Spain":
			portpic = "loading\enc_spa.tga";
			break;
		case "Holland":
			portpic = "loading\enc_hol.tga";
			break;
		case "Pirate":
			portpic = "loading\enc_pir.tga";
			break;
		}
		...
		string offtype;
		if (!CheckAttribute(chrefspp, "quest.officertype"))
		...
		if (CheckAttribute(RealShips[sti(chrefspp.Ship.Type)], "Tuning.BotPack"))
		...
		if (CheckAttribute(RealShips[sti(chrefspp.Ship.Type)], "Tuning.HighBort"))
	...
	if (CheckAttribute(&GameInterface, CurTable + "." + CurRow + ".index") && GameInterface.(CurTable).(CurRow).td4.str == "Офицер на обучении")
	{
		...
		if (IsCharacterPerkOn(chrefspp1, "Grunt"))
			texturedata = "INTERFACES\Sith\Char_Master.tga";
		if (IsCharacterPerkOn(chrefspp1, "Trader"))
			texturedata = "INTERFACES\Sith\Char_Merchant.tga";
		if (IsCharacterPerkOn(chrefspp1, "Fencer"))
			texturedata = "INTERFACES\Sith\Char_Corsair.tga";
		if (IsCharacterPerkOn(chrefspp1, "Adventurer"))
			texturedata = "INTERFACES\Sith\Char_Adventurer.tga";
		if (IsCharacterPerkOn(chrefspp1, "Buccaneer"))
			texturedata = "INTERFACES\Sith\Char_Inquisitor.tga";
		if (IsCharacterPerkOn(chrefspp1, "Agent"))
			texturedata = "INTERFACES\Sith\Char_SecretAgent.tga";
		if (IsCharacterPerkOn(chrefspp1, "SeaWolf"))
			texturedata = "INTERFACES\Sith\Char_SeaWolf.tga";
		SetNewPicture("CHARACTER_PROFESSION", texturedata);
		SetSPECIALMiniTable("TABLE_SMALLSKILL", chrefspp1);
		SetOTHERMiniTable("TABLE_SMALLOTHER", chrefspp1);
		string offtype1;
		if (!CheckAttribute(chrefspp1, "quest.officertype"))
			offtype1 = "\nСпециальность: Универсал";
		else
...
void FillATableInfo() // Заполним таблицу достижений информацией
{
	...
	for (i = 0; i < z; i++)
	{
		...
		if (sti(pchar.achievements.(attrname)) == 3)
			SetTableRowByAchievement(attrname, sti(pchar.achievements.(attrname)));
	}
	...
	for (i = 0; i < z; i++)
	{
		...
		attrname = GetAttributeName(arcur);
		if (sti(pchar.achievements.(attrname)) == 2)
			SetTableRowByAchievement(attrname, sti(pchar.achievements.(attrname)));
	}
	...
	for (i = 0; i < z; i++)
	{
		...
		attrname = GetAttributeName(arcur);
		if (sti(pchar.achievements.(attrname)) == 1)
			SetTableRowByAchievement(attrname, sti(pchar.achievements.(attrname)));
	}
	...
	for (i = 0; i < z; i++)
	{
		...
		attrname = GetAttributeName(arcur);
		if (sti(pchar.achievements.(attrname)) == 0)
			SetTableRowByAchievement(attrname, sti(pchar.achievements.(attrname)));
	}
...
void SetTableRowByAchievement(string ach_id, int level)
{
	...
	rownumberach++ row = "tr" + rownumberach;
	...
	{
		if (ach_id == "Nation_quest_E" || ach_id == "Nation_quest_F" || ach_id == "Nation_quest_H" || ach_id == "Nation_quest_S" || ach_id == "Nation_quest_P" || ach_id == "Isabella_quest" || ach_id == "LSC_quest" || ach_id == "Teno_quest" || ach_id == "Killbeggars_quest" || ach_id == "Ghostship_quest" || ach_id == "Bluebird_quest" || ach_id == "Berglarsgang_quest" || ach_id == "Mummydust_quest" || ach_id == "Enchantcity_quest" || ach_id == "ships" || ach_id == "bank_money" || ach_id == "CapBladLine" || ach_id == "WhisperLine" || ach_id == "AchShipSearch" || ach_id == "AchOrion" || ach_id == "AchRabotorg" || ach_id == "AchKondotier" || ach_id == "AchTich" || ach_id == "AchRagnar" || ach_id == "AchSalazar" || ach_id == "AchKaskos" || ach_id == "AchUmSamil")
		{
		...
		else
		{
			...
			if (level == 2)
				GameInterface.TABLE_ACHIEVEMENTS.(row).td5.str = "+ 100";
			if (level == 1)
				GameInterface.TABLE_ACHIEVEMENTS.(row).td5.str = "+ 50";
			if (level == 0)
				GameInterface.TABLE_ACHIEVEMENTS.(row).td5.str = "+ 25";
			...
			if (level == 1)
			{
				...
				GameInterface.TABLE_ACHIEVEMENTS.(row).td3.color = argb(255, 255, 171, 171)
																	   GameInterface.TABLE_ACHIEVEMENTS.(row)
																		   .td4.color = argb(255, 255, 171, 171)
																							GameInterface.TABLE_ACHIEVEMENTS.(row)
																								.td5.color = argb(255, 255, 171, 171)
			}
		}
		...
		{
			if (ach_id == "rank")
				strprogress = "Ранг персонажа:";
			if (ach_id == "money")
				strprogress = "Деньги персонажа:";
			if (ach_id == "bank_money")
				strprogress = "Сумма вкладов:";
			if (ach_id == "AchTreasure")
				strprogress = "Найдено кладов:";
			if (ach_id == "AchBlood")
				strprogress = "Вызвано кровотечений:";
			if (ach_id == "AchTravma")
				strprogress = "Нанесено травм:";
			if (ach_id == "AchStanS")
				strprogress = "Нанесено резких ударов:";
			if (ach_id == "AchStanH")
				strprogress = "Вызвано оглушений:";
			if (ach_id == "AchChest")
				strprogress = "Вскрыто сундуков:";
			if (ach_id == "taxes")
				strprogress = "Портовые сборы оплачены:";
			if (ach_id == "AchVzyatka")
				strprogress = "Выдано взяток:";
			if (ach_id == "kills")
				strprogress = "Убийств совершено:";
			if (ach_id == "ship_kills")
				strprogress = "Кораблей потоплено:";
			if (ach_id == "skel_kills")
				strprogress = "Убито нежити:";
			if (ach_id == "poisons")
				strprogress = "Нанесено отравляющих ударов:";
			if (ach_id == "criticals")
				strprogress = "Нанесено критических ударов:";
			if (ach_id == "craft")
				strprogress = "Создано предметов:";
			if (ach_id == "books")
				strprogress = "Книг прочитано:";
			if (ach_id == "officers")
				strprogress = "Офицеров нанято:";
			if (ach_id == "ships")
				strprogress = "Кораблей в эскадре:";
			if (ach_id == "heal_bottles")
				strprogress = "Выпито лечебных зелий:";
			if (ach_id == "AchFood")
				strprogress = "Скушано еды:";
			if (ach_id == "gen_quests")
				strprogress = "Выполнено генераторных квестов:";
			if (ach_id == "AchBuildColony")
				strprogress = "Отстройка колонии:";
			if (ach_id == "AchTurnir")
				strprogress = "Выиграно турниров:";
			if (ach_id == "AchDuelyant")
				strprogress = "Выиграно дуэлей:";
			if (ach_id == "AchShipOrder")
				strprogress = "Заказано кораблей:";
			if (ach_id == "AchShipSearch")
				strprogress = "Исследовано кораблей:";
			if (ach_id == "AchGoldFleet")
				strprogress = "Золотых флотов потоплено:";
			if (ach_id == "AchSityRobbery")
				strprogress = "Городов разграблено:";
		...
		{
			if (ach_id == "rank")
				strprogress = strprogress + sti(pchar.rank) + " / 40";
			if (ach_id == "money")
				strprogress = strprogress + MakeMoneyShow(sti(pchar.money), MONEY_SIGN, MONEY_DELIVER) + " / 15 000 000";
			if (ach_id == "taxes")
				strprogress = strprogress + sti(pchar.questTemp.taxescount) + " / 100";
			if (ach_id == "AchVzyatka")
				strprogress = strprogress + sti(pchar.questTemp.bribescount) + " / 50";
			if (ach_id == "kills")
				strprogress = strprogress + sti(killsall) + " / 500";
			if (ach_id == "ship_kills")
				strprogress = strprogress + sti(killshipsall) + " / 70";
			if (ach_id == "skel_kills")
				strprogress = strprogress + sti(monstersall) + " / 150";
			if (ach_id == "poisons")
				strprogress = strprogress + sti(pchar.questTemp.poisoncount) + " / 150";
			if (ach_id == "criticals")
				strprogress = strprogress + sti(pchar.questTemp.criticalcount) + " / 500";
			if (ach_id == "AchTreasure")
				strprogress = strprogress + sti(pchar.questTemp.treasurecount) + " / 30";
			if (ach_id == "AchBlood")
				strprogress = strprogress + sti(pchar.questTemp.bloodingcount) + " / 200";
			if (ach_id == "AchTravma")
				strprogress = strprogress + sti(pchar.questTemp.traumacount) + " / 300";
			if (ach_id == "AchStanS")
				strprogress = strprogress + sti(pchar.questTemp.swiftcount) + " / 300";
			if (ach_id == "AchStanH")
				strprogress = strprogress + sti(pchar.questTemp.stuncount) + " / 300";
			if (ach_id == "AchChest")
				strprogress = strprogress + sti(pchar.questTemp.chestcount) + " / 30";
			if (ach_id == "craft")
				strprogress = strprogress + sti(pchar.questTemp.craftcount) + " / 1000";
			if (ach_id == "books")
				strprogress = strprogress + sti(pchar.questTemp.bookcount) + " / 10";
			if (ach_id == "officers")
				strprogress = strprogress + sti(pchar.questTemp.officercount) + " / 10";
			if (ach_id == "heal_bottles")
				strprogress = strprogress + sti(pchar.questTemp.healcount) + " / 200";
			if (ach_id == "AchFood")
				strprogress = strprogress + sti(pchar.questTemp.foodcount) + " / 300";
			if (ach_id == "gen_quests")
				strprogress = strprogress + sti(pchar.questTemp.genquestcount) + " / 40";
			if (ach_id == "AchBuildColony")
				strprogress = strprogress + sti(pchar.questTemp.colonystate) + " / 3";
			if (ach_id == "AchTurnir")
				strprogress = strprogress + sti(pchar.questTemp.tournamentcount) + " / 7";
			if (ach_id == "AchDuelyant")
				strprogress = strprogress + sti(pchar.questTemp.duelcount) + " / 20";
			if (ach_id == "AchShipOrder")
				strprogress = strprogress + sti(pchar.questTemp.shipordercount) + " / 20";
			if (ach_id == "AchGoldFleet")
				strprogress = strprogress + sti(pchar.questTemp.GoldFleetNum) + " / 7";
			if (ach_id == "AchSityRobbery")
				strprogress = strprogress + (Statistic_AddValue(PChar, "pir_GrabbingTown", 0) + Statistic_AddValue(PChar, "eng_GrabbingTown", 0) + Statistic_AddValue(PChar, "fra_GrabbingTown", 0) + Statistic_AddValue(PChar, "spa_GrabbingTown", 0) + Statistic_AddValue(PChar, "hol_GrabbingTown", 0)) + " / 15" GameInterface.TABLE_ACHIEVEMENTS.(row).td4.str = strprogress;
		}

		if (level == 1)
		{
			if (ach_id == "rank")
				strprogress = strprogress + sti(pchar.rank) + " / 25";
			if (ach_id == "money")
				strprogress = strprogress + MakeMoneyShow(sti(pchar.money), MONEY_SIGN, MONEY_DELIVER) + " / 5 000 000";
			if (ach_id == "taxes")
				strprogress = strprogress + sti(pchar.questTemp.taxescount) + " / 50";
			if (ach_id == "AchVzyatka")
				strprogress = strprogress + sti(pchar.questTemp.bribescount) + " / 25";
			if (ach_id == "kills")
				strprogress = strprogress + sti(killsall) + " / 150";
			if (ach_id == "ship_kills")
				strprogress = strprogress + sti(killshipsall) + " / 35";
			if (ach_id == "skel_kills")
				strprogress = strprogress + sti(monstersall) + " / 75";
			if (ach_id == "poisons")
				strprogress = strprogress + sti(pchar.questTemp.poisoncount) + " / 100";
			if (ach_id == "criticals")
				strprogress = strprogress + sti(pchar.questTemp.criticalcount) + " / 250";
			if (ach_id == "AchTreasure")
				strprogress = strprogress + sti(pchar.questTemp.treasurecount) + " / 15";
			if (ach_id == "AchBlood")
				strprogress = strprogress + sti(pchar.questTemp.bloodingcount) + " / 100";
			if (ach_id == "AchTravma")
				strprogress = strprogress + sti(pchar.questTemp.traumacount) + " / 150";
			if (ach_id == "AchStanS")
				strprogress = strprogress + sti(pchar.questTemp.swiftcount) + " / 150";
			if (ach_id == "AchStanH")
				strprogress = strprogress + sti(pchar.questTemp.stuncount) + " / 150";
			if (ach_id == "AchChest")
				strprogress = strprogress + sti(pchar.questTemp.chestcount) + " / 15";
			if (ach_id == "craft")
				strprogress = strprogress + sti(pchar.questTemp.craftcount) + " / 500";
			if (ach_id == "books")
				strprogress = strprogress + sti(pchar.questTemp.bookcount) + " / 6";
			if (ach_id == "officers")
				strprogress = strprogress + sti(pchar.questTemp.officercount) + " / 6";
			if (ach_id == "heal_bottles")
				strprogress = strprogress + sti(pchar.questTemp.healcount) + " / 100";
			if (ach_id == "AchFood")
				strprogress = strprogress + sti(pchar.questTemp.foodcount) + " / 150";
			if (ach_id == "gen_quests")
				strprogress = strprogress + sti(pchar.questTemp.genquestcount) + " / 20";
			if (ach_id == "AchBuildColony")
				strprogress = strprogress + sti(pchar.questTemp.colonystate) + " / 3";
			if (ach_id == "AchTurnir")
				strprogress = strprogress + sti(pchar.questTemp.tournamentcount) + " / 3";
			if (ach_id == "AchDuelyant")
				strprogress = strprogress + sti(pchar.questTemp.duelcount) + " / 10";
			if (ach_id == "AchShipOrder")
				strprogress = strprogress + sti(pchar.questTemp.shipordercount) + " / 10";
			if (ach_id == "AchGoldFleet")
				strprogress = strprogress + sti(pchar.questTemp.GoldFleetNum) + " / 3";
			if (ach_id == "AchSityRobbery")
				strprogress = strprogress + (Statistic_AddValue(PChar, "pir_GrabbingTown", 0) + Statistic_AddValue(PChar, "eng_GrabbingTown", 0) + Statistic_AddValue(PChar, "fra_GrabbingTown", 0) + Statistic_AddValue(PChar, "spa_GrabbingTown", 0) + Statistic_AddValue(PChar, "hol_GrabbingTown", 0)) + " / 10" GameInterface.TABLE_ACHIEVEMENTS.(row).td4.str = strprogress;
		}

		if (level == 0)
		{
			if (ach_id == "rank")
				strprogress = strprogress + sti(pchar.rank) + " / 10";
			if (ach_id == "money")
				strprogress = strprogress + MakeMoneyShow(sti(pchar.money), MONEY_SIGN, MONEY_DELIVER) + " / 1 000 000";
			int deposits = 0;
			...
			if (ach_id == "bank_money")
				strprogress = strprogress + MakeMoneyShow(deposits, MONEY_SIGN, MONEY_DELIVER) + " / 50 000 000";
			if (ach_id == "AchShipSearch")
				strprogress = strprogress + sti(pchar.questTemp.shipsearchcount) + " / 124";
			if (ach_id == "taxes")
				strprogress = strprogress + sti(pchar.questTemp.taxescount) + " / 10";
			if (ach_id == "AchVzyatka")
				strprogress = strprogress + sti(pchar.questTemp.bribescount) + " / 10";
			if (ach_id == "kills")
				strprogress = strprogress + sti(killsall) + " / 50";
			if (ach_id == "ship_kills")
				strprogress = strprogress + sti(killshipsall) + " / 10";
			if (ach_id == "skel_kills")
				strprogress = strprogress + sti(monstersall) + " / 30";
			if (ach_id == "poisons")
				strprogress = strprogress + sti(pchar.questTemp.poisoncount) + " / 50";
			if (ach_id == "criticals")
				strprogress = strprogress + sti(pchar.questTemp.criticalcount) + " / 100";
			if (ach_id == "AchTreasure")
				strprogress = strprogress + sti(pchar.questTemp.treasurecount) + " / 5";
			if (ach_id == "AchBlood")
				strprogress = strprogress + sti(pchar.questTemp.bloodingcount) + " / 50";
			if (ach_id == "AchTravma")
				strprogress = strprogress + sti(pchar.questTemp.traumacount) + " / 50";
			if (ach_id == "AchStanS")
				strprogress = strprogress + sti(pchar.questTemp.swiftcount) + " / 50";
			if (ach_id == "AchStanH")
				strprogress = strprogress + sti(pchar.questTemp.stuncount) + " / 50";
			if (ach_id == "AchChest")
				strprogress = strprogress + sti(pchar.questTemp.chestcount) + " / 5";
			if (ach_id == "craft")
				strprogress = strprogress + sti(pchar.questTemp.craftcount) + " / 300";
			if (ach_id == "books")
				strprogress = strprogress + sti(pchar.questTemp.bookcount) + " / 3";
			if (ach_id == "officers")
				strprogress = strprogress + sti(pchar.questTemp.officercount) + " / 3";
			if (ach_id == "ships")
				strprogress = strprogress + sti(GetCompanionQuantity(pchar)) + " / " + (COMPANION_MAX);
			if (ach_id == "heal_bottles")
				strprogress = strprogress + sti(pchar.questTemp.healcount) + " / 50";
			if (ach_id == "AchFood")
				strprogress = strprogress + sti(pchar.questTemp.foodcount) + " / 50";
			if (ach_id == "gen_quests")
				strprogress = strprogress + sti(pchar.questTemp.genquestcount) + " / 10";
			if (ach_id == "AchBuildColony")
				strprogress = strprogress + sti(pchar.questTemp.colonystate) + " / 3";
			if (ach_id == "AchTurnir")
				strprogress = strprogress + sti(pchar.questTemp.tournamentcount) + " / 1";
			if (ach_id == "AchDuelyant")
				strprogress = strprogress + sti(pchar.questTemp.duelcount) + " / 5";
			if (ach_id == "AchShipOrder")
				strprogress = strprogress + sti(pchar.questTemp.shipordercount) + " / 5";
			if (ach_id == "AchGoldFleet")
				strprogress = strprogress + sti(pchar.questTemp.GoldFleetNum) + " / 1";
			if (ach_id == "AchSityRobbery")
				strprogress = strprogress + (Statistic_AddValue(PChar, "pir_GrabbingTown", 0) + Statistic_AddValue(PChar, "eng_GrabbingTown", 0) + Statistic_AddValue(PChar, "fra_GrabbingTown", 0) + Statistic_AddValue(PChar, "spa_GrabbingTown", 0) + Statistic_AddValue(PChar, "hol_GrabbingTown", 0)) + " / 5";
			GameInterface.TABLE_ACHIEVEMENTS.(row).td4.str = strprogress;
		}

		GameInterface.TABLE_ACHIEVEMENTS.(row).td4.str = strprogress;

		if (ach_id == "Nation_quest_E" || ach_id == "Nation_quest_F" || ach_id == "Nation_quest_H" || ach_id == "Nation_quest_S" || ach_id == "Nation_quest_P" || ach_id == "Isabella_quest" || ach_id == "LSC_quest" || ach_id == "Teno_quest" || ach_id == "Killbeggars_quest" || ach_id == "Ghostship_quest" || ach_id == "Bluebird_quest" || ach_id == "Berglarsgang_quest" || ach_id == "Mummydust_quest" || ach_id == "Enchantcity_quest" || ach_id == "CapBladLine" || ach_id == "WhisperLine" || ach_id == "AchOrion" || ach_id == "AchRabotorg" || ach_id == "AchKondotier" || ach_id == "AchTich" || ach_id == "AchRagnar" || ach_id == "AchSalazar" || ach_id == "AchKaskos" || ach_id == "AchUmSamil")
		{
...
void FillShipPlaceTable(string _tabName)
{
	...
	for (i = 1; i < MAX_CHARACTERS; i++)
	{
		...
		if (CheckAttribute(chref, "OfficerInStockMan"))
		{
			...
			GameInterface.(_tabName).(row).td1.scale = 0.8;
			GameInterface.(_tabName).(row).td2.str = "---" GameInterface.(_tabName).(row).td3.str = "---";
			GameInterface.(_tabName).(row).td3.scale = 0.8;
			...
			GameInterface.(_tabName).(row).td5.scale = 0.8;
			GameInterface.(_tabName).(row).td6.icon.group = "NATIONS";
			if (!CheckAttribute(pchar, "PirateOrder"))
				GameInterface.(_tabName).(row).td6.icon.image = "SPAIN";
			else
				GameInterface.(_tabName).(row).td6.icon.image = "PIRATE";
			GameInterface.(_tabName).(row).td6.icon.width = 40;
			GameInterface.(_tabName).(row).td6.icon.height = 40;
			GameInterface.(_tabName).(row).td6.icon.offset = "-2, 0";
...
void TableSelectChange()
{
	...
	if (CurTable == "SHIP_TABLE_LIST_LEFT")
	{
		Last_Left_Ship = sti(GameInterface.(CurTable).(CurRow).index);
		SetShipOTHERTableInfo("SHIP_TABLE_OTHER_LEFT");
	} //запоминаем новый выбранный корабль и обновляем таблицу
	if (CurTable == "SHIP_TABLE_LIST_RIGHT")
	{
		Last_Right_Ship = sti(GameInterface.(CurTable).(CurRow).index);
		SetShipOTHERTableInfo("SHIP_TABLE_OTHER_RIGHT");
	}
}
...
void FillPriceList(string _tabName, string attr1, string attr2)
{
	...
	if (attr1 != "")
	{
		...
		{
			if (attr2 == "1")
				StoreNum = GetCharacterIndex(attr1);
			else
			...
			if (StoreNum > 0)
			{
				...
				if (attr2 == "1")
					refStorage = &characters[StoreNum];
				else
					refStorage = &stores[StoreNum];
				for (i = 0; i < GOODS_QUANTITY; i++)
				{
					row = "tr" + n;
					if (attr2 == "1")
						iStoreQ = GetCargoGoods(refStorage, i);
					else
...
void FillShipInfoEncy(string _tabName)
{
	...
	for (int i = 0; i < SHIP_TYPES_QUANTITY - 1; i++)
	{
		...
		if (!CheckAttribute(Pchar, "Encyclopedia." + sShip) && bFillEncyShips)
			continue;

		int nClass = sti(refBaseShip.Class);
		string snation;
		if (_tabName == "SHIP_TABLE_LIST_LEFT" && FState_SHIP_L != 8 && FState_SHIP_L != nClass)
			continue;
		if (_tabName == "SHIP_TABLE_LIST_RIGHT" && FState_SHIP_R != 8 && FState_SHIP_R != nClass)
			continue;
		...
		{
			snation = ShipNation[FState_SHIP_N_R - 1];
			if (refBaseShip.nation.(snation))
			{
			}
			else
				continue;
		...
		{
			snation = ShipNation[FState_SHIP_N_L - 1];
			if (refBaseShip.nation.(snation))
			{
			}
			else
		...
		if (refBaseShip.Type.War == true)
			spectypes = "Военный";
		if (refBaseShip.Type.Merchant == true)
			spectypes = "Торговый";
		if (refBaseShip.Type.Merchant == true && refBaseShip.Type.War == true)
			spectypes = "Универс.";
		GameInterface.(_tabName).(sRow).td4.str = spectypes;
		GameInterface.(_tabName).(sRow).td4.scale = 0.8;

		k++;
		if (!CheckAttribute(refBaseShip, "QuestShip") && bFillEncyShips)
			pchar.questTemp.shipsearchcount = k;
	}
	ShowInfoWindowEncyShip();
	if (bFillEncyShips && pchar.questTemp.shipsearchcount == "124")
		UnlockAchievement("AchShipSearch", 3);
	...
	if (iselected < 2)
		...
		GameInterface.(_tabName).top = iselected - 2;
...
int FindLastShip(string _tabName)
{
	...
	if (_tabName == "SHIP_TABLE_LIST_LEFT")
	{
		for (q = 0; q < (SHIP_TYPES_QUANTITY + 1); q++)
		{
			...
			if (!CheckAttribute(GameInterface, "SHIP_TABLE_LIST_LEFT." + sRow + ".index"))
			{
				Last_Left_Ship = -1;
				return -1;
			} //таблица кончилась, корабль не нашли, сбрасываем выделение
			if (GameInterface.(_tabName).(sRow).index == Last_Left_Ship)
				return q; //корабль не отфильтровался, он в строчке q+1
		}
		Last_Left_Ship = -1;
	}
	else
	{
		...
		{
			sRow = "tr" + (q + 1);
			if (!CheckAttribute(GameInterface, "SHIP_TABLE_LIST_RIGHT." + sRow + ".index"))
			{
				Last_Right_Ship = -1;
				return -1;
			}
			if (GameInterface.(_tabName).(sRow).index == Last_Right_Ship)
				return q;
		}
		Last_Right_Ship = -1;
	}
...
{
	int iShip;
	if (_tabName == "SHIP_TABLE_OTHER_LEFT")
		iShip = Last_Left_Ship;
	else
		iShip = Last_Right_Ship;
	Table_Clear(_tabName, false, true, false);
	...
	if (Last_Left_Ship > -1)
	{
		...
		bool bQuest = false;
		if (checkattribute(refBaseShipL, "QuestShip"))
			bQuest = true; //не показываем флаги наций квестовым
		if (!bQuest && refBaseShipL.nation.england == true)
			SetNewPicture("S_NATION_E_L", "loading\Enc_Eng.tga");
		else
			SetNewPicture("S_NATION_E_L", "");
		if (!bQuest && refBaseShipL.nation.france == true)
			SetNewPicture("S_NATION_F_L", "loading\Enc_Fra.tga");
		else
			SetNewPicture("S_NATION_F_L", "");
		if (!bQuest && refBaseShipL.nation.holland == true)
			SetNewPicture("S_NATION_H_L", "loading\Enc_Hol.tga");
		else
			SetNewPicture("S_NATION_H_L", "");
		if (!bQuest && refBaseShipL.nation.spain == true)
			SetNewPicture("S_NATION_S_L", "loading\Enc_Spa.tga");
		else
			SetNewPicture("S_NATION_S_L", "");
		if (!bQuest && refBaseShipL.nation.pirate == true)
			SetNewPicture("S_NATION_P_L", "loading\Enc_Pir.tga");
		else
			SetNewPicture("S_NATION_P_L", "");
		...
		{
			if (_tabName == "SHIP_TABLE_OTHER_LEFT")
				SetFormatedText("SHIP_INFO_TEXT_LEFT", "ИД корабля: " + refBaseShipL.name + "\n" + GetConvertStr(sShip, "ShipsDescribe.txt"));
		}
		else
		{
			if (_tabName == "SHIP_TABLE_OTHER_LEFT")
				SetFormatedText("SHIP_INFO_TEXT_LEFT", GetConvertStr(sShip, "ShipsDescribe.txt"));
		}
	}
	if (Last_Right_Ship > -1)
	{
		...
		bQuest = false;
		if (checkattribute(refBaseShipR, "QuestShip"))
			bQuest = true; //не показываем флаги наций квестовым
		if (!bQuest && refBaseShipR.nation.england == true)
			SetNewPicture("S_NATION_E_R", "loading\Enc_Eng.tga");
		else
			SetNewPicture("S_NATION_E_R", "");
		if (!bQuest && refBaseShipR.nation.france == true)
			SetNewPicture("S_NATION_F_R", "loading\Enc_Fra.tga");
		else
			SetNewPicture("S_NATION_F_R", "");
		if (!bQuest && refBaseShipR.nation.holland == true)
			SetNewPicture("S_NATION_H_R", "loading\Enc_Hol.tga");
		else
			SetNewPicture("S_NATION_H_R", "");
		if (!bQuest && refBaseShipR.nation.spain == true)
			SetNewPicture("S_NATION_S_R", "loading\Enc_Spa.tga");
		else
			SetNewPicture("S_NATION_S_R", "");
		if (!bQuest && refBaseShipR.nation.pirate == true)
			SetNewPicture("S_NATION_P_R", "loading\Enc_Pir.tga");
		else
			SetNewPicture("S_NATION_P_R", "");
		...
		{
			if (_tabName == "SHIP_TABLE_OTHER_RIGHT")
				SetFormatedText("SHIP_INFO_TEXT_RIGHT", "ИД корабля: " + refBaseShipR.name + "\n" + GetConvertStr(sShip, "ShipsDescribe.txt"));
		}
		else
		{
			if (_tabName == "SHIP_TABLE_OTHER_RIGHT")
				SetFormatedText("SHIP_INFO_TEXT_RIGHT", GetConvertStr(sShip, "ShipsDescribe.txt"));
		}
	}
	//<--показываем флаги
	Table_UpdateWindow(_tabName);
...
void GiveItemToTraderByType(aref ch, string type)
{
	...
	for (i = 0; i < ITEMS_QUANTITY; i++)
	{
		...
		case "blade":
			...
			{
				if (itm.groupID == BLADE_ITEM_TYPE)
...
void AddToTable(string _tabName, string type)
{
	...
	for (i = 0; i < ITEMS_QUANTITY; i++)
	{
		...
		if (leftQty > 0)
		{
			if (type == "blade" && FState_BLADE && Items[i].FencingType != BladeType[FState_BLADE - 1])
				continue; //фильтрация типа легкое тяжелое

			if (type == "blade" && !checkattribute(&Items[i], "quality"))
				continue; //отфильтровываю "безоружный" и всякие посторонние предметы, у которых нет "качества"
			if (type == "blade" && FState_BLADEQ && Items[i].quality != BladeTypeQ[FState_BLADEQ - 1])
				continue; //фильтрация клинков по качеству

			if (type == "gun" && FState_GunQ)
			{
				if (Items[i].groupID == AMMO_ITEM_TYPE || !checkattribute(&Items[i], "quality"))
					continue;
			}
			if (type == "gun" && FState_GunQ && Items[i].quality != GunTypeQ[FState_GunQ - 1])
				continue; //фильтрация по качеству

			if (type == "bonusitem" && FState_BONUS && !HasSubStr(sGood, BonusType[FState_BONUS - 1]))
				continue; //фильтрация типа предметов
			...
			{
			case "blade":
				if (CheckAttribute(Items[i], "Generation"))
					GameInterface.(_tabName).(row).td2.str = makeint(Items[i].Generation.dmg_min.min) + "-" + makeint(Items[i].Generation.dmg_min.max) + " / " + makeint(Items[i].Generation.dmg_max.min) + "-" + makeint(Items[i].Generation.dmg_max.max);
				else
				...
				if (CheckAttribute(Items[i], "Generation"))
					GameInterface.(_tabName).(row).td4.str = FloatToString(stf(Items[i].Generation.Weight.min), 1) + " - " + FloatToString(stf(Items[i].Generation.Weight.max), 1);
				else
			...
			case "gun":
				...
				break;
			case "food":
				if (CheckAttribute(Items[i], "food.energy"))
					GameInterface.(_tabName).(row).td2.str = makeint(Items[i].food.energy) + " EN";
				else
					GameInterface.(_tabName).(row).td2.str = "-------";
				if (CheckAttribute(Items[i], "potion.health"))
					GameInterface.(_tabName).(row).td2.str = makeint(Items[i].potion.health) + " HP";
				if (CheckAttribute(Items[i], "potion.antidote"))
					GameInterface.(_tabName).(row).td3.str = "Да";
				else
...
string GetPerkInfo(ref itm)
{
	...
	{
		if (itm.ReqPerk == "Gunman")
			return "***\nТребуется перк 'Стрелок'.";
		if (itm.ReqPerk == "GunProfessional")
