...
void InitInterface_R(string iniName, ref pStore)
{
	...
	curRow = "tr1";
	refStore = pStore;
	if (refStore.colony == "Tortuga" || refStore.colony == "Villemstad" || refStore.colony == "PortRoyal" || refStore.colony == "Havana")
		iMaxGoodsStore = 100000;
...
{ // лево
	...
	if (inc > sti(GetStorageGoodsQuantity(refStore, idx)))
		inc = sti(GetStorageGoodsQuantity(refStore, idx));
	if (inc > 0)
	{
...
{ // право
	...
	idx = sti(GameInterface.TABLE_LIST.(curRow).index);
	inc = sti(Goods[idx].Units) * inc;
	if (inc > GetCargoGoods(refCharacter, idx))
		inc = GetCargoGoods(refCharacter, idx);
	if (inc > 0)
	{
...
void TransactionOK()
{
	...
	{ //отправляем на склад
		if (GetStorageUsedWeight(refStore) + makeint(nTradeQuantity * sti(Goods[iCurGoodsIdx].weight) / sti(Goods[iCurGoodsIdx].units)) > iMaxGoodsStore)
			return; // Фикс безразмерного склада
		SetStorageGoods(refStore, iCurGoodsIdx, iStoreQty + nTradeQuantity);
