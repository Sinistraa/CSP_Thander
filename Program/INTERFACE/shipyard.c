...
void ShowInfoWindow()
{
	...
	case "EXTRAHULLON":
		...
		if (CheckAttribute(RealShips[sti(rChr.Ship.Type)], "Tuning.HullSpecial"))
			sGroupPicture = "EXTRAHULLON";
		else
			sGroupPicture = "EXTRAHULLOFF";
		xx = 256;
		yy = 256;
		break;
	case "EXTRASAILON":
		...
		sGroup = "SHIP_UPGRADES";
		if (CheckAttribute(RealShips[sti(rChr.Ship.Type)], "Tuning.SailsSpecial"))
			sGroupPicture = "EXTRASAILON";
		else
			sGroupPicture = "EXTRASAILOFF";
		xx = 256;
		yy = 256;
		break;
	case "EXTRAGUNSON":
		...
		sGroup = "SHIP_UPGRADES";
		if (CheckAttribute(RealShips[sti(rChr.Ship.Type)], "Tuning.CannonsSpecial"))
			sGroupPicture = "EXTRAGUNSON";
		else
			sGroupPicture = "EXTRAGUNSOFF";
		xx = 256;
		yy = 256;
		break;
	case "EXTRAHULLKRENGON":
		...
		sGroup = "SHIP_UPGRADES";
		if (CheckAttribute(RealShips[sti(rChr.Ship.Type)], "Tuning.CuBot"))
			sGroupPicture = "EXTRAHULLKRENGON";
		else
			sGroupPicture = "EXTRAHULLKRENGOFF";
		xx = 256;
		yy = 256;
		break;
	case "EXTRACAPACITYON":
		...
		sGroup = "SHIP_UPGRADES";
		if (CheckAttribute(RealShips[sti(rChr.Ship.Type)], "Tuning.BotPack"))
			sGroupPicture = "EXTRACAPACITYON";
		else
			sGroupPicture = "EXTRACAPACITYOFF";
		xx = 256;
		yy = 256;
		break;
	case "EXTRABIGSIDESON":
		...
		sGroup = "SHIP_UPGRADES";
		if (CheckAttribute(RealShips[sti(rChr.Ship.Type)], "Tuning.HighBort"))
			sGroupPicture = "EXTRABIGSIDESON";
		else
			sGroupPicture = "EXTRABIGSIDESOFF";
		xx = 256;
		yy = 256;
		break;
	}
	...
	if (CurTable == "SHIPSUP_TABLE")
	{
		...
		case "tr1":
			if (CheckAttribute(RealShips[iShip], "Tuning.HullSpecial"))
				...
				upgradevalue = makeint(sti(RealShips[iShip].price) / 2);
			uptype = 1;
			break;
		case "tr2":
			if (CheckAttribute(RealShips[iShip], "Tuning.SailsSpecial"))
				...
				upgradevalue = makeint(sti(RealShips[iShip].price) * 0.35);
			uptype = 2;
			break;
		case "tr3":
			if (CheckAttribute(RealShips[iShip], "Tuning.CannonsSpecial"))
				...
				upgradevalue = makeint(sti(RealShips[iShip].price) * 0.25);
			uptype = 3;
			break;
		case "tr4":
			if (CheckAttribute(RealShips[iShip], "Tuning.CuBot"))
				...
				upgradevalue = makeint(sti(RealShips[iShip].price) * 0.4);
			uptype = 4;
			break;
		case "tr5":
			if (CheckAttribute(RealShips[iShip], "Tuning.BotPack"))
				...
				upgradevalue = makeint(sti(RealShips[iShip].price) * 0.5);
			uptype = 5;
			break;
		case "tr6":
		...
		if (upgradevalue == 0)
		{
			...
			SetFormatedText("SHIPSUP_VALUE", FindRussianMoneyString(upgradevalue) + ", апгрейд займёт часов: не менее " + timePreUpgrade);
		...
		if (sti(pchar.money) >= upgradevalue && upsq < 3)
			SetSelectable("SHIPSUP_DOUP", true);
		if (sti(pchar.money) < upgradevalue)
...
void ShowMsgMenu(bool change)
{
	...
	if (change)
		SetNodeUsing("MSG_CREW_CHECKBOX", false);
	else
		SetNodeUsing("MSG_CREW_CHECKBOX", true);
...
void DoSellShip(bool _refresh)
{
	...
	if (shipIndex != -1 && sti(chref.ship.type) != SHIP_NOTUSED)
	{
		...
		if (_refresh)
		{
			...
			{
				if (SellState == "Max")
					SendCrewToPchar(chref);
				if (SellState == "Opt")
					SendOptCrewToPchar(chref);
				if (SellState == "Fire")
					FireCrew(chref);
			...
			if (crewn > 0)
			{
				for (i = 1; i < COMPANION_MAX; i++)
				{
					...
					if (cn > 0 && crewn > 0)
					{
						...
						if (crewn != 0)
							crewn = SendCrewToShip(chref, chreff, crewn);
					}
				}
				if (crewn > 0)
					SendCrewToTavern(chref, crewn);
			}
		}
		else
		{
			pchar.Ship.Crew.Exp.Sailors = (stf(pchar.Ship.Crew.Exp.Sailors) * GetCrewQuantity(pchar) +
										   stf(chref.Ship.Crew.Exp.Sailors) * GetCrewQuantity(chref)) /
										  fTemp2;
			pchar.Ship.Crew.Exp.Cannoners = (stf(pchar.Ship.Crew.Exp.Cannoners) * GetCrewQuantity(pchar) +
											 stf(chref.Ship.Crew.Exp.Cannoners) * GetCrewQuantity(chref)) /
											fTemp2;
			pchar.Ship.Crew.Exp.Soldiers = (stf(pchar.Ship.Crew.Exp.Soldiers) * GetCrewQuantity(pchar) +
											stf(chref.Ship.Crew.Exp.Soldiers) * GetCrewQuantity(chref)) /
										   fTemp2;

			pchar.Ship.Crew.morale = (stf(pchar.Ship.Crew.morale) * GetCrewQuantity(pchar) +
									  stf(chref.Ship.Crew.morale) * GetCrewQuantity(chref)) /
									 fTemp2;
...
int SendCrewToShip(ref chref, ref chreff, int crewnum)
{
	...
	{
		chreff.Ship.Crew.Exp.Sailors = (stf(chreff.Ship.Crew.Exp.Sailors) * GetCrewQuantity(chreff) +
										stf(chref.Ship.Crew.Exp.Sailors) * crewnum) /
									   fTemp2;
		chreff.Ship.Crew.Exp.Cannoners = (stf(chreff.Ship.Crew.Exp.Cannoners) * GetCrewQuantity(chreff) +
										  stf(chref.Ship.Crew.Exp.Cannoners) * crewnum) /
										 fTemp2;
		chreff.Ship.Crew.Exp.Soldiers = (stf(chreff.Ship.Crew.Exp.Soldiers) * GetCrewQuantity(chreff) +
										 stf(chref.Ship.Crew.Exp.Soldiers) * crewnum) /
										fTemp2;

		chreff.Ship.Crew.morale = (stf(chreff.Ship.Crew.morale) * GetCrewQuantity(chreff) +
								   stf(chref.Ship.Crew.morale) * crewnum) /
								  fTemp2;
			...
			if (crewn > 0)
			{
				for (i = 1; i < COMPANION_MAX; i++)
				{
					...
					if (cn > 0 && crewn > 0)
					{
						chreff = GetCharacter(cn);
						if (crewn != 0)
							crewn = SendOptCrewToShip(chref, chreff, crewn);
					}
				}
				if (crewn > 0)
					SendCrewToTavern(chref, crewn);
...
{
	if (GetCrewQuantity(chreff) >= GetOptCrewQuantity(chreff))
		return crewnum;
...
void RepairMoneyShow()
{
	...
	if (SendMessage(&GameInterface, "lsll", MSG_INTERFACE_MSG_TO_NODE, "REPAIR_Soiling_CHECKBOX", 3, 1))
		soiltime = (8 - GetCharacterShipClass(xi_refCharacter));
	else
		soiltime = 0;
	...
	if (iTimeM < 1)
		...
		_sTime = "часов: " + iTimeM / 60 + " | минут: " + its(iTimeM - (iTimeM / 60) * 60) + "\n";
...
void OpenShipUp()
{
	...
	for (int i = 0; i < 6; i++)
	{
		...
		case 0:
			...
			else
			{
				...
				if (RealShips[iShip].Type.War == true && RealShips[iShip].Type.Merchant == false)
					...
					GameInterface.SHIPSUP_TABLE.(row).td3.str = "Апгрейд невозможен для данного корабля";
			}
			if (RealShips[iShip].Type.War == false && RealShips[iShip].Type.Merchant == true)
				GameInterface.SHIPSUP_TABLE.(row).td3.str = "Апгрейд невозможен";
			break;
		case 1:
		...
		if (upsq >= 3)
			SetFormatedText("SHIPSUP_VALUE", "Максимальное количество апгрейдов");
		else
			SetFormatedText("SHIPSUP_VALUE", FindRussianMoneyString(upgradevalue));
...
{
	if (upgradevalue > 0)
		AddMoneyToCharacter(pchar, -upgradevalue);
	...
	case 5:
		if (!CheckAttribute(shTo, "Tuning.BotPack"))
		{
			...
			if (xi_refCharacter.ship.hp > sti(shTo.HP))
				xi_refCharacter.ship.hp = sti(shTo.HP);
		}
		else
		{
...
{
	int iMax = 3;
	if (CheckAttribute(rShip, "hullNums"))
		iMax = sti(rShip.hullNums);
	return iMax;
}
