...
void InitInterface(string iniName)
{
	...
	{
	case "man":
		SetNewPicture("SETUP_BIG_PICTURE", "interfaces\sith\CharEquip_Man.tga");
		break;
	case "woman":
		SetNewPicture("SETUP_BIG_PICTURE", "interfaces\sith\CharEquip_Woman.tga");
		break;
	case "skeleton":
		SetNewPicture("SETUP_BIG_PICTURE", "interfaces\sith\CharEquip_Skeleton.tga");
		break;
	}
...
void SetVariable()
{
	...
	{
		if (CheckForExchangeAllowed(xi_refCharacter))
			SetNodeUsing("EXCHANGE", true);
		else
			SetNodeUsing("EXCHANGE", false);
	}
	else
	...
	{
		FillSetName(i);
	} //Заполняем имена комплектов снаряжения
	HideItemInfo();
	...
	if (!checkattribute(xi_refCharacter, "selectedSet"))
		xi_refCharacter.selectedSet = 1; //первое открытие

	if (iEQUIP_SET == sti(xi_refCharacter.selectedSet))
		ReEquipCharacter(); //обновляем экипировку при открытии инвентаря - могло добавиться недостающее пока бегали где-то
	else
	{
...
void FillItemsTable(int _mode) // 1 - все 2 - оружие 3 - остальное
{
	...
	for (i = 0; i < GetAttributesNum(rootItems); i++)
	{
		...
		if (Items_FindItem(GetAttributeName(curItem), &arItem) >= 0)
		{
			...
			if (_mode == 1 && arItem.ItemType == "MAP" && ok2)
				continue ok2 = ok2 && arItem.ItemType == "MAP";
			if (_mode == 2 && !ok && ok2)
				continue;
			ok = ok || arItem.ItemType == "MAP";
			if (_mode == 3 && ok && ok2)
				continue;
			if (_mode == 4 && arItem.ItemType != "MAP")
				continue;
			...
			if (GetCharacterItem(xi_refCharacter, sGood) > 0)
			{
				...
				if (IsEquipCharacterByItem(xi_refCharacter, sGood))
					GameInterface.TABLE_ITEMS.(row).td1.color = argb(255, 196, 196, 255); //на этом интерфейсе надо подцвечивать и текущий комплект???
...
float _GetAttackFactor(ref rBlade, string sType, ref kPerk)
{
	...
	case "fient":
		kAttackDmg = 0.5;
		if (CheckCharacterPerk(xi_refCharacter, "Agent"))
			kAttackDmg = kAttackDmg * 2;
		break;
	}
...
bool ThisItemCanBeEquip(aref arItem)
{
	if (CheckAttribute(xi_refCharacter, "nonremovable"))
		return false;
	if (HasSubStr(loadedLocation.id, "FencingTown"))
		return false;
	...
	{
		if (!CheckAttribute(xi_refCharacter, "StrangeMushrooms"))
			xi_refCharacter.StrangeMushrooms = 0;
		if (!CheckAttribute(xi_refCharacter, "StrangePotion"))
			xi_refCharacter.StrangePotion = 0;

		if (arItem.id == "StrangeMushrooms" && sti(xi_refCharacter.StrangeMushrooms) < 5)
			return true;
		else
		{
			if (arItem.id == "StrangePotion" && sti(xi_refCharacter.StrangePotion) < 5)
				return true;
		}
		else
		{
			if (arItem.id == "StrangeElixir" && !CheckAttribute(xi_refCharacter, "StrangeElixir"))
				return true;
		}
	}
	...
	{
		if (arItem.ID == "GunPowder" || arItem.ID == "shotgun_cartridge" || arItem.ID == "GunCap_colt" || arItem.ID == "12_gauge")
			return false;
		SendMessage(&GameInterface, "lsls", MSG_INTERFACE_MSG_TO_NODE, "EQUIP_BUTTON", 0, "#" + XI_ConvertString("Equip that"));
	}
	...
	if (arItem.groupID == BLADE_ITEM_TYPE && CheckAttribute(xi_refCharacter, "DontChangeBlade"))
		return false;
	if (arItem.groupID == GUN_ITEM_TYPE && CheckAttribute(xi_refCharacter, "DontChangeGun"))
		return false;
	...
	if (arItem.groupID == GUN_ITEM_TYPE)
	{
		...
		if (arItem.id == "mushket2x2")
			return false; // Мушкет квестового офа
		if (HasSubStr(arItem.id, "mushket") && !HasSubStr(arItem.id, "mushket_drob"))
		{
			if (!IsCharacterPerkOn(xi_refCharacter, "Gunman") && !IsCharacterPerkOn(xi_refCharacter, "GunProfessional"))
				return false;
		}

		if (CheckAttribute(arItem, "ReqPerk"))
		{
			if (arItem.ReqPerk == "Gunman" && !IsCharacterPerkOn(xi_refCharacter, "Gunman") && !IsCharacterPerkOn(xi_refCharacter, "GunProfessional"))
				return false;
			if (arItem.ReqPerk == "GunProfessional" && !IsCharacterPerkOn(xi_refCharacter, "GunProfessional"))
				return false;
		}
	...
	if (IsEquipCharacterByItem(xi_refCharacter, arItem.id))
	{
		...
		if (arItem.groupID == PATENT_ITEM_TYPE) //aw013
		{
			...
			{
				if (pchar.questTemp.Ascold == "ReciveInformationManowar" || pchar.questTemp.Ascold == "Ascold_ImMummy")
					return true;
			}
			return false;
		}
	...
	else
	{
		...
		if (HasSubStr(arItem.id, "chest"))
			SendMessage(&GameInterface, "lsls", MSG_INTERFACE_MSG_TO_NODE, "EQUIP_BUTTON", 0, "#Взломать");
		else
			SendMessage(&GameInterface, "lsls", MSG_INTERFACE_MSG_TO_NODE, "EQUIP_BUTTON", 0, "#" + XI_ConvertString("Equip that"));
		if (arItem.id == "Chest")
		{
	...
	if (arItem.groupID == CIRASS_ITEM_TYPE && xi_refCharacter.model == "Protocusto")
		return false;
	return true;
}
...
void EquipPress()
{
	...
	if (CheckAttribute(itmRef, "groupID"))
	{
		...
		if (itmGroup == BLADE_ITEM_TYPE && CheckAttribute(xi_refCharacter, "DontChangeBlade"))
			return false;
		if (itmGroup == GUN_ITEM_TYPE && CheckAttribute(xi_refCharacter, "DontChangeGun"))
			return false;
	...
	if (HasSubStr(itmRef.id, "Strange"))
	{
		...
		if (itmRef.id == "StrangeMushrooms" && sti(xi_refCharacter.StrangeMushrooms) < 5)
		{
			...
			if (IsMainCharacter(xi_refCharacter))
				Log_Info("Ваше максимальное здоровье увеличено на " + strangeHpBonus + "!");
			else
				Log_Info(GetFullName(xi_refCharacter) + " увеличивает максимальное здоровье на " + strangeHpBonus + "!")
		...
		else
		{
			if (itmRef.id == "StrangePotion" && sti(xi_refCharacter.StrangePotion) < 5)
			{
				...
				PlaySound("interface\heartbeat.wav");
				if (IsMainCharacter(xi_refCharacter))
					Log_Info("Ваша максимальная энергия увеличилась на " + strangeEnBonus + "!");
				else
				{
					Log_Info(GetFullName(xi_refCharacter) + " увеличивает максимальную энергию на " + strangeEnBonus + "!")
				}
				DumpAttributes(pchar);
				ApplayNewSkill(pchar, "", 0);
				TakeNItems(xi_refCharacter, itmRef.id, -1);
			...
			else
			{
				if (itmRef.id == "StrangeElixir" && !CheckAttribute(xi_refCharacter, "StrangeElixir"))
				{
					...
					PlaySound("interface\heartbeat.wav");
					if (IsMainCharacter(xi_refCharacter))
						Log_Info("Ваш урон в ближнем бою навсегда увеличен на десять процентов!");
					else
					{
						Log_Info(GetFullName(xi_refCharacter) + " навсегда увеличивает урон в ближнем бою на десять процентов!")
					}
					DumpAttributes(pchar);
					ApplayNewSkill(pchar, "", 0);
					TakeNItems(xi_refCharacter, itmRef.id, -1);
	...
	if (HasSubStr(itmRef.id, "Tube") && IsMainCharacter(xi_refCharacter))
	{
		...
		if (CheckAttribute(pchar, "chr_ai.bonushptube"))
			drugEffects++;
		if (drugEffects == 2 && rand(20) == 0)
			GetHigh();
		if (drugEffects == 2 && rand(4) == 0)
			GetHigh();
		if (drugEffects == 3)
			GetHigh();

		if (CheckAttribute(pchar, "drugstaken"))
			pchar.drugstaken = sti(pchar.drugstaken) + 1;
		else
		{
			pchar.drugstaken = 1;
			SetTimerFunction("ClearDrugs", 0, 0, 10);
		}
		DumpAttributes(pchar);
		ApplayNewSkill(pchar, "", 0);
...
void SetMapCross(string _MapLocId)
{
	...
	case "Antigua_Grot":
		left = 622;
		top = 235;
		break; //{в пещере справа от залива Уиллогби}
	case "SentJons_TownCave":
		left = 601;
		top = 235;
		break; //{в подземелье города Сент-Джонс}

	case "Beliz_Cave":
		left = 58;
		top = 181;
		break; //{в пещере справа от выхода из Белиза}
	case "Beliz_Cave_2":
		left = 55;
		top = 225;
		break; //{в пещере напротив выхода из Белиза}

	case "Panama_Cave":
		left = 229;
		top = 502;
		break; //{в пещере слева от выхода из города Порто Белло}
	case "PortoBello_Cave":
		left = 195;
		top = 512;
		break; //{в пещере справа от залива Москитос}

	case "Tortuga_Grot":
		left = 374;
		top = 53;
		break; //{в пещере напротив пляжа Глаз Рыбы}
	case "Tortuga_Cave":
		left = 380;
		top = 61;
		break; //{в подземелье города Тортуга}

	case "Caracas_Grot":
		left = 444;
		top = 484;
		break; //{в пещере слева от выхода из Каракаса}
	case "Maracaibo_Cave":
		left = 369;
		top = 474;
		break; //{в пещере напротив выхода из Маракайбо}
	case "Cartahena_Cave":
		left = 304;
		top = 510;
		break; //{в пещере напротив выхода из Картахены}

	case "Pearl_Grot":
		left = 66;
		top = 345;
		break; //{в пещере на западном побережье между Белизом и Санта Каталиной} 			//тут надо исправить на грот описание, если там грот
	case "SantaCatalina_Grot":
		left = 123;
		top = 445;
		break; //{в пещере справа от залива Сан Хуан дель Норте}
	case "SantaCatalina_PearlCave":
		left = 109;
		top = 466;
		break; //{в пещере слева от залива Сан Хуан дель Норте}
	case "SantaCatalina_Cave":
		left = 106;
		top = 375;
		break; //{в пещере справа от мыса Перлас}
	case "Tenotchitlan_Cave":
		left = 55;
		top = 308;
		break; //{в пещере на север от храма Инков}

	case "Barbados_Cave":
		left = 625;
		top = 385;
		break; //{в пещере недалеко от Бриджтауна}
	case "Bermudes_Dungeon":
		left = 625;
		top = 29;
		break; //{в подземелье под верьфью пиратского поселения}
	case "Caiman_Grot":
		left = 202;
		top = 173;
		break; //{в пещере справа от пустынного пляжа}
	case "Charles_Grot":
		left = 550;
		top = 202;
		break; //{в пещере справа от пляжа Бат}
	case "Cuba_Grot":
		left = 198;
		top = 44;
		break; //{в пещере справа от мыса Камагуэй}
	case "Cumana_Cave":
		left = 530;
		top = 505;
		break; //{в пещере справа от бухты Карупано}
	case "Curacao_Cave":
		left = 465;
		top = 435;
		break; //{в пещере справа от берега Приют контрабандиста}
	case "Dominica_Grot":
		left = 574;
		top = 326;
		break; //{в пещере справа от пляжа Касл Брус}
	case "Guadeloupe_Cave":
		left = 485;
		top = 269;
		break; //{в пещере слева от маяка Гваделупы}
	case "Hispaniola_Cave":
		left = 383;
		top = 119;
		break; //{в пещере недалеко от города Порт-о-Принс}
	case "Jamaica_Grot":
		left = 263;
		top = 287;
		break; //{в пещере справа от маяка Порт-Рояля}
	case "Marigo_Cave":
		left = 619;
		top = 140;
		break; //{в пещере напротив выхода из Мариго}
	case "SanJuan_Cave":
		left = 485;
		top = 153;
		break; //{в пещере недалеко от Сан-Хуана}
	case "Terks_Grot":
		left = 474;
		top = 33;
		break; //{в пещере справа от залива Северный}
	case "Trinidad_Grot":
		left = 591;
		top = 458;
		break; //{в пещере напротив маяка Тринидада и Тобаго}
	case "FortFrance_Dungeon":
		left = 499;
		top = 357;
		break; //{в подземелье города Форт де Франс}
	}
...
void ShowItemFromCharacterWindow()
{
	...
	if (sti(xi_refCharacter.index) != nMainCharacterIndex)
		SetFormatedText("ITEM_FROM_CHARACTER_NAME", "Передать предмет: " + sGoodName);
	else
...
void ClickItem(string sItem, string sEquipType, int _tabNum)
{
	...
	{
		row = "tr" + q;
		if (!checkattribute(&GameInterface, "TABLE_ITEMS." + row))
			break; //кончились строки таблицы, конец цикла
...
void ReEquipCharacter()
{
	...
	if (!checkattribute(xi_refCharacter, sSET))
		return; //не было сохранения этого комплекта
	...
	for (int q = 0; q < 10; q++)
	{
		...
		if (xi_refCharacter.(sSET).(sTemp) != GetCharacterEquipByGroup(xi_refCharacter, sTemp)) //если другой предмет, то снимаем старый, одеваем сохранённый
		{
			...
			if (xi_refCharacter.(sSET).(sTemp) != "")
			{
				EquipCharacterByItem(xi_refCharacter, xi_refCharacter.(sSET).(sTemp));
			}
		}
	}
...
void SaveEquipSet()
{
	...
	if (!checkattribute(xi_refCharacter, sSET + ".nameset"))
		xi_refCharacter.(sSET).nameset = "Комплект " + iEQUIP_SET; //в первый раз записываем "Комплект N"
	string sTemp;
	for (int q = 0; q < 10; q++)
	{
		sTemp = sEType[q];
...
void FillSetName(int iNum)
{
	...
	if (!checkattribute(xi_refCharacter, sSET))
		...
		sName = xi_refCharacter.(sSET).nameset;
...
void SaveSETName()
{
	if (GameInterface.EDIT_BOX1.lastkey == " ")
		return;
	string sSET = "Set" + iEQUIP_SET;
...
void ShowEditBox()
{
	string sSET = "Set" + iEQUIP_SET;
	if (!checkattribute(xi_refCharacter, sSET))
		GameInterface.EDIT_BOX1.str = "Комплект " + iEQUIP_SET;
	else
		GameInterface.EDIT_BOX1.str = xi_refCharacter.(sSET).nameset;
