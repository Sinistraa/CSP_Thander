...
void AddToTable()
{
	...
	for (i = 0; i < ITEMS_QUANTITY; i++)
	{
		...
		if (leftQty > 0 || rightQty > 0)
		{
			...
			if (leftQty == 0 && FIT_FilterState == FIT_OFFICER)
				continue;
			if (rightQty == 0 && FIT_FilterState == FIT_HERO)
				continue;
			//// {*} BUHO END ADDITION
...
void FillCharactersScroll()
{
	...
	for (i = 0; i < NUM_REGULAR_OFFICERS; i++)
	{
		...
		if (_curCharIdx != -1 && _curCharIdx < MAX_CHARACTERS)
		{
			...
			if (m == 0)
				refCharacter = &characters[_curCharIdx];
...
void ProcessFrame()
{
	...
	if (sti(GameInterface.CHARACTERS_SCROLL.current) != nCurScrollNum && GetCurrentNode() == "CHARACTERS_SCROLL")
	{
		...
		//#20190528-01
		if (idxChar < 0 || idxChar >= MAX_CHARACTERS)
			return;
		refCharacter = &characters[idxChar];
		if (refCharacter.id == "0")
			return;
		SetDescription();
		//FillEquipInfo();
		FillCharacterInfo();
