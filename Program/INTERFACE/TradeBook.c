...
void FillPriceListTown(string _tabName)
{
	...
	GameInterface.(_tabName).hr.td1.scale = 0.8 GameInterface.(_tabName).hr.td2.str = "Город";
...
void FillPriceList(string _tabName, string attr1)
{
	...
	if (attr1 != "")
	{
		...
		for (i = 0; i < GOODS_QUANTITY; i++)
		{
			...
			row = "tr" + n;
			sGoods = "Gidx" + i;
			if (sti(nulChr.PriceList.(attr1).(sGoods).TradeType) == TRADE_TYPE_CANNONS && !bBettaTestMode)
				continue; // не пушки
