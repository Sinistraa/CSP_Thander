							...
							if (sColony != "Panama" && sColony != "IslaMona" && sColony != "KhaelRoa" && sColony != "Caiman")
							{
								...
								for (int z = 0; z < MAX_LOCATIONS; z++)
								{
									if (GiveArealByLocation(&locations[z]) == island)
									{
										...
										if (HasSubStr(locations[z].id.label, "upstairs"))
											GameInterface.TABLE_LOCS.(row).td2.str = "Комната наверху таверны";
										n++;
									}
								}
	...
	if (colonyindex != i)
	{
		...
		for (int y = 0; y < MAX_LOCATIONS; y++)
		{
			if (GiveArealByLocation(&locations[y]) == ssColony || CheckForIslandID(&locations[y]) == ssColony)
			{
				...
				if (HasSubStr(locations[y].id.label, "upstairs"))
					GameInterface.TABLE_LOCS.(row1).td2.str = "Комната наверху таверны";
				if (HasSubStr(locations[y].id, "Arena"))
					GameInterface.TABLE_LOCS.(row1).td2.str = "Арена";
				if (HasSubStr(locations[y].id, "FencingTown_F"))
					GameInterface.TABLE_LOCS.(row1).td2.str = "Поселение фехтовальщиков";
				x++;
			}
		}
		Table_UpdateWindow("TABLE_LOCS");
