...
void CycleCirass(bool dir)
{
	...
	if (curmodel != 0)
		ShowCirassOnCharacter(pchar, CIRASS_ITEM_TYPE, itemID);
}
...
void SelectMainCharacter(bool bLeft)
{
	...
	while (1)
	{
		...
		if (FState_SEX[0] && sSex == "man")
		{
			continue;
		}
		if (FState_SEX[1] && sSex == "woman")
		{
			continue;
		}
		if (FState_SEX[2] && sSex == "skeleton")
		{
			continue;
		}
		...
		if (FState_ANIM)
		{
			if (sAnim == "man" || sAnim == "skeleton" || sAnim == "YokoDias" || sAnim == "Milenace")
			{
			}
			else
				continue;
	...
	if (CheckAttribute(pchar, "RemovePGG." + sBlockPGG))
		bDelPGG = sti(pchar.RemovePGG.(sBlockPGG));
...
void ProcessFilter()
{
	...
	if (sControl == "CHECK_HERO_SEX")
	{
		...
		if (!iNewState && FState_SEX[0] + FState_SEX[1] + FState_SEX[2] == 1) //нужно разлочить кнопку
		{
			for (i = 1; i < 4; i++)
			{
				SendMessage(&GameInterface, "lslll", MSG_INTERFACE_MSG_TO_NODE, "CHECK_HERO_SEX", 5, i, 0);
			} //разблочим все кнопки
		}
		if (FState_SEX[0] + FState_SEX[1] + FState_SEX[2] == 2) //блочим оставшийся единственным ненажатым чекбокс. иначе вообще все герои будут недоступны и бесконечный цикл
		{
			for (i = 1; i < 4; i++)
			{
				if (SendMessage(&GameInterface, "lsll", MSG_INTERFACE_MSG_TO_NODE, "CHECK_HERO_SEX", 3, i))
					continue;
				else
				{
					SendMessage(&GameInterface, "lslll", MSG_INTERFACE_MSG_TO_NODE, "CHECK_HERO_SEX", 5, i, 1);
					break;
				} //блочим ненажатую кнопку и выходим из цикла, так как она одна
			}
		}
		if (iNewState) //отключили какой-то пол
		{
			...
			case 0:
				if (sSex == "man")
					SelectMainCharacter(0);
				break; //если был выбран муж. и мы сняли чекбокс муж., то переходим к следующему персу
			case 1:
				if (sSex == "woman")
					SelectMainCharacter(0);
				break;
