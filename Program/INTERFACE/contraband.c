...
void InitInterface_RR(string iniName, ref ContraTrader, ref pStore)
{
	...
	if (checkattribute(pchar, "ContraFilter"))
		FIS_FilterState = sti(pchar.ContraFilter);
	else
		FIS_FilterState = 0;
...
void AddToTable()
{
	...
	for (i = 0; i < GOODS_QUANTITY; i++)
	{
		...
		if (tradeType == TRADE_TYPE_CANNONS)
			GameInterface.TABLE_LIST.(row).td5.str = makeint(GetStoreGoodsPrice(refStore, i, PRICE_TYPE_SELL, pchar, 1) * 1.75);
		else
...
void TransactionOK()
{
	...
	{ // SELL
		...
		if (!CheckAttribute(pchar, "contrabandmoneyback"))
			pchar.contrabandmoneyback = moneyback;
		else
			pchar.contrabandmoneyback = sti(pchar.contrabandmoneyback) + moneyback;
		pchar.money = sti(pchar.money) + moneyback;
		sld.SmugglingMoney = sti(sld.SmugglingMoney) - moneyback;
...
void ShowGoodsInfo(int iGoodIndex)
{
	...
	if (tradeType == TRADE_TYPE_CANNONS)
		iStorePrice = makeint(GetStoreGoodsPrice(refStore, iGoodIndex, PRICE_TYPE_SELL, pchar, 1) * 1.75);
	else
