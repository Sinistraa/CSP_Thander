...
void InitInterface(string iniName)
{
	...
	if (startHeroType < 1)
		startHeroType = 1; // fix
	if (startHeroType > sti(GetNewMainCharacterParam("hero_qty")))
		startHeroType = sti(GetNewMainCharacterParam("hero_qty")); // fix
...
void SetVariable(bool _init)
{
	...
	if (bRandCharNation || startHeroType < 3)
		SetSelectable("RANDCHARNATION", false);
	else
		SetSelectable("RANDCHARNATION", true);
