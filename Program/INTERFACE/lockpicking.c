...
void InitInterface_gm(string iniName)
{
	...
	if (LckPckngProcessBooster == false)
	...
	{
		if (makeint(pchar.questTemp.lockpicking.firsttime) == 1)
			firsttime = true;
		else
			firsttime = false;
	}
	...
	if (correctposition < 15)
		correctposition = 15;
	if (correctposition > 165)
		correctposition = 165;
	...
	sText2 = "Передвигая мышь вверх-вниз, подбирайте положение отмычки. Когда ее угол выбран, нажмите и удерживайте клавишу 'Ctrl' для того, чтобы начать вращать цилиндр. Для открытия замка необходимо, чтобы цилиндр был повернут на 90 градусов." sText3 = "Если цилиндр в процессе вращения неожиданно остановился, это значит, что положение отмычки выбрано неправильно. В этом случае, отпустите клавишу 'Ctrl' и дождитесь, когда цилиндр вернется в исходное положение, чтобы разблокировать отмычку. Помните, чем ближе к правильному положению вы выставите отмычку, тем на больший угол цилиндр будет повернут.";

	if (firsttime == true)
		CreateTooltip("#" + sHeader, sText1, argb(255, 255, 255, 255), sText2, argb(255, 255, 192, 192), sText3, argb(255, 255, 255, 255), "", argb(255, 255, 255, 255), "none", "none", "none", 64, 64);
}
...
void IProcessFrame()
{
		...
		if (mousechangepos == false)
		{
			...
			if (mpos != ompos)
			{
				if (rand(20) == 0)
				{
					...
					if (movsound < 10)
						PlaySound("interface\lockpicking\mov\ui_lockpicking_pickmovement_0" + movsound + ".wav");
					else
						PlaySound("interface\lockpicking\mov\ui_lockpicking_pickmovement_" + movsound + ".wav");
				}
			}
	...
	if (checkingstagenum < 460)
	{
		if ((pickposition - correctposition) <= 0)
			difference = correctposition - pickposition;
		else
			difference = pickposition - correctposition;
		...
		if (checkingstagenum == 460)
			OpenProcedure(); // Если цилиндр повернут на 90 градусов, то запускаем процесс открытия
		...
		if (checkingstagenum != 10 && checkingstagenum != 460 && checkingstagenum != makeint((1.0 - dmultiplier) * 460.0)) // Поскрипывания цилиндра при его движении
		{
			if (rand(160) == 0)
			{
				...
				if (squeaksound < 10)
					PlaySound("interface\lockpicking\cylsqueak\ui_lockpicking_cylindersqueak_0" + squeaksound + ".wav");
				else
					PlaySound("interface\lockpicking\cylsqueak\ui_lockpicking_cylindersqueak_" + squeaksound + ".wav");
			}
		}
...
void OpenChest()
{
	...
	if (sti(pchar.questTemp.chestcount) >= 5)
		UnlockAchievement("AchChest", 1);
	if (sti(pchar.questTemp.chestcount) >= 15)
		UnlockAchievement("AchChest", 2);
	if (sti(pchar.questTemp.chestcount) >= 30)
		UnlockAchievement("AchChest", 3);
	string nameitm[35];
	int itmq[35];
	int value;
	int value_alt;
	...
	case "Chest_treasure":
		...
		if (sti(pchar.Chest_treasure) > 99)
		...
		if (sti(pchar.Chest_treasure2) > 99)
	...
	case "Chest_Craftsmans":
		...
		if (sti(pchar.Chest_Craftsmans) > 99)
	...
	if (chestID == "Chest_treasure") // Малый
	{
		...
		if (itmq[0] > 0)
			itmq[0] += makeint(value / 50);
		if (itmq[1] > 0)
			itmq[1] += makeint(value / 50);
		if (itmq[2] > 0)
			itmq[2] += makeint(value / 50);
		if (itmq[3] > 0)
			itmq[3] += makeint(value / 50);
		if (itmq[4] > 0)
			itmq[4] += makeint(value / 50);
		if (itmq[5] > 0)
			itmq[5] += makeint(value / 50);
	...
	if (chestID == "Chest_Craftsmans") // Ремесленника
	{
		...
		if (itmq[5] > 0)
			itmq[5] += makeint(value / 50);
		if (itmq[6] > 0)
			itmq[6] += makeint(value / 50);
		if (itmq[7] > 0)
			itmq[7] += makeint(value / 50);
		if (itmq[8] > 0)
			itmq[8] += makeint(value / 50);
		if (itmq[9] > 0)
			itmq[9] += makeint(value / 50);
		if (itmq[10] > 0)
			itmq[10] += makeint(value / 50);

		if (value < 333)
		{
			nameitm[11] = "CompCraft_HealthTobacco";
			itmq[11] = 1;
		}
		if (value >= 333 && value < 666)
		{
			nameitm[12] = "CompCraft_EnergyTobacco";
			itmq[12] = 1;
		}
		if (value >= 666)
		{
			nameitm[13] = "CompCraft_StrengthTobacco";
			itmq[13] = 1;
		}
		if (value >= 700)
		{
			nameitm[14] = "CompCraft_Scissors";
			itmq[14] = 1;
		}
	}
	...
	if (chestID == "Chest_quest") // Странный
	{
		...
		if (value >= 1 && value < 80)
		{
			nameitm[15] = "BackPack2";
			itmq[15] = 1;
		} // Обычный рюкзак
		if (value_alt >= 80 && value_alt < 160)
		{
			nameitm[15] = "BackPack3";
			itmq[15] = 1;
		} // Хороший рюкзак
		if (value2 >= 160 && value2 < 240)
		{
			nameitm[15] = "BackPack4";
			itmq[15] = 1;
		} // Отличный рюкзак

		if (value3 >= 240 && value3 < 320)
		{
			nameitm[15] = "cirass2";
			itmq[15] = 1;
		} // Рейтарский панцирь
		...
		if (value_alt >= 400 && value_alt < 480)
		{
			nameitm[15] = "cirass4";
			itmq[15] = 1;
		} // Офицерская кираса

		if (value2 >= 480 && value2 < 560)
		{
			nameitm[15] = "Totem_1";
			itmq[15] = 1;
		} // Тотем Шочикецаль
		if (value3 >= 560 && value3 < 640)
		{
			nameitm[15] = "Totem_2";
			itmq[15] = 1;
		} // Тотем Миктлантекутли
		if (value >= 640 && value < 720)
		{
			nameitm[15] = "Totem_3";
			itmq[15] = 1;
		} // Тотем Кецалькоатля
		if (value_alt >= 720 && value_alt < 800)
		{
			nameitm[15] = "Totem_4";
			itmq[15] = 1;
		} // Тотем Мишкоатля
		if (value2 >= 800 && value2 < 880)
		{
			nameitm[15] = "Totem_7";
			itmq[15] = 1;
		} // Тотем Уицилопочтли
		if (value3 >= 880 && value3 < 999)
		{
			nameitm[15] = "Totem_8";
			itmq[15] = 1;
		} // Тотем Тлалока
		...
		if (value_alt >= 40 && value_alt < 80)
		{
			nameitm[16] = "blade22";
			itmq[16] = 1;
		} // Испанская рапира ЛО
		if (value2 >= 80 && value2 < 120)
		{
			nameitm[16] = "blade36";
			itmq[16] = 1;
		} // Саксенфедер ЛО
		if (value3 >= 120 && value3 < 160)
		{
			nameitm[16] = "blade37";
			itmq[16] = 1;
		} // Персидская сабля ЛО
		...
		if (value_alt >= 200 && value_alt < 240)
		{
			nameitm[16] = "blade34";
			itmq[16] = 1;
		} // Скаллоп СО
		if (value2 >= 240 && value2 < 280)
		{
			nameitm[16] = "blade39";
			itmq[16] = 1;
		} // Скимитар СО
		if (value3 >= 280 && value3 < 320)
		{
			nameitm[16] = "blade40";
			itmq[16] = 1;
		} // Мадонна СО
		if (value >= 320 && value < 360)
		{
			nameitm[16] = "blade46";
			itmq[16] = 1;
		} // Офицерский клеванг СО

		if (value_alt >= 360 && value_alt < 400)
		{
			nameitm[16] = "blade15";
			itmq[16] = 1;
		} // Сторта ТО
		if (value2 >= 400 && value2 < 440)
		{
			nameitm[16] = "blade21";
			itmq[16] = 1;
		} // Бастард ТО
		if (value3 >= 440 && value3 < 480)
		{
			nameitm[16] = "blade202";
			itmq[16] = 1;
		} // Цвайхандер ТО
		...
		if (value_alt >= 520 && value_alt < 560)
		{
			nameitm[16] = "topor2";
			itmq[16] = 1;
		} // Рейтарский чекан ТО
		...
		if (value3 >= 600 && value3 < 640)
		{
			nameitm[16] = "blade32";
			itmq[16] = 1;
		} // Фламберж ЛО
		if (value >= 640 && value < 680)
		{
			nameitm[16] = "blade26";
			itmq[16] = 1;
		} // Паппенхаймер ЛО

		if (value_alt >= 680 && value_alt < 720)
		{
			nameitm[16] = "blade30";
			itmq[16] = 1;
		} // Нобиль СО
		if (value2 >= 720 && value2 < 760)
		{
			nameitm[16] = "blade24";
			itmq[16] = 1;
		} // Чиавона СО
		if (value3 >= 760 && value3 < 800)
		{
			nameitm[16] = "blade25";
			itmq[16] = 1;
		} // Карабела СО
		...
		if (value_alt >= 840 && value_alt < 880)
		{
			nameitm[16] = "blade20";
			itmq[16] = 1;
		} // Клейбэг ТО
		if (value2 >= 880 && value2 < 920)
		{
			nameitm[16] = "blade33";
			itmq[16] = 1;
		} // Валлонская шпага ТО
		if (value3 >= 920 && value3 < 999)
		{
			nameitm[16] = "blade28";
			itmq[16] = 1;
		} // Танат ТО
		...
		{
			nameitm[17] = "pistol_grapebok";
			itmq[17] = 1;
		} // Трёхствольный дробовик
		if (value_alt >= 200 && value_alt < 350)
		{
			nameitm[17] = "pistol6";
			itmq[17] = 1;
		} // Бок-пистолет
		if (value2 >= 400 && value2 < 550)
		{
			nameitm[17] = "pistol9";
			itmq[17] = 1;
		} // Двузарядный пистолет
		if (value3 >= 600 && value3 < 750)
		{
			nameitm[17] = "pistol4";
			itmq[17] = 1;
		} // Четырёхствольный пистолет
		if (value >= 800 && value < 900)
		{
			nameitm[17] = "pistol5";
			itmq[17] = 1;
		} // Бретерский пистолет
		if (value_alt >= 900 && value_alt < 950)
		{
			nameitm[17] = "mushket";
			itmq[17] = 1;
		} // Фитильный мушкет
		if (value >= 950 && value < 999)
		{
			nameitm[17] = "mushket_drob";
			itmq[17] = 1;
		} // Аркебуза

		if (value >= 10 && value < 30)
		{
			nameitm[18] = "mushket_spanish";
			itmq[18] = 1;
		} // Испанский мушкетон
		if (value_alt >= 150 && value_alt < 170)
		{
			nameitm[18] = "mushket_english";
			itmq[18] = 1;
		} // Английский мушкетон
		if (value >= 600 && value < 620)
		{
			nameitm[18] = "mushket_france";
			itmq[18] = 1;
		} // Французcкий мушкетон
		if (value_alt >= 900 && value_alt < 920)
		{
			nameitm[18] = "mushket_holland";
			itmq[18] = 1;
		} // Голландский мушкетон
	}
	...
	for (i = 0; i < 35; i++)
	{
		...
		{
			if (nameitm[i] == "gold")
				AddMoneyToCharacter(pchar, itmq[i]);
			else
			{
				if (HasSubstr(nameitm[i], "blade"))
					TakeNItems(pchar, GetGeneratedItem(nameitm[i]), itmq[i]);
				else
					TakeNItems(pchar, nameitm[i], itmq[i]);
			}

			if (i == 0)
				nameitm[i] = "- " + GetConvertStr("itmname_" + nameitm[i], "ItemsDescribe.txt") + " в количестве " + itmq[i];
			else
				nameitm[i] = NewStr() + "- " + GetConvertStr("itmname_" + nameitm[i], "ItemsDescribe.txt") + " в количестве " + itmq[i];
		}
	}
...
bool CheckSkull(string SkullId)
{
	...
	{
		if (!CheckAttribute(pchar, "OwnsSkull1"))
			pchar.OwnsSkull1 = true;
		return true;
	}
	if (CheckCharacterItem(pchar, "sculMa2") || CheckAttribute(pchar, "OwnsSkull2"))
	{
		if (!CheckAttribute(pchar, "OwnsSkull2"))
			pchar.OwnsSkull2 = true;
		return true;
	}
	if (CheckCharacterItem(pchar, "sculMa3") || CheckAttribute(pchar, "OwnsSkull3"))
	{
		if (!CheckAttribute(pchar, "OwnsSkull3"))
			pchar.OwnsSkull3 = true;
		return true;
	}
	return false;
}
