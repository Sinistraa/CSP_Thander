...
void procSaveClick()
{
	...
	if (sNodName == "SAVEIMG11")
	{
		SelectSaveImage(g_nFirstSaveIndex + 10);
		return;
	}
	if (sNodName == "SAVEIMG12")
	{
		SelectSaveImage(g_nFirstSaveIndex + 11);
		return;
	}
	if (sNodName == "SAVEIMG13")
	{
		SelectSaveImage(g_nFirstSaveIndex + 12);
		return;
	}
	if (sNodName == "SAVEIMG14")
	{
		SelectSaveImage(g_nFirstSaveIndex + 13);
		return;
	}
	if (sNodName == "SAVEIMG15")
	{
		SelectSaveImage(g_nFirstSaveIndex + 14);
		return;
	}
	if (sNodName == "SAVEIMG16")
	{
		SelectSaveImage(g_nFirstSaveIndex + 15);
		return;
	}
	if (sNodName == "SAVEIMG17")
	{
		SelectSaveImage(g_nFirstSaveIndex + 16);
		return;
	}
	if (sNodName == "SAVEIMG18")
	{
		SelectSaveImage(g_nFirstSaveIndex + 17);
		return;
	}
	if (sNodName == "SAVEIMG19")
	{
		SelectSaveImage(g_nFirstSaveIndex + 18);
		return;
	}
	if (sNodName == "SAVEIMG20")
	{
		SelectSaveImage(g_nFirstSaveIndex + 19);
		return;
	}

	if (sNodName == "SAVEIMG21")
	{
		SelectSaveImage(g_nFirstSaveIndex + 20);
		return;
	}
	if (sNodName == "SAVEIMG22")
	{
		SelectSaveImage(g_nFirstSaveIndex + 21);
		return;
	}
	if (sNodName == "SAVEIMG23")
	{
		SelectSaveImage(g_nFirstSaveIndex + 22);
		return;
	}
	if (sNodName == "SAVEIMG24")
	{
		SelectSaveImage(g_nFirstSaveIndex + 23);
		return;
	}
	if (sNodName == "SAVEIMG25")
	{
		SelectSaveImage(g_nFirstSaveIndex + 24);
		return;
	}
	if (sNodName == "SAVEIMG26")
	{
		SelectSaveImage(g_nFirstSaveIndex + 25);
		return;
	}
	if (sNodName == "SAVEIMG27")
	{
		SelectSaveImage(g_nFirstSaveIndex + 26);
		return;
	}
	if (sNodName == "SAVEIMG28")
	{
		SelectSaveImage(g_nFirstSaveIndex + 27);
		return;
	}
	if (sNodName == "SAVEIMG29")
	{
		SelectSaveImage(g_nFirstSaveIndex + 28);
		return;
	}
	if (sNodName == "SAVEIMG30")
	{
		SelectSaveImage(g_nFirstSaveIndex + 29);
		return;
	}

	if (sNodName == "SAVEIMG31")
	{
		SelectSaveImage(g_nFirstSaveIndex + 30);
		return;
	}
	if (sNodName == "SAVEIMG32")
	{
		SelectSaveImage(g_nFirstSaveIndex + 31);
		return;
	}
	if (sNodName == "SAVEIMG33")
	{
		SelectSaveImage(g_nFirstSaveIndex + 32);
		return;
	}
	if (sNodName == "SAVEIMG34")
	{
		SelectSaveImage(g_nFirstSaveIndex + 33);
		return;
	}
	if (sNodName == "SAVEIMG35")
	{
		SelectSaveImage(g_nFirstSaveIndex + 34);
		return;
	}
	if (sNodName == "SAVEIMG36")
	{
		SelectSaveImage(g_nFirstSaveIndex + 35);
		return;
	}
	if (sNodName == "SAVEIMG37")
	{
		SelectSaveImage(g_nFirstSaveIndex + 36);
		return;
	}
	if (sNodName == "SAVEIMG38")
	{
		SelectSaveImage(g_nFirstSaveIndex + 37);
		return;
	}
	if (sNodName == "SAVEIMG39")
	{
		SelectSaveImage(g_nFirstSaveIndex + 38);
		return;
	}
	if (sNodName == "SAVEIMG40")
	{
		SelectSaveImage(g_nFirstSaveIndex + 39);
		return;
	}
}
...
bool ParseSaveData(string fullSaveData, ref facestr, ref locationStr, ref timeStr, ref languageID, ref playtime, ref curship, ref nation, ref difficulty, ref money, ref rank)
{
	...
	if (!GetNextSubStr(lastStr, playtime, &lastStr, "@"))
		return false;
	if (!GetNextSubStr(lastStr, curship, &lastStr, "@"))
		return false;
	if (!GetNextSubStr(lastStr, nation, &lastStr, "@"))
		return false;
	if (!GetNextSubStr(lastStr, difficulty, &lastStr, "@"))
		return false;
	if (!GetNextSubStr(lastStr, money, &lastStr, "@"))
		return false;
	if (!GetNextSubStr(lastStr, rank, &lastStr, "@"))
		return false;
...
void ProcessCustomSaveAction()
{
	...
	if (iComIndex == ACTION_ACTIVATE || iComIndex == ACTION_MOUSECLICK)
	{
		if (sNodName == "CUSTOM_SAVE_BTN_OK")
		{
			...
			PostEvent("evntSave",0,"ss", "SAVE\"+PlayerProfile.name+"\"+saveName, sSaveDescriber);
            // Warship Fix свечени¤
			ExitSaveCustomGame();
			IDoExit(RC_INTERFACE_SAVELOAD_EXIT);
		}
...
void ReloadSaveInfo()
{
	...
	ShowFaceInfo(info);
	if (nSlot >= 0 && nSlot < MAX_SAVE_SLOTS && CheckAttribute(&g_oSaveList[nSlot], "curship"))
		SSI(g_oSaveList[nSlot].curship);
	if (nSlot >= 0 && nSlot < MAX_SAVE_SLOTS && CheckAttribute(&g_oSaveList[nSlot], "nation"))
	{
		SetNewGroupPicture("FLAG_ICON", "NATIONS", g_oSaveList[nSlot].nation);
		SetFormatedText("FLAG_TEXT", "Нация: " + GetFlagName(g_oSaveList[nSlot].nation));
	}
	if (nSlot >= 0 && nSlot < MAX_SAVE_SLOTS && CheckAttribute(&g_oSaveList[nSlot], "difficulty"))
		SetFormatedText("DIFFICULTY", "Сложность: " + GetLevelComplexity(sti(g_oSaveList[nSlot].difficulty)));
	if (nSlot >= 0 && nSlot < MAX_SAVE_SLOTS && CheckAttribute(&g_oSaveList[nSlot], "money"))
		SetFormatedText("MONEY", "Денег: " + FindRussianMoneyString(sti(g_oSaveList[nSlot].money)));
	if (nSlot >= 0 && nSlot < MAX_SAVE_SLOTS && CheckAttribute(&g_oSaveList[nSlot], "rank"))
		SetFormatedText("RANK", "Ранг: " + g_oSaveList[nSlot].rank);
