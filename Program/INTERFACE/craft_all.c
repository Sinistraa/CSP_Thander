...
void InitInterface_gm(string iniName)
{
	...
	{
		if (CheckAttribute(&Items[i], "PerkReq") && !HasPerk)
			continue;
...
void FillItemsTab() // Заполнить таблицу выученных чертежей
{
	...
	for (int i = 0; i < num; i++)
	{
		...
		if (CheckAttribute(Items[GetItemIndex(drawning.ID)], "ResultNum"))
			GameInterface.ITEMS_LIST.(row).td3.str = Items[GetItemIndex(drawning.ID)].ResultNum;
		else
			GameInterface.ITEMS_LIST.(row).td3.str = 1;
...
{
	//DumpAttributes(draw);
	if (CheckAttribute(draw, "ResultNum"))
		TakeNItems(pchar, draw.ID, qnt * sti(draw.ResultNum));
	else
	...
	if (CheckAttribute(draw, "ResultNum"))
		Log_Info("Создан предмет '" + LanguageConvertString(lngFileID, Items[drIDX].name) + "' в кол-ве " + qnt * sti(draw.ResultNum) + " ед.");
	else
	...
	if (sti(pchar.questTemp.craftcount) >= 300)
		UnlockAchievement("craft", 1);
	if (sti(pchar.questTemp.craftcount) >= 500)
		UnlockAchievement("craft", 2);
	if (sti(pchar.questTemp.craftcount) >= 1000)
		UnlockAchievement("craft", 3);
...
int GetCharacterItemWithCabin(ref _refCharacter, string itemName)
{
	...
	aref chests;
	if (!CheckAttribute(locations[FindLocation(Get_My_Cabin())], "locators.box"))
		return qty;
...
void SetCraftInfo(int idx)
{
	...
	for (int i = 1; i <= sti(Items[craftable].ComponentsNum); i++)
	{
		...
		if (Items[craftable].(CIN) == "0" && GetCharacterItemWithCabin(pchar, Items[craftable].(CI)) >= 1)
			GameInterface.COMPONENTS_LIST.(row).td2.str = "В наличии";
