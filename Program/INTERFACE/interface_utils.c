...
{
	if (!checkattribute(_character, "selectedSet"))
		return 0; //у этого персонажа не было сохранено комплектов
	int i, q;
	string sSET, sTemp;
	for (i = 1; i < 10; i++)
	{
		if (_character.selectedSet == i)
			continue; //текущий комплект уже скрыт в списке перемещения предметов
		sSET = "Set" + i;
		if (!checkattribute(_character, sSET))
			continue; //нет комплекта
		for (q = 0; q < 10; q++)
		{
			sTemp = sEType[q];
			if (_character.(sSET).(sTemp) == sItemName && GetCharacterEquipByGroup(_character, sTemp) != sItemName)
				return i;
...
void SetShipQualityTable(ref _chr, string _bar1, string _bar7, string _bar2, string _bar3, string _bar4, string _bar5, string _bar6)
{
	...
	if (sti(refShip.BaseType) <= SHIP_SP_SANFELIPE) //рассчитываем качество только для НЕквестовых кораблей
	{
		...
		int MaxiCannonDiff;
		if (rBaseShip.CannonsQuantity > 7)
			MaxiCannonDiff = 1;
		if (rBaseShip.CannonsQuantity > 13)
			MaxiCannonDiff = 2;
		if (rBaseShip.CannonsQuantity > 29)
			MaxiCannonDiff = 3;
		if (rBaseShip.CannonsQuantity > 49)
			MaxiCannonDiff = 4;
		if (rBaseShip.CannonsQuantity > 79)
			MaxiCannonDiff = 5;
		...
		if (CheckAttribute(refShip, "Bonus_Capacity"))
			Bonus_Capacity = refShip.Bonus_Capacity;
		else
			Bonus_Capacity = 0;
		if (CheckAttribute(refShip, "Bonus_HP"))
			Bonus_HP = refShip.Bonus_HP;
		else
			Bonus_HP = 0;
		if (CheckAttribute(refShip, "Bonus_SpeedRate"))
			Bonus_SpeedRate = refShip.Bonus_SpeedRate;
		else
			Bonus_SpeedRate = 0;
		if (CheckAttribute(refShip, "Bonus_TurnRate"))
			Bonus_TurnRate = refShip.Bonus_TurnRate;
		else
			Bonus_TurnRate = 0;
		...
		if (CheckAttribute(refShip, "Tuning.BotPack"))
			fTemp = fTemp / 1.5;
		if (CheckAttribute(refShip, "Tuning.Capacity"))
			fTemp = fTemp / 1.2;
		...
		if (CheckAttribute(refShip, "Tuning.TurnRate"))
			fTemp = fTemp / 1.2;
		...
		if (CheckAttribute(refShip, "Tuning.MastMultiplier"))
			fTemp = fTemp + 0.3;
		fMast = 100 * (stf(rBaseShip.MastMultiplier) + 0.3 - fTemp) / 0.6;

		fTemp = stf(refShip.WindAgainstSpeed);
		if (CheckAttribute(refShip, "Tuning.WindAgainst"))
			fTemp = stf(FloatToString((fTemp - 0.005) * stf(refShip.Class) * 2 / (stf(refShip.Class) * 2 + 1), 2)); // :(
	...
	if (tradeType == PRICE_TYPE_BUY)
	{
		...
		if (CheckAttribute(&Items[itmIdx], "groupID"))
		{
			if (Items[itmIdx].groupID == BLADE_ITEM_TYPE || Items[itmIdx].groupID == GUN_ITEM_TYPE)
				skillModify *= 10.0; //меняем после всех перков, иначе скидка от перков непропорциональная
		}
	}
	else
	{
...
void SortTable(string sControl, int iColumn, bool bIsString, bool bAsc, int _iLinesCount)
{
	...
	if (_iLinesCount < 1)
	{
		...
		bNoSize = false; //если число строк не передано, ставим 998 строк для цикла
	...
	for (m = 0; m < _iLinesCount - 2; m++)
	{
		for (n = 0; n < _iLinesCount - 2; n++)
		{
			...
			if (bNoSize)
			{
				if (!checkattribute(&GameInterface, sControl + "." + sNextRow))
				{
					_iLinesCount = n + 2;
					bNoSize = false;
					break;
				}
			} //если число строк не было указано, проверяем, не кончилась ли таблица
