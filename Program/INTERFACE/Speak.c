...
void ProcessCancelExit()
{
	if (iFlag == CAPTURE)
	{
		Ship_Surrender(sti(rSpeakCharacter.index));
	}
...
void IDoExit(int exitCode, bool bCode)
{
	...
	if (interfaceResultCommand != RC_INTERFACE_SPEAK_EXIT_AND_CAPTURE)
	{
		if (iFlag == NEUTRAL)
		{
			...
			if (bBettaTestMode)
				trace("etape neutre speak");
		}
		if (iFlag == TAX)
		{
			AnalizeSituationsTax(rSpeakCharacter);
			if (bBettaTestMode)
				trace("etape taxe speak");
		}
		if (iFlag == ENEMY)
		{
			AnalizeSituationsEnemy();
			if (bBettaTestMode)
				trace("etape ennemi speak");
		}
	}
...
{
	if (bBettaTestMode)
		trace("etape demande_ranc speak");
	string sText = "";
	string sAnswerText = "";
...
{
	if (bBettaTestMode)
		trace("etape demande_taxe speak");
	...
	else
	{
		...
		case 0:
			if (iStringsQuantity == 1)
			{
				DisAgreeRansom();
				if (bBettaTestMode)
					trace("etape 10 speak");
			}
			else
			{
				OkTax();
				if (bBettaTestMode)
					trace("etape 11 speak");
			}
			break;
		case 1:
			DisAgreeRansom();
			if (bBettaTestMode)
				trace("etape 12 speak");
			break;
		}
	}
}
