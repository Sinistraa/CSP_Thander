...
void InitInterface_R(string iniName, ref pTrader)
{
	...
	if (checkattribute(pchar, "ItemTradeFilter"))
		FIT_FilterState = sti(pchar.ItemTradeFilter);
	else
		FIT_FilterState = 0;
	...
	{
	case "man":
		SetNewPicture("EQUIP_BIG_PICTURE", "interfaces\sith\CharEquip_Man.tga");
		break;
	case "woman":
		SetNewPicture("EQUIP_BIG_PICTURE", "interfaces\sith\CharEquip_Woman.tga");
		break;
	case "skeleton":
		SetNewPicture("EQUIP_BIG_PICTURE", "interfaces\sith\CharEquip_Skeleton.tga");
		break;
	}
	...
	if (!IsPCharHaveTreasurer())
	{
		...
		SetNodeUsing("Autotrade_This", false);
	} //скрываем автозакупку без казначея
}
...
void AddToTable()
{
	...
	for (i = ITEMS_QUANTITY; i < TOTAL_ITEMS; i++)
	{
		...
		if (leftQty > 0 || rightQty > 0)
		{
			...
			if (leftQty == 0 && FIT_FilterState == FIT_BACKPACK)
				continue;
			if (rightQty == 0 && FIT_FilterState == FIT_TRADER)
				continue;
			//// {*} BUHO END ADDITION
...
void DoChange1()
{
	...
	if (row != "tr0")
		ShowGoodsInfo(sti(GameInterface.TABLE_LIST.(row).index));
	nocheck = true;
}
...
void Autotrade_All()
{
	if (!IsPCharHaveTreasurer())
	{
		Log_Info("Нет казначея. Закупки некому проводить!");
		return;
	}
...
void BuyConsume(ref chr)
{
	...
	for (i = 0, n = 1; i < ITEMS_QUANTITY; i++)
	{
		...
		{
			if (!HasSubStr(Items[i].id, "CompCraft"))
				continue; //в списке только расходники
		}
		else
		{
			if (Items[i].SortIndex == 1 || Items[i].SortIndex == 2 || Items[i].ItemType == "CRAFTCOMPONENTS")
				...
				continue; //в списке расходники и крафт
		}
		itemname = Items[i].id;
		if (itemname == "Lockpick")
			continue; //отмычки пропускаем
		if (itemname == "CompCraft_Tools" || itemname == "CompCraft_Locksmith" || itemname == "CompCraft_Puleleyka")
			continue; //исключение выбранных Шахом штук
		...
		//row = "tr" + n;
		if (checkattribute(chr, "TransferItems." + itemname))
			qty = sti(chr.TransferItems.(itemname));
		else
			qty = 0;
		...
		if (qty > curqty)
		{
			...
			continue; //этого припаса уже было достаточно
		...
		{
			qty = curTraderQty;
			bNotEnoughTrader = true;
		} //TO DO - пока что без возможности покупать из воздуха
		if (qty == 0)
			continue; //нечего купить у торговца
		...
		if (qty > makeint(freeWeight / fItemWeight))
			qty = makeint(freeWeight / fItemWeight);
		if (qty == 0)
			continue;		   //не хватает места даже для одного такого предмета
		...
		if (qty == 0)
			continue; //не хватает денег даже на один такой предмет

		iMoneyQty -= iCost * qty;
		TakeNItems(chr, itemname, qty);
		TakeNItems(refStoreChar, itemname, -qty);
		if (qty > 0 && GetFullName(chr) == GetFullName(PChar))
			Log_Info(GetFullName(chr) + " куплено: " + "'" + LanguageConvertString(idLngFile, Items[i].name) + "' " + qty + " шт. " + iCost * qty + " зол.");
	...
	if (bAdded)
	{
		Log_Info(GetFullName(chr) + " пополнил" + sTemp + " комплект расходников.");
		return;
	}
	if (bNotOverWeight)
	{
		Log_Info(GetFullName(chr) + " не получил" + sTemp + " новых расходников. Кончились деньги.");
		return;
	}
	if (bNotEnough)
	{
		Log_Info(GetFullName(chr) + " не получил" + sTemp + " новых расходников из-за перегруза.");
		return;
	}
	if (bNotEnoughTrader)
	{
		Log_Info(GetFullName(chr) + " не получил" + sTemp + " новых расходников. Не было в наличии у торговца.");
		return;
	}
...
{
	if (!checkattribute(chr, "TransferItems.SellRestriction"))
		return; //не включено разрешение продавать
	int qty = 0;
	int i, n, iCost;
	int curqty = 0;
	int iMoneyQty = 0;
...
void Fastsale_trash(ref character)
{
	...
	{
		if (z == 0)
			iOfficer = GetMainCharacterIndex();
		else
			iOfficer = GetOfficersIndex(pchar, z);
		if (iOfficer != -1)
		{
			if (CheckAttribute(&characters[iOfficer], "fighter") || z == 0)
			{
				for (i = ITEMS_QUANTITY; i < TOTAL_ITEMS; i++)
				{
					...
					if (sellQty > 0)
					{
						if (Items[i].id == "slave_01" || Items[i].id == "pistol3")
							continue;
						if (
							Items[i].quality == "poor" || Items[i].quality == "ordinary"
						...
						)
						{
							...
							salemoney += sellcost if (CheckAttribute(Items[i], "groupID") && Items[i].groupID == GUN_ITEM_TYPE) sellpiece4 += sellqty;
							else sellpiece1 += sellqty;
				...
				for (i = 0; i < ITEMS_QUANTITY; i++)
				{
					...
					if (sellQty > 0)
					{
						...
						if (
							(ok1) || (ok2) || (sName == "Лютня") || (sName == "Свечи") || (sName == "Баклан") || (sName == "Старое ведро") || (sName == "Коралл") || (sName == "Трубка") || (sName == "Башмак") || (sName == "Кружка") || (sName == "Мешочек соли") || (sName == "Дешевая подзорная труба") || (sName == "Обычная подзорная труба") || (sName == "Серебряное кольцо с сапфиром") || (sName == "Золотое кольцо с изумрудом") || (sName == "Золотое кольцо с сапфиром") || (sName == "Большая жемчужина") || (sName == "Маленькая жемчужина") || (sName == "Камея") || (sName == "Ожерелье") || (sName == "Золотое кольцо с рубином") || (sName == "Пугающая фигурка") || (sName == "Серебряный кубок") || (sName == "Алебастровый сосуд") || (sName == "Древняя курительница") || (sName == "Раскрашенный сосуд"))
						{
							sellcost = GetTradeItemPrice(Items_FindItemIdx(sGood), PRICE_TYPE_SELL) * sellqty
																										  TakeNItems(&characters[iOfficer], sGood, -sellqty);
							TakeNItems(refStoreChar, sGood, sellqty);
							salemoney += sellcost if (CheckAttribute(Items[i], "groupID") && Items[i].groupID == GUN_ITEM_TYPE) sellpiece4 += sellqty;
							else sellpiece2 += sellqty;
							//				Log_Info(sName + ": " + sellQty + " шт. " + sellcost + " зол.")
						}
					}
				}
				...
				for (i = 0; i < ITEMS_QUANTITY; i++)
				{
					...
					{
						if (
							(sName == "Тельная ладанка") || (sName == "Нефритовая маска") || (sName == "Двойная маска") || (sName == "Идол Великой Матери") || (sName == "Чаша Ололиуки") || (sName == "Кубок-тотем Тепейоллотля") || (sName == "Оберег Эхекатля") || (sName == "Оберег Тлальчитонатиу"))
						{
	...
	AddCharacterExpToSkill(rTreasurer, "Commerce", MakeInt(abs(salemoney) / 800) + rand(1) + 2)
		salemoney = 0 sellpiece1 = 0 sellpiece2 = 0 sellpiece3 = 0 sellpiece4 = 0
}
