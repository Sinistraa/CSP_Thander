...
{
	if (checkattribute(pchar, "iCurTab"))
		iCurTab = sti(pchar.iCurTab);
	else
	{
		iCurTab = 3;
		pchar.iCurTab = 3;
...
void SetVariable()
{
	...
	//navy <--
	string texturedata;
	if (IsCharacterPerkOn(xi_refCharacter, "Grunt"))
		texturedata = "INTERFACES\Sith\Char_Master.tga";
	if (IsCharacterPerkOn(xi_refCharacter, "Trader"))
	...
	if (IsCharacterPerkOn(xi_refCharacter, "Adventurer"))
		texturedata = "INTERFACES\Sith\Char_Adventurer.tga";
	if (IsCharacterPerkOn(xi_refCharacter, "Buccaneer"))
		texturedata = "INTERFACES\Sith\Char_Inquisitor.tga";
	if (IsCharacterPerkOn(xi_refCharacter, "Agent"))
		texturedata = "INTERFACES\Sith\Char_SecretAgent.tga";
	if (IsCharacterPerkOn(xi_refCharacter, "SeaWolf"))
...
void ShowInfoWindow()
{
	...
	{
		text5 = "Рубака";
		text6 = GetConvertStr("Master", "HeroDescribe.txt");
	}
	if (IsCharacterPerkOn(xi_refCharacter, "Trader"))
	{
		text5 = "Торговец";
		text6 = GetConvertStr("Merchant", "HeroDescribe.txt");
	}
	if (IsCharacterPerkOn(xi_refCharacter, "Fencer"))
	{
		text5 = "Фехтовальщик";
		text6 = GetConvertStr("Corsair", "HeroDescribe.txt");
	}
	if (IsCharacterPerkOn(xi_refCharacter, "Adventurer"))
	{
		text5 = "Авантюрист";
		text6 = GetConvertStr("Adventurer", "HeroDescribe.txt");
	}
	if (IsCharacterPerkOn(xi_refCharacter, "Buccaneer"))
	{
		text5 = "Буканьер";
		text6 = GetConvertStr("Inquisitor", "HeroDescribe.txt");
	}
	if (IsCharacterPerkOn(xi_refCharacter, "Agent"))
	{
		text5 = "Тайный Агент";
		text6 = GetConvertStr("SecretAgent", "HeroDescribe.txt");
	}
	if (IsCharacterPerkOn(xi_refCharacter, "SeaWolf"))
	{
		text5 = "Морской Волк";
		text6 = GetConvertStr("SeaWolf", "HeroDescribe.txt");
	}
	...
	case "CHARACTERS_SCROLL":
		...
		if (nChooseNum > 6 && nChooseNum < 7 + MAX_NUM_FIGHTERS)
			nChooseNum = 7; //всех абордажников в цифру 7
		switch (nChooseNum)
		{
		case 0:
	...
	case "TABLE_SHIP_OTHERS":
		if (!checkAttribute(GameInterface, CurTable + "." + CurRow + ".UserData.ID"))
			return; //нет описания у подпунктов бонуса скорости
		sHeader = XI_ConvertString(GameInterface.(CurTable).(CurRow).UserData.ID);
		sText1 = GetRPGText(GameInterface.(CurTable).(CurRow).UserData.ID);
		break;
...
void FillSkillTables()
{
	...
	for (i = 1; i <= 7; i++)
	{
		...
		if (GameInterface.TABLE_SPECIAL.(row).td4.str == "10")
			GameInterface.TABLE_SPECIAL.(row).td4.color = COLOR_MONEY;
		else
			GameInterface.TABLE_SPECIAL.(row).td4.color = COLOR_NORMAL;
	}
	...
	if (CheckAttribute(xi_refCharacter, "chr_ai.bonushptube"))
	...
	if (CheckAttribute(xi_refCharacter, "chr_ai.bonusweighttube"))
		GameInterface.TABLE_OTHER.tr7.td3.color = argb(255, 196, 255, 196);
...
string ShowStatValue(string type)
{
	...
	case "swiftstrike":
		...
		break;
	case "poison":
		if (CheckAttribute(xi_refCharacter, "chr_ai.poison"))
			return its(sti(FloatToString(xi_refCharacter.chr_ai.poison, 0)));
		else
			return "0";
		break;
	case "stun":
	...
	case "recharge":
		...
		if (weaponID != "")
		{
			...
			if (rechargetime != "0")
				retime = FloatToString(sti(weapon.chargespeed) * (1.0 - stf(rechargetime) / 100), 1);
			else
				retime = its(weapon.chargespeed);
		}
		if (rechargetime != "0")
...
string CheckForSpecial(string type)
{
	...
	if (weaponID != "")
	{
		...
		if (CheckAttribute(weapon, "special"))
		{
			...
			{
			case "blooding":
				if (CheckAttribute(weapon, "special.valueB"))
					blod += sti(weapon.special.valueB);
				if (LAi_GetBladeFencingType(xi_refCharacter) == "FencingLight")
				{
					...
					return FloatToString(makefloat(blod), 1) + "%/" + (FloatToString(10.0 + 15.0, 1)) + " сек.";
				break;
			case "crit":
				if (IsCharacterPerkOn(xi_refCharacter, "Fencer"))
				{
				...
				if (CheckAttribute(weapon, "special.valueCrB"))
					critvalue += sti(weapon.special.valueCrB);
				if (addluck)
					critvalue += GetCharacterSPECIALSimple(xi_refCharacter, SPECIAL_L);
				critdamage = 100 + (GetCharacterSPECIALSimple(xi_refCharacter, SPECIAL_L) * 5);
				return its(critvalue) + "%/" + its(critdamage) + "%" break;
			case "swiftstrike":
				...
				else
				{
					if (CheckAttribute(weapon, "special.valueSS"))
					{
						...
						return FloatToString(0.0, 1) + "%/" + (FloatToString(0.0, 1)) + " сек.";
				}
			...
			case "breakchance":
				float BB = 0.0;
				float CB = 0.0;
				if (CheckAttribute(weapon, "special.valueBB"))
					BB = sti(weapon.special.valueBB);
				if (CheckAttribute(weapon, "special.valueCB"))
					CB = sti(weapon.special.valueCB);
				if (LAi_GetBladeFencingType(xi_refCharacter) == "FencingHeavy")
				{
					...
					if (HasSubStr(xi_refCharacter.equip.blade, "topor"))
						return FloatToString(8.0 + BB + (coeff * 2), 1) + "%/" + FloatToString(8.0 + CB + (coeff * 2), 1) + "%";
					return FloatToString(5.0 + BB + (coeff * 2), 1) + "%/" + FloatToString(5.0 + CB + (coeff * 2), 1) + "%";
				}
				else
					return FloatToString(makefloat(BB), 1) + "%/" + FloatToString(makefloat(CB), 1) + "%";
				break;
			case "stun":
				int stunchance = 0;
				if (CheckAttribute(weapon, "special.valueStS"))
					stunchance = sti(weapon.special.valueStS);
				return stunchance + "%/1-3 сек.";
				break;
			case "trauma":
				int trauma = 0;
				if (CheckAttribute(weapon, "special.valueT"))
					trauma = sti(weapon.special.valueT);
				return trauma + "%/20-60 сек.";
				break;
			case "poisonattack":
				int poisonattackV = 0;
				if (CheckAttribute(weapon, "special.valueP"))
					poisonattackV = sti(weapon.special.valueP);
				if (xi_refCharacter.sex == "skeleton" || xi_refCharacter.sex == "crab" || HasSubStr(xi_refCharacter.model, "Canib_"))
					poisonattackV += 15;
		...
		else
		{
			...
			case "breakchance":
				if (LAi_GetBladeFencingType(xi_refCharacter) == "FencingHeavy")
				{
					coeff = makefloat(GetCharacterSkillSimple(xi_refCharacter, "FencingHeavy")) / 20;
					if (HasSubStr(xi_refCharacter.equip.blade, "topor"))
						return FloatToString(8.0 + (coeff * 2), 1) + "%/" + FloatToString(8.0 + (coeff * 2), 1) + "%";
...
void UpdateStatsValues()
{
	...
	if (CheckAttribute(xi_refCharacter, "Ship.Cannons"))
	{
		...
		if (sti(rCannon.type) == CANNON_NAME_CULVERINE)
			cannontype = "Кул(" + rCannon.caliber + "фт" + ")";
		if (sti(rCannon.type) == CANNON_NAME_CANNON)
			cannontype = "Пуш(" + rCannon.caliber + "фт" + ")";
	}
	for (int n = 1; n <= 5; n++)
	{
		sRow = "tr" + (n + 1);
		...
		{
		case 1:
			GameInterface.TABLE_SHIP_OTHERS.(sRow).td1.icon.group = "GOODS";
			GameInterface.TABLE_SHIP_OTHERS.(sRow).td1.icon.image = "Cannon_48";
			break;
		case 2:
			GameInterface.TABLE_SHIP_OTHERS.(sRow).td1.icon.group = "GOODS";
			GameInterface.TABLE_SHIP_OTHERS.(sRow).td1.icon.image = "Balls";
			break;
		case 3:
			GameInterface.TABLE_SHIP_OTHERS.(sRow).td1.icon.group = "GOODS";
			GameInterface.TABLE_SHIP_OTHERS.(sRow).td1.icon.image = "Grapes";
			break;
		case 4:
			GameInterface.TABLE_SHIP_OTHERS.(sRow).td1.icon.group = "GOODS";
			GameInterface.TABLE_SHIP_OTHERS.(sRow).td1.icon.image = "Knippels";
			break;
		case 5:
			GameInterface.TABLE_SHIP_OTHERS.(sRow).td1.icon.group = "GOODS";
			GameInterface.TABLE_SHIP_OTHERS.(sRow).td1.icon.image = "Bombs";
			break;
		}
		GameInterface.TABLE_SHIP_OTHERS.(sRow).td1.icon.width = 20;
		GameInterface.TABLE_SHIP_OTHERS.(sRow).td1.icon.height = 20;
		if (n == 1)
			GameInterface.TABLE_SHIP_OTHERS.(sRow).td2.str = "Дальность стрельбы: ";
		else
			GameInterface.TABLE_SHIP_OTHERS.(sRow).td2.str = rangefrom[n - 1];
		if (n > 1)
		...
		if (CheckAttribute(xi_refCharacter, "Ship.type") && sti(xi_refCharacter.ship.type) != SHIP_NOTUSED)
		{
			...
			{
			case 1:
				GameInterface.TABLE_SHIP_OTHERS.(sRow).td3.str = cannontype;
				break;
			case 2:
				GameInterface.TABLE_SHIP_OTHERS.(sRow).td3.str = GetShootDistance(xi_refCharacter, "ball") + " ед.";
				break;
			case 3:
				GameInterface.TABLE_SHIP_OTHERS.(sRow).td3.str = GetShootDistance(xi_refCharacter, "grape") + " ед.";
				break;
			case 4:
				GameInterface.TABLE_SHIP_OTHERS.(sRow).td3.str = GetShootDistance(xi_refCharacter, "knippel") + " ед.";
				break;
			case 5:
				GameInterface.TABLE_SHIP_OTHERS.(sRow).td3.str = GetShootDistance(xi_refCharacter, "bomb") + " ед.";
				break;
			}
		}
		else
			GameInterface.TABLE_SHIP_OTHERS.(sRow).td3.str = "0 ед.";
	...
	for (n = 6; n < 14; n++)
	{
		sRow = "tr" + (n + 1);
		if (n < 7)
			GameInterface.TABLE_SHIP_OTHERS.(sRow).UserData.ID = "SpeedBonus";
		...
		GameInterface.TABLE_SHIP_OTHERS.(sRow).td2.align = "left";
		if (CheckAttribute(xi_refCharacter, "Ship.type") && sti(xi_refCharacter.ship.type) != SHIP_NOTUSED)
			GameInterface.TABLE_SHIP_OTHERS.(sRow).td3.str = ShowStatValue("shipspeed" + (n - 5));
		else
			GameInterface.TABLE_SHIP_OTHERS.(sRow).td3.str = "0%";
		GameInterface.TABLE_SHIP_OTHERS.(sRow).td3.scale = 0.8;
		GameInterface.TABLE_SHIP_OTHERS.(sRow).td3.align = "right";
	}
	...
	for (n = 1; n < 8; n++)
	{
		sRow = "tr" + (n + 14);
		if (n < 2)
			GameInterface.TABLE_SHIP_OTHERS.(sRow).UserData.ID = "ManevrBonus";
		...
		if (n == 1)
		{
			GameInterface.TABLE_SHIP_OTHERS.(sRow).td2.textoffset = "0,0";
			GameInterface.TABLE_SHIP_OTHERS.(sRow).td2.str = "Маневренность:";
		}
		GameInterface.TABLE_SHIP_OTHERS.(sRow).td2.scale = 0.8;
		GameInterface.TABLE_SHIP_OTHERS.(sRow).td2.align = "left";
...
void ChoosePerk()
{
	...
	if (CheckAttribute(&ChrPerksList, "list." + perkName + ".condition"))
	{
		...
		{
			if (perkName == "SeaDogProfessional" || perkName == "GrapplingProfessional" || perkName == "CannonProfessional" || perkName == "Builder" || perkName == "InstantRepair" || perkName == "ShipDefenseProfessional" || perkName == "SailingProfessional" || perkName == "EmergentSurgeon")
				ok = false;
		}
	}
	else
	...
	{
		int contrap = 0;
		if (CheckAttribute(pchar, "contrabandmoneyback"))
			contrap = sti(pchar.contrabandmoneyback);
		SetFormatedText("CONTRAP", contrap + "/" + (3000000 + (MOD_SKILL_ENEMY_RATE * 100000)));
	}
	else
		SetFormatedText("CONTRAP", "");
...
void AcceptPerk()
{
	...
	if (perkName == "SeaDogProfessional")
	{
		pchar.SeaDogProfessionalSwitch = 1;
		log_info("Вымпел Грозы Морей теперь можно поднять в меню наций.");
	}
	// перерисуем все -->
	SetFormatedText("TABSTR_1", XI_ConvertString("Personal_abilities") + " " + xi_refCharacter.perks.FreePoints_self);
	SetFormatedText("TABSTR_2", XI_ConvertString("Ship_abilities") + " " + xi_refCharacter.perks.FreePoints_ship);
