...
void ShowPGGInfo()
{
	...
	{ // нет ПГГ в списке
		...
		string texturedata;
		if (IsCharacterPerkOn(chr, "Grunt"))
			texturedata = "INTERFACES\Sith\Char_Master.tga";
		if (IsCharacterPerkOn(chr, "Trader"))
			texturedata = "INTERFACES\Sith\Char_Merchant.tga";
		if (IsCharacterPerkOn(chr, "Fencer"))
		...
		if (IsCharacterPerkOn(chr, "Agent"))
			texturedata = "INTERFACES\Sith\Char_SecretAgent.tga";
		if (IsCharacterPerkOn(chr, "SeaWolf"))
...
void OnTableClick()
{
	...
	if (!SendMessage(&GameInterface, "lsl", MSG_INTERFACE_MSG_TO_NODE, sControl, 1))
	{
		...
		else
		{
			lastsort = iColumn;
			bLastSort = 1;
		} //запоминаем сортировку и меняем направление сортировки, если это повторный клик по колонке
