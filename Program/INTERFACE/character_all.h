...
void FillCharactersScroll()
{
	...
	if (checkAttribute(pchar, "chr_ai.HeavyTrauma"))
		pRef2.str4 = "#" + "Ранен " + (sti(pchar.chr_ai.HeavyTrauma) * 24 - makeint(environment.time)) + " ч"; // инфошка травмы ГГ - Gregg
	...
	for (i = 0; i < nOffSize; i++)
	{
		...
		if (_curCharIdx != -1)
		{
			...
			if (checkAttribute(&Characters[_curCharIdx], "HPminusDaysNeedtoRestore"))
				pRef2.str4 = "#" + "Ранен " + (sti(Characters[_curCharIdx].HPminusDaysNeedtoRestore) - sti(Characters[_curCharIdx].HPminusDays)) + "д"; // инфошка контузии - Gregg
		}
		else
		{
