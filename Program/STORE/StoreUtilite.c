...
int GetStoreGoodsPrice(ref _refStore, int _Goods, int _PriceType, ref chref, int _qty)
{
	...
	if (_PriceType == PRICE_TYPE_BUY)
	{
		...
		if (tradeType == TRADE_TYPE_CANNONS)
			skillModify *= 1.3; //цены пушек меняем после применения перков, иначе непропорционально их влияние на итог
	}
	else
	{
	...
	// boal 23.01.2004 -->
	if (MakeInt(basePrice * tradeModify * skillModify + 0.5) < 1)
		return 1;
	// boal 23.01.2004 <--
	if (CheckAttribute(pchar, "Goods.Store.Contraband"))
...
int GetProductGoods(int tradeType, bool bLarge, int _i)
{
	...
	// boal 25.01.2004 -->
	case TRADE_TYPE_NORMAL:
		return makeint((20 + rand(10) * 10) / rate);
		break;
	case TRADE_TYPE_AMMUNITION:
		return makeint((20 + rand(10) * 10) / rate);
		break;
	case TRADE_TYPE_EXPORT:
		...
		break;
	case TRADE_TYPE_IMPORT:
		return makeint((10 + rand(6) * 10) / rate);
		break;
	case TRADE_TYPE_CONTRABAND:
		return rand(20);
		break;
	case TRADE_TYPE_CANNONS:
		...
		break;
	case TRADE_TYPE_AGGRESSIVE:
		return makeint((5 + rand(6) * 10) / 2 * rate);
		break;
		// boal 25.01.2004 <--
	}
...
void UpdateSmugglers()
{
	...
	{
		max_gold = 1000000 increment = 100000;
	}
