...
void Ascold_actorSetWarrior(string qName)
{
	...
	if (iTemp != -1 && !LAi_IsDead(&characters[iTemp]))
		LAi_SetWarriorTypeNoGroup(&characters[iTemp]);
	for (i = 1; i <= 8; i++)
	{
...
void BlueBird_seaBattle(string qName)
{
	...
	if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
		sld = GetCharacter(NPC_GenerateCharacter("BlueBirdCapitain", "PGG_DeLuck_0", "man", "spy", 20, PIRATE, 30, true));
	else
		sld = GetCharacter(NPC_GenerateCharacter("BlueBirdCapitain", "PGG_DeLuck_0", "man", "man_fast", 20, PIRATE, 30, true));
	sld.name = "Фольке";
	sld.lastname = "Дэллюк";
	...
	sTemp = "quest_ship_10";
	if (pchar.location.from_sea == "Shore_ship1")
		sTemp = "quest_ship_8";
...
void BlueBird_over(string qName)
{
	...
	AddSimpleRumourCity("А, судар" + GetSexPhrase("ь", "ыня") + "! Вы же ещё не знаете главной новости! Бывший владелец местного магазина - Паскаль Вуазье сбежал! Оказывается, он скупал все товары, что местные пираты добывали с торговых судов. Для этого дела у него целый пакгауз отведён был. А когда торгаши прознали и наняли охотников за его головой - спешно всё продал и подался в бега, ха-ха-ха. Вот делов-то было! почитай неделю без рому сидели, пока этот пройдоха Бенджамин Франклин дело к своим рукам не прибрал.\n" +
							"Да что толку - торгаш с него никудышный - всё никак не найдёт простофилю, который бы согласился склад прежнего хозяина арендовать... В политику бы ему - там таким ушлым самое место, хе-хе...",
						"Pirates", 30, 3, "");
	// --> ugeen
}
...
void PiratesLine_q1_arrest(string qName)
{
	...
	for (i = 1; i <= 3; i++)
	{
		...
		if (i == 1)
			...
			LAi_ActorFollow(sld, characterFromId("QuestSold_1"), "", -1);
	}
...
void PQ8_controlShore48(string qName)
{
	...
	{																		 //ГГ вовремя и на месте -->
		...
		if (sti(pchar.questTemp.piratesLine.crewHero) > 5)
			pchar.questTemp.piratesLine.crewHero = 5;
...
void HostessChurch_null(string qName) //нулим квест
{
	...
	if (characters[GetCharacterIndex(pchar.questTemp.different.HostessChurch.city + "_Hostess")].questChurch == "taken")
	{
		...
		if (CheckAttribute(pchar, "HellSpawnLocation"))
...
{
	if (!CheckAttribute(pchar, "quest." + qName + ".CapId"))
		return;
	int capIndex = GetCharacterIndex(pchar.quest.(qName).CapId) if (capIndex != -1)
	{
...
void ChurchNightGuard_exitFromLoc(string qName)
{
	...
	if (sti(pchar.questTemp.different.Church_NightGuard.NightQty) == 0)
		pchar.questTemp.different.Church_NightGuard.NightQty = 1;
	else
		pchar.questTemp.different.Church_NightGuard.NightQty = sti(pchar.questTemp.different.Church_NightGuard.NightQty) + GetQuestPastDayParam("questTemp.different.Church_NightGuard");
...
void LSC_SaveSesilGalard_2()
{
	...
	for (i = 1; i <= 3; i++)
	{
		...
		if (sti(pchar.rank) < 12)
			FantomMakeCoolFighter(sld, iTemp, 50, 50, "topor3", "pistol2", 50);
		else
			FantomMakeCoolFighter(sld, iTemp, 80, 80, "topor2", "pistol6", 50);
		LAi_SetWarriorType(sld);
		LAi_group_MoveCharacter(sld, "EnemyFight");
...
void LSC_fightEnterResidence(string qName)
{
	...
	//отравляем наверх
	string sLocator;
	iTemp = GetCharacterIndex("Ment_1") if (iTemp != -1)
	{
		sld = &characters[iTemp];
	...
	//------------------------
	iTemp = GetCharacterIndex("Ment_2") if (iTemp != -1)
	{
		sld = &characters[iTemp];
		LAi_CharacterReincarnation(sld, false, false);
		LAi_SetLSCoutTypeNoGroup(sld);
	...
	//------------------------
	iTemp = GetCharacterIndex("Ment_3") if (iTemp != -1)
	{
		sld = &characters[iTemp];
		LAi_CharacterReincarnation(sld, false, false);
		LAi_SetLSCoutTypeNoGroup(sld);
	...
	//в резиденции
	iTemp = GetCharacterIndex("Ment_4") if (iTemp != -1)
	{
		sld = &characters[iTemp];
		LAi_SetWarriorTypeNoGroup(sld);
		LAi_warrior_DialogEnable(sld, false);
	...
	iTemp = GetCharacterIndex("Ment_5") if (iTemp != -1)
	{
		sld = &characters[iTemp];
		LAi_SetWarriorTypeNoGroup(sld);
		LAi_warrior_DialogEnable(sld, false);
...
void LSC_climeUsurer_4(string qName)
{
	...
	LAi_LocationFightDisable(loadedLocation, true);
	if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
		sld = GetCharacter(NPC_GenerateCharacter("DeadmansGod", "mictlantecuhtli", "skeleton", "spy", 100, PIRATE, 0, true));
	else
		sld = GetCharacter(NPC_GenerateCharacter("DeadmansGod", "mictlantecuhtli", "skeleton", "man_fast", 100, PIRATE, 0, true));
...
void Teno_startInShore_2()
{
	...
	//Ставим вождя
	if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
		sld = GetCharacter(NPC_GenerateCharacter("Montesuma", "Aztec", "man", "spy", 50, PIRATE, -1, true)); // LEO: Превозмогаторам - страдать 01.12.2021
	else
...
void DragunInvansion3(string qName)
{
	...
	for (int i = 0; i <= 4; i++)
	{
		...
		if (i == 3)
			LAi_ActorRunToLocation(sld, "reload", "reload3", "none", "", "", "", 20);
		if (i == 4)
			LAi_ActorRunToLocation(sld, "reload", "reload5", "none", "", "", "", 20);
	}
}
...
void PrepareBridgeTown(string qName)
{
	...
	//контрик нужен
	sld = &characters[GetCharacterIndex("Bridgetown_Smuggler")] LoginCharacter(sld, "Bridgetown_tavern");
}
...
{
	if (CheckAttribute(PChar, "quest.Winterwood2Place"))
		Pchar.quest.Winterwood2Place.over = "yes";
	if (CheckAttribute(PChar, "quest.CapBloodLineTimer_3"))
		Pchar.quest.CapBloodLineTimer_3.over = "yes";
...
void AttakShip(string qName)
{
	...
	for (int i = 0; i <= 9; i++)
	{
		...
		if (iChar != -1)
		{
			sld = &characters[iChar] LAi_SetWarriorTypeNoGroup(sld);
			if (Pchar.questTemp.CapBloodLine.withCrew)
				LAi_SetImmortal(sld, false);
		}
	}
	LAi_group_FightGroups("EnemyFight", LAI_GROUP_PLAYER, true);
...
void RestoreBridgetown()
{
	...
	//прерывания снять
	if (CheckAttribute(PChar, "quest.CureLordMovie"))
		Pchar.quest.CureLordMovie.over = "yes";
	if (CheckAttribute(PChar, "quest.DragunInvansion"))
		Pchar.quest.DragunInvansion.over = "yes";
	if (CheckAttribute(PChar, "quest.CapBloodLine_q1_1"))
		Pchar.quest.CapBloodLine_q1_1.over = "yes";
	if (CheckAttribute(PChar, "quest.onPlantation"))
		Pchar.quest.onPlantation.over = "yes";
	if (CheckAttribute(PChar, "quest.CapBloodDetectBlades1"))
		Pchar.quest.CapBloodDetectBlades1.over = "yes";
	if (CheckAttribute(PChar, "quest.CapBloodDetectBlades2"))
		Pchar.quest.CapBloodDetectBlades2.over = "yes";
	if (CheckAttribute(PChar, "quest.CapBloodLineTimer_1"))
		Pchar.quest.CapBloodLineTimer_1.over = "yes";
	if (CheckAttribute(PChar, "quest.CureMisStid"))
		Pchar.quest.CureMisStid.over = "yes";
	if (CheckAttribute(PChar, "quest.PrepareToEscape2"))
		Pchar.quest.PrepareToEscape2.over = "yes";
	if (CheckAttribute(PChar, "quest.PrepareToEscape3"))
		Pchar.quest.PrepareToEscape3.over = "yes";
	if (CheckAttribute(PChar, "quest.WinterwoodDuel"))
		Pchar.quest.WinterwoodDuel.over = "yes";
	if (CheckAttribute(PChar, "quest.MoneyForDieke"))
		Pchar.quest.MoneyForDieke.over = "yes";
	if (CheckAttribute(PChar, "quest.CapBloodLineTimer_2"))
		Pchar.quest.CapBloodLineTimer_2.over = "yes";
	if (CheckAttribute(PChar, "quest.QUsurer"))
		Pchar.quest.QUsurer.over = "yes";
	if (CheckAttribute(PChar, "quest.CapBloodEscape1"))
		Pchar.quest.CapBloodEscape1.over = "yes";
	if (CheckAttribute(PChar, "quest.CapBloodEscape2"))
		Pchar.quest.CapBloodEscape2.over = "yes";
	if (CheckAttribute(PChar, "quest.CapBloodEscape3"))
		Pchar.quest.CapBloodEscape3.over = "yes";
	if (CheckAttribute(PChar, "quest.CapBloodEscape4"))
		Pchar.quest.CapBloodEscape4.over = "yes";
	if (CheckAttribute(PChar, "quest.CapBloodEscape5"))
		Pchar.quest.CapBloodEscape5.over = "yes";
	if (CheckAttribute(PChar, "quest.CapBloodEscape6"))
		Pchar.quest.CapBloodEscape6.over = "yes";
	if (CheckAttribute(PChar, "quest.SolderTakeBladesDie"))
		Pchar.quest.SolderTakeBladesDie.over = "yes";
...
void Slavetrader_GetRandomShore() //выбор бухты
{
	...
	while (pchar.questTemp.Slavetrader.Island.Shore == "")
	{
		...
		pchar.questTemp.Slavetrader.Island.Shore = GetIslandRandomShoreId(pchar.questTemp.Slavetrader.Island);
		if (!isLocationFreeForQuests(pchar.questTemp.Slavetrader.Island))
			pchar.questTemp.Slavetrader.Island.Shore = ""; //уж лучше фик куда, чем глюки и конфликты
	}
}
...
void Slavetrader_Bonanza(string qName) //cоздание индейцев
{
	...
	for (i = 1; i <= 8; i++)
	{
		...
		sTemp = LAi_FindNearestFreeLocator("goto", locx, locy, locz);
		if (sTemp == "")
			sTemp = LAi_FindNearestLocator("goto", locx, locy, locz);
...
void zpq_seaBattle(string qName)
{
	LAi_group_Delete("EnemyFight");
	if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
		sld = GetCharacter(NPC_GenerateCharacter("zpqCaptain", "Lil_Jim_0", "man", "spy", 40, PIRATE, 1, true));
	else
		sld = GetCharacter(NPC_GenerateCharacter("zpqCaptain", "Lil_Jim_0", "man", "man_fast", 40, PIRATE, 1, true));
...
void CreateCarlos() //создаем Карлоса
{
	ref sld, ch;
	if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
		sld = GetCharacter(NPC_GenerateCharacter("Carlos", "ozg_carlos", "man", "spy", 20, PIRATE, -1, false));
	else
		sld = GetCharacter(NPC_GenerateCharacter("Carlos", "ozg_carlos", "man", "man_fast", 20, PIRATE, -1, false));
	sld.name = "Карлос";
	sld.lastname = "Кассир";
...
void CreateHoum() //создадим Холма
{
	ref sld;
	if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
		sld = GetCharacter(NPC_GenerateCharacter("Houm", "ozg_horn", "man", "spy", 25, ENGLAND, -1, false));
	else
		sld = GetCharacter(NPC_GenerateCharacter("Houm", "ozg_horn", "man", "man_fast", 25, ENGLAND, -1, false));
...
void Headhunter_CreateHoumShips(string qName) //cоздание кораблей Хоума
{
	...
	if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
		sld = GetCharacter(NPC_GenerateCharacter("Houm_helper", "pirate_1", "man", "spy", 35, ENGLAND, -1, true)); //создание кэпа-помощника
	else
		sld = GetCharacter(NPC_GenerateCharacter("Houm_helper", "pirate_1", "man", "man_fast", 35, ENGLAND, -1, true)); //создание кэпа-помощника
	sld.name = "Андре";
	sld.lastname = "Деготь";
...
void Headhunter_CreateRatpinasse(string qName) //создание пинаса Аделаида с перпомом
{
	...
	if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
		sld = GetCharacter(NPC_GenerateCharacter("RatSeaOfficer", "officer_15", "man", "spy", 35, FRANCE, -1, true)); //создание кэпа
	else
		sld = GetCharacter(NPC_GenerateCharacter("RatSeaOfficer", "officer_15", "man", "man_fast", 35, FRANCE, -1, true)); //создание кэпа
...
void CreateRatOfficer() //посадим в таверну самого Крысу - не будем модель прятать
{
	ref sld;
	if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
		sld = GetCharacter(NPC_GenerateCharacter("RatOfficer", "ozg_piRat", "man", "spy", 10, PIRATE, -1, false));
	else
		sld = GetCharacter(NPC_GenerateCharacter("RatOfficer", "ozg_piRat", "man", "man_fast", 10, PIRATE, -1, false));
	sld.name = "Матиас";
	sld.lastname = "Грин";
	sld.dialog.filename = "Quest\Other_quests_NPC.c";
...
void Headhunter_CreateRatTruepinasse(string qName) //создание пинаса Аделаида с Крысой
{
	...
	if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
		sld = GetCharacter(NPC_GenerateCharacter("RatCaptain", "ozg_piRat", "man", "spy", 25, PIRATE, -1, true)); // LEO: Превозмогаторам страдать 08.12.2021
	else
		sld = GetCharacter(NPC_GenerateCharacter("RatCaptain", "ozg_piRat", "man", "man_fast", 25, PIRATE, -1, true)); //создание кэпа
	sld.name = "Матиас";
	sld.lastname = "Грин";
	sld.DontRansackCaptain = true;
	sld.HeroModel = "ozg_piRat,ozg_piRat_1,ozg_piRat_2,ozg_piRat_3,ozg_piRat_4,ozg_piRat_5";
...
void RatHunters_Enter(string qName) //пришли киллеры
{
	...
	{
		if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
			ref sld = GetCharacter(NPC_GenerateCharacter("RatHunters" + i, "officer_7", "man", "spy", iTemp, PIRATE, -1, true)); // LEO: Превозмогаторам страдать 08.12.2021
		else
			sld = GetCharacter(NPC_GenerateCharacter("RatHunters" + i, "officer_7", "man", "man_fast", iTemp, PIRATE, -1, true));
...
void CreateHalen() //создаем Халена и ближайшее окружение
{
	ref sld;
	if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
		sld = GetCharacter(NPC_GenerateCharacter("Halen", "ozg_martin", "man", "spy", 30, HOLLAND, -1, false)); // LEO: Превозмогаторам страдать 08.12.2021
	else
		sld = GetCharacter(NPC_GenerateCharacter("Halen", "ozg_martin", "man", "man_fast", 30, HOLLAND, -1, false));
	sld.name = "Мартин";
...
void Headhunter_SeabattleInPort(string qName) //создание корвета Халена
{
	...
	{
		if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
			sld = GetCharacter(NPC_GenerateCharacter("Halen1", "officer_17", "man", "spy", 25, HOLLAND, -1, true)); // LEO: Превозмогаторам страдать 08.12.2021
		else
			sld = GetCharacter(NPC_GenerateCharacter("Halen1", "officer_17", "man", "man_fast", 25, HOLLAND, -1, true));
		if (bHardBoss)
			sld.AlwaysReload = true; //перезарядка независимо от Дозарядки
	}
	else
	{
		if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
			sld = GetCharacter(NPC_GenerateCharacter("Halen1", "ozg_martin", "man", "spy", 25, HOLLAND, -1, true)); // LEO: Превозмогаторам страдать 08.12.2021
		else
			sld = GetCharacter(NPC_GenerateCharacter("Halen1", "ozg_martin", "man", "man_fast", 25, HOLLAND, -1, true)); //создание кэпа
		sld.name = "Мартин";
		sld.lastname = "ван Хален";
		sld.HeroModel = "ozg_martin,ozg_martin_1,ozg_martin_2,ozg_martin_3,ozg_martin_4,ozg_martin_5";
		if (bHardBoss)
	...
	if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
		sld = GetCharacter(NPC_GenerateCharacter("Halen_helper", "off_hol_2", "man", "spy", 25, HOLLAND, -1, true)); // LEO: Превозмогаторам страдать 08.12.2021
	else
		sld = GetCharacter(NPC_GenerateCharacter("Halen_helper", "off_hol_2", "man", "man_fast", 25, HOLLAND, -1, true)); //создание кэпа
	sld.DontRansackCaptain = true;
	if (bHardBoss)
		sld.AlwaysReload = true;													   //перезарядка независимо от Дозарядки
...
void CreateMiko() //создадим Мико Пистольеро
{
	ref sld;
	if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
		sld = GetCharacter(NPC_GenerateCharacter("Miko", "ozg_miko", "man", "spy", 40, SPAIN, -1, false)); // LEO: Превозмогаторам страдать 08.12.2021
	else
		sld = GetCharacter(NPC_GenerateCharacter("Miko", "ozg_miko", "man", "man_fast", 40, SPAIN, -1, false));
	...
	{
		if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
			sld = GetCharacter(NPC_GenerateCharacter("AddPearlguard" + i, "Pirate_" + (rand(24) + 1), "man", "spy", 25, SPAIN, -1, true)); // LEO: Превозмогаторам страдать 08.12.2021
		else
			sld = GetCharacter(NPC_GenerateCharacter("AddPearlguard" + i, "Pirate_" + (rand(24) + 1), "man", "man_fast", 25, SPAIN, -1, true));
...
void Headhunter_Jahunters(string qName) //наймиты Джа в бухте
{
	...
	{
		if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
			ref sld = GetCharacter(NPC_GenerateCharacter("JaHunters" + i, "officer_17", "man", "spy", 25, PIRATE, -1, true)); // LEO: Превозмогаторам страдать 08.12.2021
		else
			sld = GetCharacter(NPC_GenerateCharacter("JaHunters" + i, "officer_17", "man", "man_fast", 25, PIRATE, -1, true));
...
void Create_Ja(string qName) //cоздаем Фрегат "Ахерон" с Джа
{
	...
	bQuestDisableMapEnter = true;
	if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
		sld = GetCharacter(NPC_GenerateCharacter("Jafar", "QuestCaptain", "man", "spy", 28, PIRATE, -1, true)); // LEO: Превозмогаторам - страдать 01.12.2021
	else
		sld = GetCharacter(NPC_GenerateCharacter("Jafar", "QuestCaptain", "man", "man_fast", 28, PIRATE, -1, true));
	sld.name = "Джа";
...
void FreeTichingituOver(string qName) //удаляем Тичингиту
{
	...
	if (CheckCharacterItem(pchar, "letter_1"))
		RemoveItems(pchar, "letter_A", 1); // 170712
}
