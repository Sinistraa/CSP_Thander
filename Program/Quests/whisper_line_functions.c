...
void CaptureCapitol_SeaBattle()
{
	...
	for (i = 1; i < 8; i++)
	{
		...
		if (i == 1)
			FantomMakeQuickSailor(sld, SHIP_GALEON50, "Откровение", CANNON_TYPE_CANNON_LBS32, 50, 50, 50);
		if (i == 2)
			FantomMakeQuickSailor(sld, SHIP_GALEON1, "Деяния святых апостолов", CANNON_TYPE_CANNON_LBS24, 50, 50, 50);
		if (i == 3)
			FantomMakeQuickSailor(sld, SHIP_REVENGE, "Евангелие", CANNON_TYPE_CANNON_LBS32, 50, 50, 50);
		if (i == 4)
			FantomMakeQuickSailor(sld, SHIP_GALEON_H, "Плач Иеремии", CANNON_TYPE_CANNON_LBS32, 50, 50, 50);
		if (i == 5)
			FantomMakeQuickSailor(sld, SHIP_FELIPE, "Притчи", CANNON_TYPE_CANNON_LBS32, 50, 50, 50);
		if (i == 6)
			FantomMakeQuickSailor(sld, SHIP_WARGALLEON2, "Псалмы", CANNON_TYPE_CANNON_LBS32, 50, 50, 50);
		if (i == 7)
			FantomMakeQuickSailor(sld, SHIP_GALEON_L, "Бытие", CANNON_TYPE_CANNON_LBS16, 50, 50, 50);
		...
		if (i == 1)
			Group_SetGroupCommander("CaptureCapitol_SeaBattle", sld.id);
		SetCharacterRelationBoth(sti(sld.index), GetMainCharacterIndex(), RELATION_ENEMY);
...
void CaptureCapitol_ShoreBattleRespite(string q)
{
	...
	if (startherotype == 2)
	{
		...
		FantomMakeCoolFighter(sld, 100, 100, 100, "blade24", "pistol4", 80);
...
void CaptureCapitol_OnLeaderDeath_1(string q)
{
	...
	if (startherotype == 2)
		...
		sld = GetCharacter(NPC_GenerateCharacter("Wh_Jim", "officer_" + (rand(63) + 1), "man", "man", 5, PIRATE, -1, false));
...
void WhisperTitanic_ToRandomTown()
{
	if (CheckAttribute(&colonies[1], "nation"))
	{
		...
		string prevCity = "";
		if (CheckAttribute(pchar, "TitanicCity"))
			prevCity = pchar.TitanicCity;
		while (!townIsSet)
		{
			nation = storeArray[rand(howStore - 1)];
			pchar.TitanicCity = colonies[nation].id;
			if (colonies[nation].id == "Beliz" || colonies[nation].id == "Santacatalina" || colonies[nation].id == "PortoBello" || colonies[nation].id == "Cartahena" || colonies[nation].id == "Maracaibo" || colonies[nation].id == "Caracas" || colonies[nation].id == "Cumana")
				townIsSet = true;
			if (prevCity == pchar.TitanicCity)
				townIsSet = false;
		}
