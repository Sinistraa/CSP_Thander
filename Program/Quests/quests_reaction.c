...
void QuestComplete(string sQuestName, string qname)
{
	...
	case "Survive_In_SeaOfficer":
		...
		if (iTemp != -1)
		{
			...
			if (CheckAttribute(sld, "PGGAi"))
			{
				...
				if (CheckAttribute(sld, "PerkValue.HPPlus"))
					npchar.PerkValue.HPPlus = 0;
	...
	case "saved_by_off":
		...
		if (!CheckAttribute(stelsTavern, "tavern"))
			pchar.questTemp.stels.tavern = 0;
	...
	case "Gatri_Hunters":
		if (CheckAttribute(pchar, "BSStart") && !CheckAttribute(pchar, "BSInProgress"))
		{
			...
			if (!CheckAttribute(pchar, "BSPrologue.GatriHunters"))
				AddQuestRecord("BSPrologue", "4");
			else
				AddQuestRecord("BSPrologue", "5");
	...
	case "Rand_ContrabandInterruptionAtSeaEnded":
		...
		AddSimpleRumourCity("А вот давеча баталия у маяка была. Какой-то капитан пытался продать крупную партию контрабанды, так его патрульная эскадра за энтим делом накрыла. С поличным, значит. " +
								"Грохоту было - весь город переполошился, а в местной лавке крышу ядром проломило! Да только без толку всё. Местные-то - скупщики значит, в джунглях спрятались, а капитан морем ушёл. Как есть, с носом нашу эскадру оставил! Так из бухты вырулил - любой лоцман позавидует!",
	...
	case "convoy_refused":
		...
		if (checkquestattribute("generate_convoy_quest_progress", "completed"))
		{
			...
			if (sti(pchar.questTemp.genquestcount) >= 10)
				UnlockAchievement("gen_quests", 1);
			if (sti(pchar.questTemp.genquestcount) >= 20)
				UnlockAchievement("gen_quests", 2);
			if (sti(pchar.questTemp.genquestcount) >= 40)
				UnlockAchievement("gen_quests", 3);
		}
	...
	case "Followers_activate": //Активация ходока
		for (i = 1; i <= 2; i++)
		{
			if (GetCharacterIndex("Walker_" + i) == -1)
				continue;
		...
		for (i = 1; i <= 2; i++)
		{
			if (GetCharacterIndex("Walker_sold_" + i) == -1)
				continue;
		...
		for (i = 1; i <= 2; i++)
		{
			if (GetCharacterIndex("WalkerGirl_husband_" + i) == -1)
				continue;
		...
		for (i = 1; i <= 2; i++)
		{
			if (GetCharacterIndex("WalkerGirl_sold_" + i) == -1)
				continue;
			...
			while (i < iTemp)
			{
				...
				if (iModel[ShipType] != "")
				{
					...
					{ //главарь банды
						...
						sld = GetCharacter(NPC_GenerateCharacter("MayorQuestGang_" + i, iModel[ShipType], "man", "man", Rank, PIRATE, 0, true));
					LAi_SetActorType(sld);
		...
		{ //шпион в коммонах
			...
			if (locations[iTemp].fastreload == characters[GetCharacterIndex(pchar.GenQuest.SeekSpy.MayorId)].city)
			{
				...
				if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
					sld = GetCharacter(NPC_GenerateCharacter("MayorQuestSpy", "citiz_" + (rand(11) + 1), "man", "spy", Rank, PIRATE, 0, true)); // LEO: Пизда превозмогаторам 08.12.2021
				else
					sld = GetCharacter(NPC_GenerateCharacter("MayorQuestSpy", "citiz_" + (rand(11) + 1), "man", "man_fast", Rank, PIRATE, 0, true));
				sld.dialog.filename = "MayorQuests_dialog.c";
	...
	case "SeekSpy_Afrer":
		...
		AddSimpleRumour(RandPhraseSimple("Вы знаете, губернатор " + sTemp + " поручил неко" + GetSexPhrase("ему капитану", "й морячке") + " " + GetMainCharacterNameDat() + " найти в городе вражеского лазутчика. И вы знаете, он" + GetSexPhrase("", "а") + " его наш" + GetSexPhrase("ел", "ла") + " и убил" + GetSexPhrase("", "а") + "!",
										 "Наш губернатор дал задание капитану " + GetMainCharacterNameDat() + " разыскать в городе вражеского агента. И " + GetSexPhrase("этот капитан нашел и прикончил", "она нашла и прикончила") + " лазутчика! Прекрасная работа, скажу я вам..."),
		...
		if (CheckAttribute(pchar, "GenQuest.KillSmugglers") && pchar.GenQuest.KillSmugglers.Areal == GiveArealByLocation(&locations[reload_location_index]))
		{
			...
			AddQuestUserData("MayorsQuestsList", "ColonyName", XI_ConvertString("Colony" + characters[GetCharacterIndex(pchar.GenQuest.KillSmugglers.MayorId)].city + "Gen"));
				...
				AddSimpleRumour(RandPhraseSimple("Вы знаете, губернатор " + sTemp + " поручил неко" + GetSexPhrase("ему капитану", "й морячке") + " " + GetMainCharacterNameDat() + " найти и уничтожить контрабандистов. И скажу я вам, он" + GetSexPhrase("", "а") + " это сделал" + GetSexPhrase("", "а") + ".",
												 "Наш губернатор " + sTemp + " дал задание капитану " + GetMainCharacterNameDat() + " разыскать контрабандистов в окрестностях города. Он" + GetSexPhrase("", "а") + " наш" + GetSexPhrase("ел", "ла") + " их и убил" + GetSexPhrase("", "а") + ", значит цены у торговцев скоро подскочат..."),
								sti(characters[sti(pchar.GenQuest.KillSmugglers.MayorId)].nation), 5, 1);
		}
		break;
	...
	case "DestroyPirate_PirateIsOut":
		...
		AddSimpleRumour(LinkRandPhrase("Вы слышали о недавнем бое капитана " + GetMainCharacterNameGen() + " с пиратами? Говорят, что пираты сумели ускользнуть от не" + GetSexPhrase("го", "е") + "...",
									   ...
									   "Эх, капитан " + GetFullName(pchar) + ", как же это вы упустили этих пиратов, а? Ищи теперь ветра в поле..."),
	...
	case "DestroyPirate_after":
		...
		AddSimpleRumour(LinkRandPhrase("Вы знаете, губернатор " + GetFullName(npchar) + " поручил некоему капитану " + GetMainCharacterNameDat() + " найти и уничтожить пирата, который занимался грабежом в окрестных водах. Так вот, кэп наш" + GetSexPhrase("ел", "ла") + " и потопил" + GetSexPhrase("", "а") + " " + GetStrSmallRegister(pchar.GenQuest.DestroyPirate.ShipType) + "!",
									   ...
									   "Ха, наконец-то наш губернатор взялся за обеспечение безопасного плавания в водах " + XI_ConvertString("Colony" + npchar.city + "Gen") + " - он поручил капитану " + GetMainCharacterNameDat() + " разыскать пирата, не дававшего покоя нашим торговцам. И вы знаете, эт" + GetSexPhrase("от капитан", "а девушка") + " потопил" + GetSexPhrase("", "а") + " " + GetStrSmallRegister(pchar.GenQuest.DestroyPirate.ShipType) + "!"),
						sti(npchar.nation), 5, 1);
		break;
	...
	case "DestroyPirate_toShore":
		...
		AddSimpleRumour(LinkRandPhrase("Вы знаете, губернатор " + GetFullName(npchar) + " поручил некоему капитану " + GetMainCharacterNameDat() + " найти и уничтожить пирата, что грабит торговцев у нас под носом. И что вы думаете, он ограбил и капитана " + GetMainCharacterNameGen() + " тоже!",
									   ...
									   "О-о-о, капитан, я слышал вам здорово досталось недавно, хи-хи... Как вам местные пираты? Не хотите еще их поискать? Ха-ха-ха!!"),
						sti(npchar.nation), 5, 1);
		//снимаем кораблики пирата и торговцев
		LAi_group_Delete("MayorPirateGroup");
		Group_DeleteGroup("MQGroupPirate");
	...
	case "Berglars_Ostin":
		chrDisableReloadToLocation = true;
		if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
			sld = GetCharacter(NPC_GenerateCharacter("Ostin", "Ostin", "man", "spy", 30, SPAIN, -1, false)); //LEO: Превозмогаторам страдать 08.12.2021
		else
			sld = GetCharacter(NPC_GenerateCharacter("Ostin", "Ostin", "man", "man_fast", 30, SPAIN, -1, false));
	...
	case "ColonelLinch_Exit":
		...
		sld.name = "Полковник Томас";
		sld.lastname = "Линч";
		sld.model.animation = "man" SendMessage(sld, "lss", MSG_CHARACTER_SETMODEL, sld.model, sld.model.animation);
		FaceMaker(sld);
	...
	case "ThreeFleutes_TalkIntoLaVega":
		...
		sld.name = "Капитан";
		sld.lastname = "Купер";
		sld.model.animation = "man" sld.model = "officer_8";
		SendMessage(sld, "lss", MSG_CHARACTER_SETMODEL, sld.model, sld.model.animation);
		FaceMaker(sld);
	...
	case "Sp6TakeMess_FightInPort":
		if (GetCharacterIndex("Messanger") == -1)
		{
			...
			characters[GetCharacterIndex("Messanger")].lifeDay = 0;
		break;
	...
	case "Fr2Letter_OutTavern_2":
		...
		if (GetQuestPastMinutesParam("questTemp") < 5)
		{
			...
			Log_QuestInfo("Не успел выбежать из таверны, рулим в бухту Пальмовый берег.");
		break;
	...
	case "Fr2Letter_OutTavern_3":
		if (GetQuestPastMinutesParam("questTemp") < 13)
		{
			...
			Log_QuestInfo("Не успел, рулим в бухту Пальмовый берег.");
		break;
...
*/
	...
	case "Ascold_InGraveAfterFight":
		...
		if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
			sld = GetCharacter(NPC_GenerateCharacter("LeifEricson", "Mummy", "skeleton", "spy", 100, PIRATE, -1, true)); // LEO: Превозмогаторам страдать 08.12.2021
		else
			sld = GetCharacter(NPC_GenerateCharacter("LeifEricson", "Mummy", "skeleton", "man_fast", 100, PIRATE, -1, true));
	...
	case "Teno_youWinFight":
		...
		if (iTemp != -1)
			characters[iTemp].dialog.currentnode = "Aztec1AF";
		iTemp = GetCharacterIndex("AztecCitizen_2");
		if (iTemp != -1)
			characters[iTemp].dialog.currentnode = "Aztec2AF";
	...
	//ОЗГ, Карлос Кассир
	case "CarlosDie":
		if (CheckAttribute(PChar, "quest.CarlosPlace"))
			Pchar.quest.CarlosPlace.over = "yes";
		if (CheckAttribute(PChar, "quest.HeadhunterTimer"))
			Pchar.quest.HeadhunterTimer.over = "yes";
		DoQuestCheckDelay("hide_weapon", 1.0);
		chrDisableReloadToLocation = false;
	...
	case "PDM_PI_Dead_Skelety":
		...
		if (pchar.rank >= 1 && pchar.rank <= 6)
			AddCharacterCrew(pchar, -30);
	...
	case "PDM_NK_Litsenzia":
		if (GetDaysContinueNationLicence(HOLLAND) < 30)
			GiveNationLicence(HOLLAND, 30);
		if (GetDaysContinueNationLicence(SPAIN) < 30)
			GiveNationLicence(SPAIN, 30);
		if (GetDaysContinueNationLicence(ENGLAND) < 30)
			GiveNationLicence(ENGLAND, 30);
		if (GetDaysContinueNationLicence(FRANCE) < 30)
			GiveNationLicence(FRANCE, 30);
	...
	case "SpookyScarySkeleton":
		if (loadedLocation.id == "Labirint_1" || loadedLocation.id == "Labirint_2" || loadedLocation.id == "Labirint_3")
		{
			...
			{
				if (sti(pchar.rank) > 20)
					pRank = sti(pchar.rank) + 5 + MOD_SKILL_ENEMY_RATE;
				else
					pRank = sti(pchar.rank) + 5 + sti(MOD_SKILL_ENEMY_RATE / 2);
			}
			else
	...
	case "SpawnGiantEvilSkeleton":
		if (loadedLocation.id != "DeckWithReefs" || CheckAttribute(pchar, "GiantEvilSkeletonSpawned"))
		{
			chrDisableReloadToLocation = false;
			break;
		}
		...
		if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
			sld = GetCharacter(NPC_GenerateCharacter("GiantEvilSkeleton", "PGG_Giant_5", "skeleton", "Giant_spy", iRank, PIRATE, 1, true)); // LEO: Превозмогаторам - страдать 01.12.2021
		else
...
{
	if (CheckAttribute(pchar, "Grandma"))
		return;
...
void QuestCursedSceleton()
{
	...
	if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
		sld = GetCharacter(NPC_GenerateCharacter("CursedSkeleton", "PGG_Skeletcap_0", "skeleton", "skeleton_spy", sti(pchar.rank) + 20, PIRATE, 1, true));
	else
		sld = GetCharacter(NPC_GenerateCharacter("CursedSkeleton", "PGG_Skeletcap_0", "skeleton", "skeleton_fast", sti(pchar.rank) + 20, PIRATE, 1, true));
