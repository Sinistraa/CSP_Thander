...
bool Ship_AutoAbordage(ref rCharacter, float fMinEnemyDistance)
{
	...
	if (fMinEnemyDistance < 70)
	{
		...
		if (Ship_GetDistance2DRel(rCharacter, rShipCharacter) > 8100.0)
		...
		if (!Character_IsAbordageEnable(rShipCharacter) || rShipCharIdx == GetMainCharacterIndex())
			return bSuccess;
		...
		if (rCharCrew < (stf(rShipCharCrew) * 0.6))
			return bSuccess;
		...
		if (bGrapplingProfessional || bAngleTest)
		{
			...
			{ // победа
				...
				{ //Change to consider group power
					//#20190706-01
	...
	for (k = 1; k < COMPANION_MAX; k++)
	{
		...
		if (cn >= 0)
		{
			...
			if (!GetRemovable(rCharacter))
				continue; // квестовые сами себ¤ спасают
