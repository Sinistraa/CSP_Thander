...
void SetTuningShipStates(ref chr)
{
	...
	if (rShip.class == 7)
		return; //мелкие корабли отсекаем
	//if(rShip.stolen == 0) return;	//генератор будет только на захваченных в море

	int iRand = 30;
	if (rand(iRand) == 0)
		SetShipMaxCaliberTuning(chr, true); //увеличение калибра
	if (rand(iRand) == 0)
		SetShipCapacityTuning(chr, true); //увеличение трюма
	if (rand(iRand) == 0)
		SetShipSpeedRateTuning(chr, true); //увеличение скорости
	if (rand(iRand) == 0)
		SetShipMastMultiplierTuning(chr, true); //увеличение прочности мачт
	if (rand(iRand) == 0)
		SetShipMaxCrewTuning(chr, true); //увеличение команды
	if (rand(iRand) == 0)
		SetShipTurnRateTuning(chr, true); //увеличение маневренности
	if (rand(iRand) == 0)
		SetShipHPTuning(chr, true); //увеличение ХП
}
...
void SetTuningStates2Ship(ref chr, bool MaxCaliber, bool Capacity, bool SpeedRate, bool MaxCrew, bool TurnRate, bool HP, bool MastMulti)
{
	...
	if (MaxCaliber)
		SetShipMaxCaliberTuning(chr, MaxCaliber); //увеличение калибра
	if (Capacity)
		SetShipCapacityTuning(chr, Capacity); //увеличение трюма
	if (SpeedRate)
		SetShipSpeedRateTuning(chr, SpeedRate); //увеличение скорости
	if (MastMulti)
		SetShipMastMultiplierTuning(chr, MastMulti); //увеличение прочности мачт
	if (MaxCrew)
		SetShipMaxCrewTuning(chr, MaxCrew); //увеличение команды
	if (TurnRate)
		SetShipTurnRateTuning(chr, TurnRate); //увеличение маневренности
	if (HP)
		SetShipHPTuning(chr, HP); //увеличение ХП
}
