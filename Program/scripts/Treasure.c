...
void FillMapForTreasure(ref item)
{
	...
	if (GetCharacterSPECIALSimple(pchar, SPECIAL_L) > rand(10))
	...
	{
		if (rand(12 + GetCharacterSPECIALSimple(pchar, SPECIAL_L)) == 1)
			item.MapTypeIdx = -1;
	...
	{ // из кусочков даем супер вещи
		/*while (item.MapLocId == "Muelle_town_04" || item.MapLocId == "Havanna_town_04") // не для халявы
       {
		...
		if (GetCharacterSPECIALSimple(pchar, SPECIAL_L) > rand(10))
			FillBoxForTreasureSuper(item);
		if (rand(100) == 0)
			item.BoxTreasure.mushket_Shtuzer = 1;
		if (rand(100) == 0)
...
string GetUniquePirateNameString(string type)
{
	...
	{
	case "Barbarigo":
		return "Агостино Барбариго";
		break;
	case "BlackBeard":
		return "Чёрной Бороды";
		break;
	case "Levasser":
		return "Левассера";
		break;
...
void Xzibit(ref item) //сундучный метод
{
	...
	{
	case 0:
		item.BoxTreasure.Chest_ammo = 1;
		break;
	case 1:
		item.BoxTreasure.Chest_treasure = 1;
		break;
	case 2:
		item.BoxTreasure.Chest_quest = 1;
		break;
	case 3:
		item.BoxTreasure.Chest_Craftsmans = 1;
		break;
	}
}
...
void FillBoxForTreasureAddition(ref item)
{
	...
	if (!CheckMainHeroTotem(sName) && rand(20) == 1)
		item.BoxTreasure.(sName) = 1;
}
...
void SetTreasureHunter(string temp)
{
	...
	for (i = 1; i <= j; i++)
	{
		if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
			sld = GetCharacter(NPC_GenerateCharacter(sCapId + i, "Pirate_" + (rand(24) + 1), "man", "spy", sti(PChar.rank) + 5, PIRATE, 0, true));
		else
			sld = GetCharacter(NPC_GenerateCharacter(sCapId + i, "Pirate_" + (rand(24) + 1), "man", "man_fast", sti(PChar.rank) + 5, PIRATE, 0, true));
		SetFantomParamHunter(sld); //крутые парни
...
void Survive_In_Sea_Go2Land()
{
	...
	{
		if (colonies[n].nation == "none")
			continue; // необитайки
		if (sti(colonies[n].nation) == PIRATE)
			continue;
		if (colonies[n].from_sea == "")
			continue; // необитайки
		if (!CheckAttribute(&Colonies[n], "islandLable") || Colonies[n].islandLable == "Mein")
			continue; // только острова
...
void GhostShipInit()
{
	...
	//============> Капитан призрака
	if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
		sld = GetCharacter(NPC_GenerateCharacter("GhostCapt", "PotCMobCap", "skeleton", "spy", 55, PIRATE, -1, true));
	else
		sld = GetCharacter(NPC_GenerateCharacter("GhostCapt", "PotCMobCap", "skeleton", "man_fast", 55, PIRATE, -1, true));
...
void Map_CreateFastWarriorTreasure()
{
	...
	{
		sld = GetCharacter(NPC_GenerateCharacter(sCapId + i, "off_hol_2", "man", "man", sti(PChar.rank) + 5, PIRATE, 15, true));
		if (CheckAttribute(PChar, "Luke.SpawnMaks"))
			CreatePGG_War(sld, PIRATE, pchar);
		else
			SetShipHunter(sld);
		SetFantomParamHunter(sld); //крутые парни
		SetCaptanModelByEncType(sld, "war");
