...
void GoldFleet()
{
    ...
    for (int k = 1; k <= (nfreg + ngal); k++)
    {
        ...
        if (k <= nfreg)
        {
            sld.Ship.Mode = "war";
            if (k < 3)
                sld.ship.type = GenerateShipExt(SHIP_SP_SANFELIPE, 1, sld);
            else
                sld.ship.type = GenerateShipExt(SHIP_ALEXIS, 1, sld);
            SetBaseShipData(sld);
            ...
            {
                if (k < 3)
                    AddCharacterGoods(sld, i, 10000);
                else
                    AddCharacterGoods(sld, i, 5000);
            }
...
void DefeatGoldFleet(string temp)
{
    ...
    if (sti(pchar.questTemp.GoldFleetNum) >= 1)
        UnlockAchievement("AchGoldFleet", 1);
    if (sti(pchar.questTemp.GoldFleetNum) >= 3)
        UnlockAchievement("AchGoldFleet", 2);
    if (sti(pchar.questTemp.GoldFleetNum) >= 7)
        UnlockAchievement("AchGoldFleet", 3);
