...
{
	if (GetCharacterItem(chref, "indian11") > 0)
		return;
	...
	for (i = 0; i < COMPANION_MAX; i++)
	{
		...
		if (cn >= 0)
		{
			...
			if (sti(chref.Ship.soiling) < 100)
			{
				...
				{
					if (rand(4) < 1)
						chref.Ship.soiling = sti(chref.Ship.soiling) + 1;
				}
				else
					chref.Ship.soiling = sti(chref.Ship.soiling) + 1;
			}
...
void DailyEatCrewUpdateForShip(ref rChar, bool IsCompanionTraveler) // IsCompanionTraveler - спец флаг для компаньонов-путешественников
{
		...
		if (cn > 0)
		{
			if (GetCargoGoods(rChar, GOOD_MEDICAMENT) < 1)
			{
				if (!IsCompanionTraveler)
					Log_Info("На " + XI_ConvertString(RealShips[sti(rChar.Ship.Type)].BaseName + "Dat") + " ''" + rChar.Ship.Name + "'' от болезней умерло " + FindRussianSailorString(cn, "No") + "!");
			...
			else
			{
				...
				{
					if (!IsCompanionTraveler)
						Log_Info("На " + XI_ConvertString(RealShips[sti(rChar.Ship.Type)].BaseName + "Dat") + " ''" + rChar.Ship.Name + "'' осталось мало медикаментов!");
				}
			}
		}
		// рабы
	...
	if (iCrewQty > 0)
	{
		if (GetCargoGoods(rChar, GOOD_RUM) >= iCrewQty)
		{
			...
			{
				if (!IsCompanionTraveler)
					Log_Info("На " + XI_ConvertString(RealShips[sti(rChar.Ship.Type)].BaseName + "Dat") + " ''" + rChar.Ship.Name + "'' закончились запасы рома!");
			}
			// поднимем мораль
	...
	else
	{
		...
		if (!IsCompanionTraveler)
			Log_Info("На " + XI_ConvertString(RealShips[sti(rChar.Ship.Type)].BaseName + "Dat") + " ''" + rChar.Ship.Name + "'' матросы голодают. Мораль команды падает!");
