...
void OfficersReactionResult()
{
	...
	if (CheckAttribute(Pchar, "questTemp.MunityOfficerIDX") && Pchar.questTemp.MunityOfficerIDX.begin == "1")
		return;
...
int FindFreeRandomOfficer()
{
	...
	string OfficerId;
	// special -->
	if (GetCharacterMaxOfficersQty(Pchar) < (GetNotQuestPassengersQuantity(Pchar) + GetCompanionQuantity(Pchar) - 1))
	{
		pchar.LeadershipLose = true;
		return -1;
	}
	if (GetCharacterMaxOfficersQty(Pchar) == (GetNotQuestPassengersQuantity(Pchar) + GetCompanionQuantity(Pchar) - 1))
	{
		DeleteAttribute(pchar, "LeadershipLose");
		return 0;
	}
	else
		DeleteAttribute(pchar, "LeadershipLose");
