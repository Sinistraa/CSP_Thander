...
int GenerateShip(int iBaseType, bool isLock)
{
	...
	{
	case 0:
		rRealShip.MastMultiplier = stf(stf(rRealShip.MastMultiplier) - makefloat(0.02 * rand(15)));
		break;
	case 1:
		rRealShip.MastMultiplier = stf(stf(rRealShip.MastMultiplier) + makefloat(0.02 * rand(15)));
		break;
...
int GenerateShipExt(int iBaseType, bool isLock, ref chr)
{
	...
	if (CheckAttribute(rRealShip, "QuestShip"))
		QuestShipDifficultyBoosts(rRealShip);
	else
	...
	rRealShip.soiling = 0;
	if (rand(4) == 0)
		GetRandomSpecialUpgrade(rRealShip);
...
void GetRandomSpecialUpgrade(ref rRealShip)
{
	for (int i = 0; i < 1 + rand(2); i++)
	{
		...
		case 0:
			...
			{
				if (CheckAttribute(rRealShip, "Tuning.HullSpecial") && rRealShip.Tuning.HullSpecial == "1")
					break; //уже был этот апгрейд
				rRealShip.Tuning.HullSpecial = 1;
				rRealShip.price = makeint(sti(rRealShip.price) * 1.5);
			}
			break;
		case 1:
			if (CheckAttribute(rRealShip, "Tuning.SailsSpecial") && rRealShip.Tuning.SailsSpecial == "1")
				break; //уже был этот апгрейд
		...
		case 2:
			if (rRealShip.Type.War == true && rRealShip.Type.Merchant == false)
			{
				if (CheckAttribute(rRealShip, "Tuning.CannonsSpecial") && rRealShip.Tuning.CannonsSpecial == "1")
					break; //уже был этот апгрейд
			...
			break;
		case 3:
			if (CheckAttribute(rRealShip, "Tuning.CuBot") && rRealShip.Tuning.CuBot == "1")
				break; //уже был этот апгрейд
		...
		case 4:
			...
			{
				if (CheckAttribute(rRealShip, "Tuning.BotPack") && rRealShip.Tuning.BotPack == "1")
					break; //уже был этот апгрейд
			...
			break;
		case 5:
			if (CheckAttribute(rRealShip, "Tuning.HighBort") && rRealShip.Tuning.HighBort == "1")
				break; //уже был этот апгрейд
...
void SetCabinTypeEx(ref rRealShip, int shipclass) //Выдача случайной каюты по классу не квестовым - Gregg
{
	...
	if (shipclass == 6 || shipclass == 5)
	{
		...
		{
		case 0:
			rRealShip.CabinType = "Cabin_Small";
			return;
			break;
		case 1:
			rRealShip.CabinType = "New_Cabin1";
			return;
			break;
		}
	}
	if (shipclass == 4 || shipclass == 3)
	{
		...
		{
		case 0:
			rRealShip.CabinType = "Cabin_Medium";
			return;
			break;
		case 1:
			rRealShip.CabinType = "Cabin_Medium2";
			return;
			break;
		case 2:
			rRealShip.CabinType = "New_Cabin2";
			return;
			break;
		}
	}
	if (shipclass == 2 || shipclass == 1)
	{
		...
		{
		case 0:
			rRealShip.CabinType = "Cabin_Quest";
			return;
			break;
		case 1:
			rRealShip.CabinType = "Cabin";
			return;
			break;
		case 2:
			rRealShip.CabinType = "Cabin_Huge";
			return;
			break;
		}
	}
...
int CreateBaseShip(int iBaseType)
{
	...
	{
	case 7:
		rRealShip.HullArmor = 4 + (rand(1) * hullarmor);
		break;
	case 6:
		rRealShip.HullArmor = 12 + (rand(1) * hullarmor);
		break;
	case 5:
		rRealShip.HullArmor = 16 + (rand(1) * hullarmor);
		break;
	case 4:
		rRealShip.HullArmor = 20 + (rand(2) * hullarmor);
		break;
	case 3:
		rRealShip.HullArmor = 24 + (rand(2) * hullarmor);
		break;
	case 2:
		rRealShip.HullArmor = 32 + (rand(2) * hullarmor);
		break;
	case 1:
		rRealShip.HullArmor = 42 + (rand(2) * hullarmor);
		break;
	}
...
void ResetShipCannonsDamages(ref refCharacter)
{
	...
	if (CheckAttribute(refShip, "Cannons.Borts.fcannon.damages"))
	{
		DeleteAttribute(refShip, "Cannons.Borts.fcannon.damages");
	}
	if (CheckAttribute(refShip, "Cannons.Borts.bcannon.damages"))
	{
		DeleteAttribute(refShip, "Cannons.Borts.bcannon.damages");
	}
	if (CheckAttribute(refShip, "Cannons.Borts.lcannon.damages"))
	{
		DeleteAttribute(refShip, "Cannons.Borts.lcannon.damages");
	}
	if (CheckAttribute(refShip, "Cannons.Borts.rcannon.damages"))
	{
		DeleteAttribute(refShip, "Cannons.Borts.rcannon.damages");
	}
}
...
void EmptyAllFantomShips()
{
	...
	for (i = FANTOM_CHARACTERS; i < TOTAL_CHARACTERS; i++) // фантомы в море
	{
		...
		else
		{
			...
			if (CheckAttribute(chr, "lifeday") && sti(chr.lifeday) == 0)
...
float FindShipSpeed(aref refCharacter)
{
	...
	if (!CheckAttribute(refCharacter, "Cargo"))
...
float SpeedBySkill(aref refCharacter)
{
	...
	if (CheckAttribute(&RealShips[sti(refCharacter.Ship.Type)], "Tuning.HullSpecial"))
		fSpeedPerk *= 0.75;
	if (CheckAttribute(&RealShips[sti(refCharacter.Ship.Type)], "Tuning.CuBot"))
		fSpeedPerk *= 1.05;
...
float TurnBySkill(aref refCharacter)
{
	...
	if (CheckAttribute(&RealShips[sti(refCharacter.Ship.Type)], "Tuning.SailsSpecial"))
		fSpeedPerk *= 0.8;
	if (CheckAttribute(&RealShips[sti(refCharacter.Ship.Type)], "Tuning.HullSpecial"))
		fSpeedPerk *= 0.75;
	if (CheckAttribute(&RealShips[sti(refCharacter.Ship.Type)], "Tuning.CuBot"))
...
{ // от загрузки трюма
	if (!CheckAttribute(_refCharacter, "Cargo"))
...
{ // от повреждения корпуса
	...
	fSpeedFromHp = 0.8 + fSpeedFromHp * 0.3;
	if (fSpeedFromHp > 1.0)
		fSpeedFromHp = 1.0; // некий запас перед снижением скорости
	return fSpeedFromHp;
}
...
{ // от команды
	...
	float fTRFromPeople;
	if (iArcadeSails == 1)
		fTRFromPeople = 0.3 + ((GetCrewExp(_refCharacter, "Sailors") * fCrewCur) / (fCrewOpt * GetCrewExpRate())) * 0.7;
	else
		fTRFromPeople = 0.05 + ((GetCrewExp(_refCharacter, "Sailors") * fCrewCur) / (fCrewOpt * GetCrewExpRate())) * 0.95;
	if (fTRFromPeople > 1)
		fTRFromPeople = 1;
	return fTRFromPeople;
}
...
float Cannon_GetRechargeTimeValue(aref aCharacter)
{
	...
	float fMultiply = 1.0;
	if (CheckCharacterPerk(aCharacter, "FastReload"))
		fMultiply = 0.9;
	else
		fMultiply = 1.0 if (IsCharacterPerkOn(aCharacter, "ImmediateReload")) fMultiply *= 0.5;
	else fMultiply = fMultiply - 0.0;
	if (CheckAttribute(&RealShips[sti(aCharacter.Ship.Type)], "Tuning.CannonsSpecial"))
		fMultiply *= 1.2;
...
void SetShipyardStore(ref NPChar)
{
	...
	{
		j6 = i;
		if (sti(ShipsTypes[storeArray[i]].Class) == 6)
			break; //ищем начало 6 класса
	}
	...
	{
		j5 = i;
		if (sti(ShipsTypes[storeArray[i]].Class) == 5)
			break; //ищем начало 5 класса
	}
	if (sti(PChar.rank) >= 1)
	{
	...
	{
		j4 = i;
		if (sti(ShipsTypes[storeArray[i]].Class) == 4)
			break; //ищем начало 4 класса
	}
	if (sti(PChar.rank) >= 5)
	{
	...
	{
		j3 = i;
		if (sti(ShipsTypes[storeArray[i]].Class) == 3)
			break; //ищем начало 3 класса
	}
	...
	{
		j2 = i;
		if (sti(ShipsTypes[storeArray[i]].Class) == 2)
			break; //ищем начало 2 класса
	}
	...
	{
		j1 = i;
		if (sti(ShipsTypes[storeArray[i]].Class) == 1)
			break; //ищем начало 1 класса
	}
...
int GetPortManPriceExt(ref NPChar, ref chref)
{
	...
	if (sti(NPChar.nation) == GetBaseHeroNation())
		fRelation = 0.5; // если нация родная- снижаем цену в два раза
	...
	if (CheckAttribute(rRealShip, "CannonsQuantity"))
	{
		if (sti(rRealShip.CannonsQuantity) > 0)
		{
			if (CheckAttribute(rChar, "Ship.Cannons.Type"))
			{
				...
				if (CheckAttribute(rCannon, "Cost"))
					cannon_price = sti(rCannon.Cost);
				else
				{
					cannon_price = 0;
					if (CurrentInterface != INTERFACE_SHIPYARD1 && CurrentInterface != INTERFACE_SHIPYARD2)
						log_info("ОШИБКА 1500-404 - сделайте скрин и отправьте разработчикам краткое описание ситуации и сохранение");
				}
...
int GenerateShipTop(int iBaseType, bool isLock, ref chr)
{
	...
	{
	case 7:
		rRealShip.HullArmor = 8;
		break;
	case 6:
		rRealShip.HullArmor = 16;
		break;
	case 5:
		rRealShip.HullArmor = 20;
		break;
	case 4:
		rRealShip.HullArmor = 24;
		break;
	case 3:
		rRealShip.HullArmor = 28;
		break;
	case 2:
		rRealShip.HullArmor = 36;
		break;
	case 1:
		rRealShip.HullArmor = 46;
		break;
	}
