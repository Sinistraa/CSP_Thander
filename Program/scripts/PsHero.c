		...
		if (LAi_IsDead(chr))
		{
			if (CheckAttribute(chr, "PGGAi.Rebirth") && sti(chr.PGGAi.Rebirth))
			{
				...
				if (chr.sex != "woman")
					...
					chr.Dialog.FileName = "pgg_dialog_town.c";
...
*/
	...
	if (!CheckAttribute(chr, "KilledTimes"))
		chr.KilledTimes = 1;
	else
		chr.KilledTimes = sti(chr.KilledTimes) + 1;
...
void PGG_SetUpForTask(ref chr)
{
	...
	if (sTask == PGG_TASK_WORKONSTORE || sTask == PGG_TASK_SAILTOISLAND || sTask == PGG_TASK_WORKCONVOY)
	{
		...
		{ //Не пускаем в море слишком много ПГГ
			...
			{
				int aftersailTasks = sti(chr.PGGAi.AfterSail.TaskCounter) + 1 chr.PGGAi.AfterSail.TaskCounter = aftersailTasks;
...
void PGG_UpdateEquip(ref chr)
{
	...
	if (bShipNotUsed || bOk)
	{
		...
		if (n != 0)
		{
			...
			if (!bShipNotUsed)
			{
				if (MOD_SKILL_ENEMY_RATE * sti(chr.rank) > rand(10 * MOD_SKILL_ENEMY_RATE))
					bOk = true;
			}
	...
	if (rand(100) < 5)
	{
		...
		if (rand(1000) < MOD_SKILL_ENEMY_RATE * 100)
		{
			...
			if (GetCharacterItem(chr, "potion1") > 30)
				chr.Items.potion1 = 30;
			TakeNItems(chr, "potion2", rand(makeint(sti(chr.rank) / 5 + 0.5))); // даю меньше
			if (GetCharacterItem(chr, "potion2") > 10)
				chr.Items.potion2 = 10;
		}
...
bool PGG_CheckForQuestOffer(ref chr)
{
	...
	if (rand(2) == 1 || CheckAttribute(chr, "PGGWhisperQuestStart"))
...
void PGG_Q1RemoveShip(string qName)
{
	...
	if (CheckAttribute(PChar, "GenQuest.PGG_Quest.Stage") && sti(PChar.GenQuest.PGG_Quest.Stage) < 2 && sti(PChar.GenQuest.PGG_Quest.Stage) >= -1)
	{
		...
		if (chr.sex != "woman")
			...
			CloseQuestHeader("Gen_PGGQuest1woman");
	}
	DeleteAttribute(chr, "AlwaysFriend");
}
...
void PGG_Q1PGGDead(string qName)
{
	...
	if (sti(PChar.GenQuest.PGG_Quest.Template))
	{
		if (sti(PChar.GenQuest.PGG_Quest.Stage) < 3)
		{
			...
			if (CheckAttribute(chr, "PGGAi.ActiveQuest"))
			{
				if (chr.sex != "woman")
					...
					AddQuestRecord("Gen_PGGQuest1woman", "q1_PGGDeadNoShips");
			}
			else
			{
				if (chr.sex != "woman")
					...
					AddQuestRecord("Gen_PGGQuest1woman", "q1_PGGDeadNoShips_1");
			}
		...
		else
		{
			if (chr.sex != "woman")
				...
				AddQuestRecord("Gen_PGGQuest1woman", "q1_PGGDeadShips");
		}
		if (chr.sex != "woman")
			AddQuestUserData("Gen_PGGQuest1", "sPsName", GetFullName(chr));
		else
			AddQuestUserData("Gen_PGGQuest1woman", "sPsName", GetFullName(chr));
	...
	else
	{
		...
		if (chr.sex != "woman")
			...
			AddQuestRecord("Gen_PGGQuest1woman", "q1_PGGDead");
		if (chr.sex != "woman")
			AddQuestUserData("Gen_PGGQuest1", "sPsName", GetFullName(chr));
		else
			AddQuestUserData("Gen_PGGQuest1woman", "sPsName", GetFullName(chr));
...
void PGG_Q1AfterShoreFight()
{
	...
	PChar.GenQuest.PGG_Quest.Goods.Taken = 500 + drand(500) + MakeInt(GetSquadronFreeSpace(PChar, sti(PChar.GenQuest.PGG_Quest.Goods)) / (3 + drand(2))) chr = CharacterFromID(PChar.GenQuest.PGG_Quest.PGGid);
	if (!CheckAttribute(PChar, "PGG_EnemyPP"))
	{
...
string SelectRandomPGG(string sex, string animation)
{
	...
	{
		Log_TestInfo("ПГГ не выбран.") return "";
	}
}
