...
void FillAboardCabinBox(ref _location, ref _npchar)
{
	...
	if (ok) // не квестовый
	{
		...
		if (rand(50) == 0)
		{
			...
			{
			case 0:
				_location.box1.items.Chest_ammo = 1;
				break;
			case 1:
				_location.box1.items.Chest_treasure = 1;
				break;
			case 2:
				_location.box1.items.Chest_quest = 1;
				break;
			case 3:
				_location.box1.items.Chest_Craftsmans = 1;
				break;
			}
			Log_TestInfo("xzibit");
		}
...
void HolLine2_NullGoods() //для второго квеста голл. линейки.
{
	...
	for (int i = 1; i <= COMPANION_MAX; i++)
	{
		...
		if (chComp != -1)
		{
			...
			if (CheckAttribute(pchar, "questTemp.EbonyQtyElseNeed"))
				sld.Ship.Cargo.Goods.Ebony = 0;
			if (CheckAttribute(pchar, "questTemp.CoffeeQtyElseNeed"))
				sld.Ship.Cargo.Goods.Coffee = 0;
			if (CheckAttribute(pchar, "questTemp.MahoganyQtyElseNeed"))
				sld.Ship.Cargo.Goods.Mahogany = 0;
			if (CheckAttribute(pchar, "questTemp.SandalQtyElseNeed"))
				sld.Ship.Cargo.Goods.Sandal = 0;
		}
	}
}
...
void IsabellaCheckBreachState()
{
	...
	iTemp = Statistic_AddValue(PChar, "pir_KillFort", 0) +
			...
			Statistic_AddValue(PChar, "spa_KillFort", 0) +
			Statistic_AddValue(PChar, "hol_KillFort", 0);
	if (iTemp > sti(pchar.RomanticQuest.State.KillFort))
		pchar.RomanticQuest.Cheking.KillFort = iTemp - sti(pchar.RomanticQuest.State.KillFort);
	iTemp = Statistic_AddValue(PChar, "pir_TakeTown", 0) +
			Statistic_AddValue(PChar, "eng_TakeTown", 0) +
			Statistic_AddValue(PChar, "fra_TakeTown", 0) +
			Statistic_AddValue(PChar, "spa_TakeTown", 0) +
			Statistic_AddValue(PChar, "hol_TakeTown", 0);
	if (iTemp > sti(pchar.RomanticQuest.State.TakeTown))
		pchar.RomanticQuest.Cheking.TakeTown = iTemp - sti(pchar.RomanticQuest.State.TakeTown);
	iTemp = Statistic_AddValue(PChar, "pir_GrabbingTown", 0) +
			Statistic_AddValue(PChar, "eng_GrabbingTown", 0) +
			Statistic_AddValue(PChar, "fra_GrabbingTown", 0) +
			Statistic_AddValue(PChar, "spa_GrabbingTown", 0) +
			Statistic_AddValue(PChar, "hol_GrabbingTown", 0);
	if (iTemp > sti(pchar.RomanticQuest.State.GrabbingTown))
		pchar.RomanticQuest.Cheking.GrabbingTown = iTemp - sti(pchar.RomanticQuest.State.GrabbingTown);
	iTemp = Statistic_AddValue(PChar, "pir_AbordShip", 0) +
			Statistic_AddValue(PChar, "eng_AbordShip", 0) +
			Statistic_AddValue(PChar, "fra_AbordShip", 0) +
			Statistic_AddValue(PChar, "spa_AbordShip", 0) +
			Statistic_AddValue(PChar, "hol_AbordShip", 0);
	if (iTemp > sti(pchar.RomanticQuest.State.AbordShip))
		pchar.RomanticQuest.Cheking.AbordShip = iTemp - sti(pchar.RomanticQuest.State.AbordShip);
	iTemp = Statistic_AddValue(PChar, "pir_KillShip", 0) +
			Statistic_AddValue(PChar, "eng_KillShip", 0) +
			Statistic_AddValue(PChar, "fra_KillShip", 0) +
			Statistic_AddValue(PChar, "spa_KillShip", 0) +
			Statistic_AddValue(PChar, "hol_KillShip", 0);
	if (iTemp > sti(pchar.RomanticQuest.State.KillShip))
		pchar.RomanticQuest.Cheking.KillShip = iTemp - sti(pchar.RomanticQuest.State.KillShip);
	return;
}
...
void SetSkeletonsToLocation(aref _location)
{
	...
	if (sti(pchar.rank) > 6)
	{
		if (sti(pchar.rank) > 20)
			...
			iRank = sti(pchar.rank) + sti(MOD_SKILL_ENEMY_RATE / 2);
	}
	else
	...
	if (CheckAttribute(_location, "DestroyGhost"))
	{
		...
		if (!CheckAttribute(pchar, "Dovahkiin") && rand(3) == 0)
			LAi_group_SetCheck(LAI_GROUP_MONSTERS, "Dovahkiin");
...
void SetReefSkeletonsToLocation(aref _location, string loc)
{
	...
	{
		if (sti(pchar.rank) > 20)
			iRank = sti(pchar.rank) + 5 + MOD_SKILL_ENEMY_RATE;
		else
			iRank = sti(pchar.rank) + 5 + sti(MOD_SKILL_ENEMY_RATE / 2);
	}
	else
	...
	for (int i = 0; i < num; i++)
	{
		...
		if (i == 0)
		{
			if (GetDataDay() == 3 && GetDataMonth() == 3 && GetTime() >= 3.0 && GetTime() < 4.0 && loc == "MountainPath" && !CheckAttribute(pchar, "salasarmet") && CheckAttribute(pchar, "SalasarEventKnow"))
			{
				...
				if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
					sld = GetCharacter(NPC_GenerateCharacter("salasar", "salasar", "skeleton", "spy", iRank, PIRATE, 1, true)); // LEO: Превозмогаторам - страдать 01.12.2021
				else
		...
		if (loc == "MountainPath")
		{
			if (locator == "rock" || locator == "wall" || locator == "top")
				locator = "skeleton5"; //нет скелетам на скалах и на крыше
		...
		if (loc == "DeckWithReefs")
			ChangeCharacterAddressGroup(sld, _location.id, "monsters", locator);
	}
...
void QuestCheckTakeBoxes(ref itemsRef)
{
	...
	if (CheckAttribute(itemsRef, "Treasure"))
	{
		...
		if (sti(pchar.questTemp.treasurecount) >= 5)
			UnlockAchievement("AchTreasure", 1);
		if (sti(pchar.questTemp.treasurecount) >= 15)
			UnlockAchievement("AchTreasure", 2);
		if (sti(pchar.questTemp.treasurecount) >= 30)
			UnlockAchievement("AchTreasure", 3);
