...
{
	if (bHigherSelfRate)
		return 30 - GetCharacterSPECIALSimple(_chref, SPECIAL_I);
	else
...
int ApplayNavyPenaltyToSkill(ref _refCharacter, string skillName, int sumSkill)
{
	if (IsCompanion(_refCharacter) && GetRemovable(_refCharacter)) //пусть будет для компаньонов тоже sti(_refCharacter.index) == GetMainCharacterIndex()) // только для главного, чтоб не тормозить всю игру
	{
		...
		if (CheckAttribute(_refCharacter, "chr_ai.Trauma"))
			sumSkill = sumSkill - 20; //штраф от травмы - Gregg
		if (CheckAttribute(_refCharacter, "chr_ai.HeavyTrauma"))
			sumSkill = sumSkill - 30; //штраф от тяжелой травмы - Gregg
	}
	else
	{
		if (CheckAttribute(_refCharacter, "chr_ai.Trauma"))
			sumSkill = sumSkill - makeint(sumSkill * 0.4); //штраф от травмы - Gregg
	}
	return sumSkill;
}
...
void ApplayNewSkill(ref _chref, string _skill, int _addValue)
{
	if (_chref == pchar)
	{
		...
		if (sti(pchar.questTemp.bloodingcount) >= 50)
			UnlockAchievement("AchBlood", 1);
		if (sti(pchar.questTemp.bloodingcount) >= 100)
			UnlockAchievement("AchBlood", 2);
		if (sti(pchar.questTemp.bloodingcount) >= 200)
			UnlockAchievement("AchBlood", 3);
		if (sti(pchar.questTemp.swiftcount) >= 50)
			UnlockAchievement("AchStanS", 1);
		if (sti(pchar.questTemp.swiftcount) >= 150)
			UnlockAchievement("AchStanS", 2);
		if (sti(pchar.questTemp.swiftcount) >= 300)
			UnlockAchievement("AchStanS", 3);
		if (sti(pchar.questTemp.stuncount) >= 50)
			UnlockAchievement("AchStanH", 1);
		if (sti(pchar.questTemp.stuncount) >= 150)
			UnlockAchievement("AchStanH", 2);
		if (sti(pchar.questTemp.stuncount) >= 300)
			UnlockAchievement("AchStanH", 3);
		if (sti(pchar.questTemp.traumacount) >= 50)
			UnlockAchievement("AchTravma", 1);
		if (sti(pchar.questTemp.traumacount) >= 150)
			UnlockAchievement("AchTravma", 2);
		if (sti(pchar.questTemp.traumacount) >= 300)
			UnlockAchievement("AchTravma", 3);
		if (sti(pchar.rank) == 10)
			UnlockAchievement("rank", 1); // 10 уровень
		if (sti(pchar.rank) == 25)
			UnlockAchievement("rank", 2); // 25 уровень
		if (sti(pchar.rank) == 40)
			UnlockAchievement("rank", 3); // 40 уровень
	}
	...
	{
		if (_chref.model.animation == "man")
			_chref.model.animation = "man_fast";
		// if (_chref.model.animation == "man_A") _chref.model.animation = "man_A_fast";
		if (_chref.model.animation == "Jessika")
			_chref.model.animation = "Jessika_fast";
		if (_chref.model.animation == "YokoDias")
			_chref.model.animation = "YokoDias_fast";
		if (_chref.model.animation == "Milenace")
			_chref.model.animation = "Milenace_fast";
		if (_chref.model.animation == "Danielle")
			_chref.model.animation = "Danielle_fast";
		if (_chref.model.animation == "Giant")
			_chref.model.animation = "Giant_fast";
		if (_chref.model.animation == "Moscovit")
			_chref.model.animation = "Moscovit_fast";
		if (_chref.model.animation == "Chani")
			_chref.model.animation = "Chani_fast";
		if (_chref.model.animation == "skeleton")
			_chref.model.animation = "skeleton_fast";
	}
...
void RestoreModelsBeforeDrugs()
{
	...
	DeleteAttribute(pchar, "HighOnDrugs")
		DeleteAttribute(pchar, "GenQuest.CamShuttle") for (int i = 0; i < MAX_CHARACTERS; i++)
	{
		ref chr;
...
int GetShipClassNavySkill(int shipClass)
{
	...
	case 1:
		needSkill = 100;
		break; // LEO: Немного апнули требования навигации 01.12.2021
	case 2:
		needSkill = 85;
		break;
	case 3:
	...
	case 6:
		if (MOD_SKILL_ENEMY_RATE == 10)
			...
			needSkill = 1;
		break; // LEO: Превозмогаторам страдать 07.12.2021
	case 7:
		needSkill = 0;
		break;
	}
...
int GetSummonSkillFromNameSimple(ref _refCharacter, string skillName)
{
	...
	if (sti(_refCharacter.index) == sti(pchar.index))
	{
		...
		if (iOfficer != -1)
		{
			...
			if (bNoBonusSkillOff)
			{
				if ((iOffSkill - sumSkill) > 25)
					sumSkill = sumSkill + 25;
				else
				{
...
bool CheckForExchangeAllowed(ref _chref)
{
	if (!LAi_group_IsActivePlayerAlarm())
	{
		if (bSeaActive || CheckAttribute(loadedLocation, "fastreload") || CheckAttribute(loadedLocation, "townsack") || _chref.location == pchar.location)
		{
			...
			if (!ok && !CheckAttribute(_chref, "isquest") && !CheckAttribute(_chref, "nonremovable"))
				return true;
		}
	}
	else
...
int GetMaxItemsWeight(ref _chref)
{
	if (CheckAttribute(_chref, "Skill.Fencing"))
	{
		...
		if (IsCharacterPerkOn(_chref, "EnergyPlus"))
			iBonus = 30;
		if (IsCharacterPerkOn(_chref, "EnergyPlusFixed"))
			iBonus = 30;
		if (IsCharacterPerkOn(_chref, "Trader"))
			iBonus += 35;
		// Lugger -->
		...
		if (CheckAttribute(_chref, "chr_ai.bonusweighttube"))
			iBonus += 60 + sti(_chref.rank);
		if (bDifficultyWeight)
			iBonus = iBonus + CHAR_ITEMS_WEIGHT + GetCharacterSPECIALSimple(_chref, SPECIAL_S) * (GetCharacterSPECIALSimple(_chref, SPECIAL_E) + 12);
		else
...
int Statistic_AddValue(ref _chref, string _attrName, int _add) // set and get(_add==0) value
{
	...
	if (_add > 0)
	{
		...
		// Убийства нежити
		if (monstersall >= 30)
			UnlockAchievement("skel_kills", 1);
		if (monstersall >= 75)
			UnlockAchievement("skel_kills", 2);
		if (monstersall >= 150)
			UnlockAchievement("skel_kills", 3);

		// Общие убийства
		if (killsall >= 50)
			UnlockAchievement("kills", 1);
		if (killsall >= 150)
			UnlockAchievement("kills", 2);
		if (killsall >= 500)
			UnlockAchievement("kills", 3);

		// Потопления кораблей
		if (killshipsall >= 10)
			UnlockAchievement("ship_kills", 1);
		if (killshipsall >= 35)
			UnlockAchievement("ship_kills", 2);
		if (killshipsall >= 70)
			UnlockAchievement("ship_kills", 3);

		if (!CheckCharacterPerk(pchar, "AboardCollector") && killsall >= 200 && aboardships >= 20)
		{
			SetCharacterPerk(pchar, "AboardCollector");
			log_info("Вы овладели ''Мародёрством''.");
		}
	}
	// <---
...
{
	if (i <= -10)
		return "Награда за голову " + (-i * 10000);
	if (i < 0)
		return "Враждебная";
	if (i <= 20)
		return "Нейтральная";
...
void ChangeAttributesFromCharacter(ref CopyChref, ref PastChref, bool _dialogCopy)
{
	...
	if (CheckAttribute(PastChref, "VISUAL_Ci"))
		CopyChref.heromodel = PastChref.heromodel;
	...
	if (CheckAttribute(PastChref, "PerkValue.HPPlus"))
		CopyChref.chr.chr_ai.hp_max = makeint(PastChref.chr_ai.hp_max) - 80;
	else
		CopyChref.chr.chr_ai.hp_max = makeint(PastChref.chr_ai.hp_max);

	if (CheckAttribute(PastChref, "PerkValue.HPPlus"))
		LAi_SetHP(CopyChref, makeint(PastChref.chr_ai.hp_max) - 80, makeint(PastChref.chr_ai.hp_max) - 80);
	else
...
void SetAllAchievements(int level)
{
	if (level == 0)
	{
		pchar.Statistic.Solder_g = 0 pchar.Statistic.Solder_s = 0 pchar.Statistic.Citizen_g = 0 pchar.Statistic.Citizen_s = 0 pchar.Statistic.Monster_g = 0 pchar.Statistic.Monster_s = 0 pchar.Statistic.Warrior_g = 0 pchar.Statistic.Warrior_s = 0 for (int i = 1; i < 8; i++)
		{
	...
	if (bPortPermission)
		pchar.achievements.taxes = level; // Кол-во оплаченных портовых сборов 175 ---
	if (bBribeSoldiers)
		pchar.achievements.AchVzyatka = level; // Кол-во выданных взяток  175 ---
	...
	if (bFillEncyShips)
		pchar.achievements.AchShipSearch = level; // Изучить все кораблики 100
	...
	if (Pchar.questTemp.CapBloodLine == 1)
		pchar.achievements.CapBladLine = level; // Выполнение линейки Блада 100
	if (startherotype == 2)
		pchar.achievements.WhisperLine = level; // Выполнение линейки Виспер 100
...
void initNewMainCharacter()
{
	...
	{
		if (sti(ch.nation) != SPAIN)
			ch.HeroParam.ToCityId = FindAlliedColonyForNation(sti(ch.nation), true);
		else
			ch.HeroParam.ToCityId = FindAlliedColonyForNationExceptColony("Caracas"); //Каракас не берем, там оборотень
	}
	else
	{
	...
	if (sti(ch.nation) != PIRATE || startHeroType == 1)
