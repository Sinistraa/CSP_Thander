...
       
	...
	case "MushketHotkey":
		if (!CheckAttribute(pchar, "mushket"))
			break;
		if (!CheckCharacterItem(pchar, pchar.mushket))
			break;
		if (LAi_CheckFightMode(pchar))
		{
			if (!CheckAttribute(pchar, "mushket.timer"))
				pchar.mushket.timer = false;
			if (pchar.mushket.timer)
				break;
	...
	case "PusherHotkey":
		if (!CheckCharacterPerk(pchar, "Buldozer"))
			break;
...
void GameOver(string sName)
{
	...
	case "land":
		if (mc.sex != "woman")
			StartPictureAsVideo("loading\DeathMan_" + rand(1) + ".tga", 4);
		else
			StartPictureAsVideo("loading\DeathWoman_" + rand(1) + ".tga", 4);
...
bool CheckSaveGameEnabled()
{
	...
	if (bDisableMapEnter && !CheckBattleSeaSaveEnabled())
		TmpBool = false;
			...
			if (CheckAttribute(Locations[idxLoadLoc], "fastreload"))
			{
				if (Pchar.questTemp.CapBloodLine == false)
				{
					...
					if (sNation != "none" && sNation != "4")
					{
						...
						if (HasSubStr(Locations[idxLoadLoc].id, "_Town") && bTmpBool)
							TmpBool = false;
					}
				}
			}
...
{
	if (CheckAttribute(pchar, "fix20210121"))
		return;
