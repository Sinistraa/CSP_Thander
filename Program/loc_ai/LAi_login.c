...
void LAi_CharacterPostLogin(ref location)
{
	...
	if (!actLoadFlag)
	{
		...
		if (CheckAttribute(pchar, "catorga.on"))
			DeleteAttribute(pchar, "catorga");
		if (CheckAttribute(pchar, "catorga"))
			pchar.catorga.on = "1";
		if (HasSubStr(location.id, "Crypt") && !HasSubStr(location.id, "Big"))
			location.id.label = "Crypt";
		...
		for (int i = 1; i <= 20; i++)
		{
			...
			if (CheckAttribute(pchar, offNum))
			{
				...
				if (FindFellowtravellers(pchar, chr) == FELLOWTRAVEL_NO)
				{
					...
					if (CheckOfficersPerk(pchar, "EmergentSurgeon"))
						healing_time -= makeint(healing_time / 10 * 3); //снижения от перков врачей
					else
					{
						if (CheckOfficersPerk(pchar, "Doctor2"))
							healing_time -= makeint(healing_time / 10 * 2);
						else
						{
							if (CheckOfficersPerk(pchar, "Doctor1"))
								healing_time -= makeint(healing_time / 10);
						}
					}
...
void CheckBSFinish()
{
	if (!CheckAttribute(pchar, "BSFinish") && sti(pchar.rank) >= 25)
	{
		...
		{
			if (!CheckAttribute(pchar, "BSInProgress"))
				pchar.BSStart = true;
	...
	if (CheckAttribute(pchar, "WhisperPGG"))
	{
		...
		if (IsCompanion(sld) || IsOfficer(sld))
		{
			if (CheckNPCQuestDate(sld, "AmmoUpdate"))
			{
				...
				if (sti(sld.Items .12_gauge) < maxShells)
					sld.Items .12_gauge = sti(sld.Items .12_gauge) + maxShells / 10;
				if (sti(sld.Items.grapeshot) < maxShells)
					sld.Items.grapeshot = sti(sld.Items.grapeshot) + maxShells / 10;
			}
		}
	}
}
...
void GenerateSpySeeker(ref location)
{
	...
	if (!CheckAttribute(pchar, "SpySeeker.dayrandom"))
		pchar.SpySeeker.dayrandom = 0;
	...
	if (HasSubStr(location.id, "Common") && rand(1000) > 750 && pchar.dayrandom != pchar.SpySeeker.dayrandom && !HasSubStr(location.id, "Crypt") && GetCityNation(location.fastreload) != 4)
	{
		if (bOK || bOK2)
		{
			...
			if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
				ref rChar = GetCharacter(NPC_GenerateCharacter("SpySeeker", "officer_" + (1 + drand(63)), "man", "spy", pchar.rank, GetCityNation(location.fastreload), -1, false)); //LEO: Превозмогаторам страдание 08.12.2021
			else
				rChar = GetCharacter(NPC_GenerateCharacter("SpySeeker", "officer_" + (1 + drand(63)), "man", "man_fast", pchar.rank, GetCityNation(location.fastreload), -1, false));
			rChar.Dialog.FileName = "Common_Seeker.c";
			LAi_SetImmortal(rChar, true);
			rChar.saveItemsForDead = true;
			SetCharacterPerk(rChar, "Fencer");
			if (bHardBoss)
				rChar.AlwaysReload = true; //перезарядка независимо от Дозарядки
			...
			if (iRel == RELATION_ENEMY)
				pchar.SpySeeker = "Enemy";
			else
				pchar.SpySeeker = "Friend";
