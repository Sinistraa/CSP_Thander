	...
	if (chr.chr_ai.tmpl != LAI_TMPL_FIGHT && !LAi_grp_alarmactive)
	{
		...
		else
		{
			...
			if (!LAi_tmpl_SetFight(chr, &Characters[trg]))
			{
				...
				chr.chr_ai.type.checkTarget = rand(4) + 3;
		}
	}
	else
	{
...
void LAi_type_patrol_TestControl(aref chr)
{
	...
	if (GetNationRelation2MainCharacter(sti(chr.nation)) == RELATION_ENEMY)
		iRand = 2;
	else
		iRand = GetRelation2BaseNation(sti(chr.nation)); // 0-  1-  2-
	if (isDay())
	{
		if (rand(iRand) < 2)
		{
		...
		if (CheckNationLicence(sti(chr.nation)))
			iRand = 10;
		else
		{
