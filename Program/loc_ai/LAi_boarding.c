...
void LAi_StartBoarding(int locType, ref echr, bool isMCAttack)
{
	...
	if (CheckOfficersPerk(mchr, "MusketsShoot") && IsFort == false)
	{
		...
		if (ecrewBak > ecrew)
			ecrewBak = ecrew; //aw013
		PlaySound("INTERFACE\_musketshot_" + rand(3) + ".wav");
		if (CheckAttribute(RealShips[sti(echr.Ship.Type)], "Tuning.HighBort"))
			ecrewBak = makeint(ecrewBak / 2);
	...
	if (CheckOfficersPerk(echr, "MusketsShoot") && IsFort == false)
	{
		...
		ecrewBak = makeint(ecrewBak * (2.1 - tmpDefence) / 2.0);
		if (ecrewBak > mcrew)
			ecrewBak = mcrew; //aw013
		PlaySound("INTERFACE\_musketshot_" + rand(3) + ".wav");
		if (CheckAttribute(RealShips[sti(mchr.Ship.Type)], "Tuning.HighBort"))
			ecrewBak = makeint(ecrewBak / 2);
		...
		{ // не грузим офов, если каюту
			...
			for (int i = 1; i <= MAX_NUM_FIGHTERS; i++)
			{
				...
				if (logined > maxLocators)
					break;
				if (CheckAttribute(characters[idx], "AboardRestriction"))
					continue;
...
void LAi_SetBoardingActors(string locID)
{
	...
	if (HasSubStr(boarding_enemy.model, "trader_"))
		boarding_enemy.Ship.Mode = "Trade";
