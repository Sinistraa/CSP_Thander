...
bool LAi_CreateEncounters(ref location)
{
	...
	//Можем ли генерить
	if (CheckAttribute(location, "DisableEncounters"))
		return false;
	if (!CheckAttribute(location, "locators.encdetector") || !CheckNPCQuestDate(location, "Enc_date") || bDisableLandEncounters)
		return false;
	if (CheckAttribute(location, "fastreload") && !HasSubStr(location.id, "exitTown"))
	...
	if (!CheckAttribute(location, "onUninhabitedIsland"))
	{
		...
		if (findsubstr(location.Id, "LaVega", 0) != -1)
			return false;
		if (findsubstr(location.Id, "PuertoPrincipe", 0) != -1)
			return false;
		if (findsubstr(location.Id, "LeFransua", 0) != -1)
			return false;
	}
	...
	case 0:
		...
		if (sti(pchar.rank) > 6 || MOD_SKILL_ENEMY_RATE == 10)
		{
			if (sti(pchar.rank) > 20)
				iRank = sti(pchar.rank) + sti(MOD_SKILL_ENEMY_RATE * 2.5);
			else
				iRank = sti(pchar.rank) + sti(MOD_SKILL_ENEMY_RATE * 1.5);
		}
		else
	...
	case 1:
		...
		if (num < 2)
			return false;
		if (CheckAttribute(pchar, "GenQuest.EncGirl") && pchar.GenQuest.EncGirl != "close")
			return false;
		...
		{ //--------------- замануха в колодец -------------------
			...
			if (sAreal == "SentMartin" || sAreal == "SantaCatalina")
			{
				...
				if (iTemp != -1 && characters[iTemp].location == str)
					return false;
	...
	case 4:
		...
		while (i < num)
		{
			...
			if (model[iMassive] != "")
			{
				sAnime = "man" chr = GetCharacter(NPC_GenerateCharacter("Convict_" + i, model[iMassive], "man", sAnime, iRank, PIRATE, -1, true));
				SetFantomParamFromRank(chr, iRank, true);
				// locator = GetAttributeName(GetAttributeN(grp, i));
				locator = GetAttributeName(GetAttributeN(grp, 1)); // LEO
				ChangeCharacterAddressGroup(chr, location.id, encGroup, locator);
		...
		break
			...
			case 5 : if (isDay() || location.type == "seashore" || location.type == "mayak") return false;
		num = LAi_CalculateRaidersQuantity(GetAttributesNum(grp));
		if (num <= 0)
			num = 2; //если локаторов меньше четырех
		...
		{
			if (i == 0 && rand(6) == 1)
				chr = GetCharacter(NPC_GenerateCharacter(str + i, "Canib_boss", "man", "man", iRank + 12, PIRATE, 1, true)); // с шансом 1 на 7 встреч, первый индиан может быть главой каннибалов
			else
				chr = GetCharacter(NPC_GenerateCharacter(str + i, "Canib_" + (rand(5) + 1), "man", "man", iRank + 6, PIRATE, 1, true));
			SetFantomParamFromRank(chr, iRank, true);
			//Получим локатор для логина
			locator = GetAttributeName(GetAttributeN(grp, 1));
			...
			if (chr.model == "Canib_boss") // Глава каннибалов круче своих по экипировке
			{
				...
				iRand = rand(5);
				if (iRand == 0)
					TakeNItems(chr, "jewelry1", (rand(24) + 1));
				if (iRand == 1)
					TakeNItems(chr, "jewelry2", (rand(24) + 1));
				if (iRand == 2)
					TakeNItems(chr, "jewelry3", (rand(24) + 1));
				if (iRand == 3)
					TakeNItems(chr, "jewelry4", (rand(24) + 1));
				if (iRand == 4)
					TakeNItems(chr, "jewelry5", (rand(24) + 1));
				if (iRand == 5)
				{
					if (rand(1) == 0)
						TakeNItems(chr, "StrangeMushrooms", 1);
					else
						TakeNItems(chr, "StrangePotion", 1);
					chr.SaveItemsForDead = true;
				}

				iRand = rand(25);
				if (iRand == 5)
					TakeNItems(chr, "indian18", (rand(24) + 1));
				if (iRand == 10)
					TakeNItems(chr, "indian19", (rand(24) + 1));
				if (iRand == 15)
					TakeNItems(chr, "indian20", (rand(24) + 1));
				if (iRand == 20)
					TakeNItems(chr, "indian21", (rand(24) + 1));
				if (iRand == 25)
					TakeNItems(chr, "indian22", (rand(24) + 1));
