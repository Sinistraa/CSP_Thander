void ProcessDialogEvent()
{
	...
	case "AntiPGGContra":
		...
		bool canDeal = GetCompanionQuantity(pchar) == 1 && sti(RealShips[sti(pchar.ship.type)].Class) > 2 if (CheckCharacterPerk(pchar, "UnlimitedContra") || canDeal)
		{
			npchar.quest.trade_date = lastspeak_date;
			Pchar.quest.contraband.CurrentPlace = SelectSmugglingLocation();
			Pchar.quest.contraband.City = NPChar.city;
	...
	case "Trade": //торговля с контрами - Gregg
		...
		case 2:
			...
			if (npchar.itemtype == "indian11")
				npchar.itemtype = "indian" + (22 - drand1(2));
			aref ind;
			Items_FindItem(npchar.itemtype, &ind);
			npchar.pricevalue = sti(ind.price) * 10;
			if (CheckAttribute(ind, "groupID") && sti(ind.price) < 100)
				npchar.pricevalue = sti(npchar.pricevalue) * 25;
		...
		case 4:
			...
			Link.l2.go = "Exit";
			break;
			else Dialog.Text = "Увы, сегодня ничего интересного у меня в наличии нет.";
...
//navy <-- */
	...
	case "Travel_pay":
		...
		{
			Dialog.Text = "Похоже, у тебя проблемы с наличностью." Link.l1 = "Жаль, приду позже.";
		}
		Link.l1.go = "Exit";
		break;
