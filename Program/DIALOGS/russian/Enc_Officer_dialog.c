...
void ProcessDialogEvent()
{
	...
	case "w_give_china_sword":
		...
		if (sti(npchar.Skill.Sailing) > 100)
			npchar.Skill.Sailing = 100;
		npchar.Skill.FencingHeavy = sti(npchar.Skill.FencingHeavy) + 20;
		if (sti(npchar.Skill.FencingHeavy) > 100)
			npchar.Skill.FencingHeavy = 100;
	...
	case "hire":
		if (makeint(Pchar.money) >= makeint(Npchar.quest.OfficerPrice))
		{
			...
			if (sti(pchar.questTemp.officercount) >= 3)
				UnlockAchievement("officers", 1);
			if (sti(pchar.questTemp.officercount) >= 6)
				UnlockAchievement("officers", 2);
			if (sti(pchar.questTemp.officercount) >= 10)
				UnlockAchievement("officers", 3);
	...
	case "CheckMushket":
		...
		{
			if (findsubstr(npchar.model.animation, "man2_ab", 0) == -1)
				Npchar.CanTakeMushket = true;
		}
	...
	case "Get_out_fight":
		if (CheckAttribute(Npchar, "HalfImmortal"))
			DeleteAttribute(Npchar, "HalfImmortal");
		CheckForReleaseOfficer(GetCharacterIndex(Npchar.id));
		RemovePassenger(Pchar, Npchar);
	...
	case "TravelToCaiman":
		for (i = 1; i < COMPANION_MAX; i++)
		{
			...
			if (cn_3 == -1)
				continue;
			if (NPChar.name == characters[cn_3].name)
				NPChar.realcompanionidx = &characters[cn_3].index;
		}
	...
	case "TravelToDominica":
		for (i = 1; i < COMPANION_MAX; i++)
		{
			...
			if (cn_4 == -1)
				continue;
			if (NPChar.name == characters[cn_4].name)
				NPChar.realcompanionidx = &characters[cn_4].index;
		}
	...
	case "TravelToTerks":
		for (i = 1; i < COMPANION_MAX; i++)
		{
			...
			if (cn_5 == -1)
				continue;
			if (NPChar.name == characters[cn_5].name)
				NPChar.realcompanionidx = &characters[cn_5].index;
		}
	...
	case "TravelToLaVega":
		for (i = 1; i < COMPANION_MAX; i++)
		{
			...
			if (cn_6 == -1)
				continue;
			if (NPChar.name == characters[cn_6].name)
				NPChar.realcompanionidx = &characters[cn_6].index;
		}
	...
	case "TravelToPuertoPrincipe":
		for (i = 1; i < COMPANION_MAX; i++)
		{
			...
			if (cn_7 == -1)
				continue;
			if (NPChar.name == characters[cn_7].name)
				NPChar.realcompanionidx = &characters[cn_7].index;
		}
	...
	case "TravelToFortOrange":
		for (i = 1; i < COMPANION_MAX; i++)
		{
			...
			if (cn_8 == -1)
				continue;
			if (NPChar.name == characters[cn_8].name)
				NPChar.realcompanionidx = &characters[cn_8].index;
		}
	...
	case "TravelToPirates":
		for (i = 1; i < COMPANION_MAX; i++)
		{
			...
			if (cn_9 == -1)
				continue;
			if (NPChar.name == characters[cn_9].name)
				NPChar.realcompanionidx = &characters[cn_9].index;
		}
	...
	case "TravelToLeFransua":
		for (i = 1; i < COMPANION_MAX; i++)
		{
			...
			if (cn_10 == -1)
				continue;
			if (NPChar.name == characters[cn_10].name)
				NPChar.realcompanionidx = &characters[cn_10].index;
		}
		characters[sti(NPChar.realcompanionidx)].CompanionTravel.ToColonyID = "LeFransua";
			...
			if (nFind == 0)
			{
				...
				for (i = 1; i < COMPANION_MAX; i++)
				{
					...
					if (cn == -1)
						continue;
					if (NPChar.name == characters[cn].name)
						NPChar.realcompanionidx = &characters[cn].index;
				}
