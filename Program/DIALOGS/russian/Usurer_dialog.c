...
void ProcessDialogEvent()
{
	...
	case "First time":
		if (!CheckAttribute(npchar, "CapitolMoney"))
			npchar.CapitolMoney = 500000 + drand(30) * 50000;
	...
	case "Loan_GiveWork":
		...
		if (npchar.quest.trade_date != lastspeak_date || bBettaTestMode)
		{
			...
			case 0: // найти должника
				if (CheckAttribute(pchar, "GenQuest.Loan.FindCitizen"))
					break;
			...
			case 1: // доставить сундуки
				if (CheckAttribute(pchar, "GenQuest.LoanChest.TakeChest"))
					break;
	...
	for (n = 2; n < MAX_CHARACTERS; n++)
	{
		...
		if (sTemp == "trader")
		{
			if (NPChar.city == ch.city)
				continue;
			if (ch.location == "none" || ch.City == "Caiman")
				continue;
			storeArray[howStore] = n;
			howStore++;
			continue;
...
void CheckForAchievement()
{
	...
	if (deposits >= 50000000)
		UnlockAchievement("bank_money", 3);
}
