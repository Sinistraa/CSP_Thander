...
void ProcessDialogEvent()
{
	...
	if (!CheckAttribute(npchar, "PatentPrice"))
	{
		if (IsCharacterPerkOn(pchar, "Adventurer"))
			npchar.PatentPrice = (6000 + rand(4) * 500);
		else
	...
	case "PGG_peace_made":
		...
		dialog.Text = "Считайте, что всё уже улажено. Любая охота, которую мог" + NPCharSexPhrase(sld, "", "ла") + " вести на вас " + GetFullName(sld) + " должна в скором времени прекратиться." Link.l1 = "Буду надеяться, что вы правы.";
		Link.l1.go = "exit";
		break;
	...
	case "NationLicenceType2":
		if (IsCharacterPerkOn(pchar, "Agent"))
			iSumm = sti(npchar.LicenceType) * MOD_SKILL_ENEMY_RATE * 500 + 200000 + (sti(PChar.rank) * 1000) - (GetSummonSkillFromName(PChar, SKILL_COMMERCE) + GetSummonSkillFromName(PChar, SKILL_LEADERSHIP)) * 1000;
		else
	...
	case "patent_2":
		...
		if (IsCharacterPerkOn(pchar, "Agent"))
			pchar.PatentPrice = 100000 * MOD_SKILL_ENEMY_RATE - (GetSummonSkillFromName(PChar, SKILL_LEADERSHIP)) * 5000 + 35000 + (sti(PChar.rank) * 5000);
		else
	...
	case "Contraband":
		if (IsCharacterPerkOn(pchar, "Agent"))
			...
			Pchar.questTemp.Relations.sum = makeint(0.1 * stf(Pchar.rank) / stf(Pchar.reputation) * DIPLOMAT_SUM);
	...
	case "SeekHVIC_res":
		...
		if (findsubstr(attrLoc, "оливер", 0) != -1 && findsubstr(attrLoc, "траст", 0) != -1)
		{
			...
			if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
				sld = GetCharacter(NPC_GenerateCharacter("PoorKillSponsor", "smuggler_boss", "man", "spy", 30, PIRATE, -1, true)); // LEO: Страдать превозмогаторам 07.12.2021
			else
				sld = GetCharacter(NPC_GenerateCharacter("PoorKillSponsor", "smuggler_boss", "man", "man_fast", 30, PIRATE, -1, true));
			sld.name = "Оливер";
			sld.lastname = "Траст";
