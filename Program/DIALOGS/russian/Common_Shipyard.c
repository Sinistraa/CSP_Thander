...
void ProcessDialogEvent()
{
	...
	if (HasSubStr(attrLoc, "shiporderend2_"))
	{
		...
		{
		case 6:
			RealShips[sti(compref.Ship.Type)].HullArmor = 12 + (rand(1) * hullarmor);
			break;
		case 5:
			RealShips[sti(compref.Ship.Type)].HullArmor = 16 + (rand(1) * hullarmor);
			break;
		case 4:
			RealShips[sti(compref.Ship.Type)].HullArmor = 20 + (rand(2) * hullarmor);
			break;
		case 3:
			RealShips[sti(compref.Ship.Type)].HullArmor = 24 + (rand(2) * hullarmor);
			break;
		case 2:
			RealShips[sti(compref.Ship.Type)].HullArmor = 32 + (rand(2) * hullarmor);
			break;
		case 1:
			RealShips[sti(compref.Ship.Type)].HullArmor = 42 + (rand(2) * hullarmor);
			break;
		}

		NPChar.reputation = sti(NPChar.reputation) + (8 - iClass); //минимальная репа ~12. две единицы репы ~ одна единица навыка корабела
		if (sti(NPChar.reputation) > 100)
			NPChar.reputation = 100;
		WaitDate("", 0, 0, 0, 0, 20);
		...
		if (sti(pchar.questTemp.shipordercount) >= 5)
			UnlockAchievement("AchShipOrder", 1);
		if (sti(pchar.questTemp.shipordercount) >= 10)
			UnlockAchievement("AchShipOrder", 2);
		if (sti(pchar.questTemp.shipordercount) >= 20)
			UnlockAchievement("AchShipOrder", 3);
		if (sti(pchar.ShipInOrder) < 1)
			CloseQuestHeader("ShipOrder");
	...
	case "ShipyardsMapOk_2":
		...
		else
		{
			...
			case 6:
				...
				Log_info("Вы получили сундук ремесленника.") break;
			}
		}
		break;
...
string checkOrderMatherial(ref NPChar)
{
	...
	for (int k = 0; k < 8; k++)
	{
		...
		if (amount < 0)
			amount = amount + sti(NPChar.questtemp.(sGood1));
		else
		...
		amount = sti(NPChar.questtemp.(sItem2));
		if (amount > 0)
			sLeft += LanguageConvertString(idLngFile, Items[FindItem(sItem2)].name) + " - " + amount + "шт., ";
	}
	sItem2 = "chest";
	amount = GetCharacterItem(pchar, sItem2) - sti(NPChar.questtemp.(sItem2));
	if (amount < 0)
	...
	amount = sti(NPChar.questtemp.(sItem2));
	if (amount > 0)
		sLeft += "Сундуков с монетами - " + amount + "шт.";
