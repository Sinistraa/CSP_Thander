...
void ProcessDialogEvent()
{
	...
	case "First time":
		...
		if (GetNationRelation2MainCharacter(sti(NPChar.nation)) == RELATION_ENEMY && sti(NPChar.nation) != PIRATE)
		{
			...
			if (Pchar.location == "Santiago_Incquisitio")
	...
	case "NotPegYou_2":
		...
		if (sti(pchar.questTemp.bribescount) >= 10)
			UnlockAchievement("AchVzyatka", 1);
		if (sti(pchar.questTemp.bribescount) >= 25)
			UnlockAchievement("AchVzyatka", 2);
		if (sti(pchar.questTemp.bribescount) >= 50)
			UnlockAchievement("AchVzyatka", 3);
	...
	case "SoldierNotBlade":
		...
		NextDiag.TempNode = "First Time";
		if (NPChar.chr_ai.type == "guardian")
