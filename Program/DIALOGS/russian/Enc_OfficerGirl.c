...
void ProcessDialogEvent()
{
	...
	case "Helen_meet":
		...
		if (sti(pchar.reputation) >= 70)
		{
			if (CheckAttribute(npchar, "MetWhisper"))
			{
				...
				dialog.Text = "Приветствую вас, капитан. ";
	...
	case "Angellica_2":
		if (!CheckAttribute(pchar, "AngellicaDuelInfo"))
		{
			dialog.Text = "Победи меня на дуэли и за скромное вознаграждение я присоединюсь к тебе.";
			pchar.AngellicaDuelInfo = true;
		}
		else
			dialog.Text = "Ну что, решил" + GetSexPhrase("ся", "ась") + "?";
