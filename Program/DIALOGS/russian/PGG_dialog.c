...
void ProcessDialogEvent()
{
	...
	case "officer":
		...
		Dialog.Text = NPCharRepPhrase(NPChar, RandSwear() + RandPhraseSimple("Это - то, что нужно! Наши клинки напьются вражеской крови! Одна проблемка, мне нужно " + NPChar.Quest.Officer.Price + " пиастров, чтобы расплатиться с долгами.", "Ты выглядишь настоящ" + GetSexPhrase("им морским волком", "ей морской волчицей") + ". И если ты готов" + GetSexPhrase("", "а") + " заплатить " + NPChar.Quest.Officer.Price + " пиастров, то, пожалуй, мы сработаемся."),
	...
	case "companion":
		...
		{
			Dialog.Text = NPCharRepPhrase(NPChar, RandSwear() + RandPhraseSimple("Ты мне нравишься! По рукам! Я уже слышу звон пиастров в наших карманах!", "А ты " + GetSexPhrase("парень", "девка") + " не промах... Думаю, мы сработаемся!"),
