...
void ProcessDialogEvent()
{
	...
	case "First time":
		if (NPChar.id == "GWIK_char")
			...
			dialog.text = "Приветствую вас. Я - " + GetFullName(NPChar) + ", представитель офиса Голландской Вест-Индской Торговой Компании на острове Сан Мартин. Представьтесь, пожалуйста.";
	...
	case "Meet_2":
		if (!CheckAttribute(npchar, "met"))
			dialog.text = "Приятно познакомиться, капитан. По какому поводу зашли?";
		else
	...
	case "trade_info_2":
		for (int i = 0; i < MAX_COLONIES; i++)
		{
			...
			if (rcolony.id != "Caiman" && rcolony.id != "Panama" && rcolony.nation != "none")
