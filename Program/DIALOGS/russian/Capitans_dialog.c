...
void ProcessDialogEvent()
{
	...
	if (npchar.id == "BlackBeardNPC" && CheckAttribute(npchar, "met") && Dialog.CurrentNode == "First time")
		Dialog.CurrentNode = "Titch_7";
	...
	case "Titch_check":
		if (npchar.questionsstate == 1)
		{
			...
			for (int x = 0; x <= 5; x++)
			{
				if (coolstory1[x] == GetStrSmallRegister(dialogEditStrings[1]))
					check1 = true;
			}
			if (check1)
			{
		...
		if (npchar.questionsstate == 2)
		{
			...
			for (int y = 0; y <= 3; y++)
			{
				if (coolstory2[y] == GetStrSmallRegister(dialogEditStrings[1]))
					check2 = true;
			}
			if (check2)
			{
