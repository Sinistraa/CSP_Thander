...
void ProcessDialogEvent()
{
	...
	case "FightInArena_7":
		...
		case 1:
			...
			if (sti(pchar.questTemp.tournamentcount) >= 1)
				UnlockAchievement("AchTurnir", 1);
			if (sti(pchar.questTemp.tournamentcount) >= 3)
				UnlockAchievement("AchTurnir", 2);
			if (sti(pchar.questTemp.tournamentcount) >= 7)
				UnlockAchievement("AchTurnir", 3);
