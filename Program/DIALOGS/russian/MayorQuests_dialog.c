void ProcessDialogEvent()
{
	...
	case "SeekSpy_street":
		...
		dialog.Text = NPCharRepPhrase(npchar,
									  PCharRepPhrase(LinkRandPhrase("А, это ты, " + GetFullName(Pchar) + ", " + GetSexPhrase("старый пройдоха", "отчаянная деваха") + "", "" + GetSexPhrase("Рад тебя видеть, старина", "Рад тебя видеть, красавица") + "" + Pchar.name, "О, да это же сам" + GetSexPhrase("", "а") + " капитан " + GetFullName(Pchar)) + LinkRandPhrase(". Думал, тебя уже нет на этом свете!", ". И, наверное, как всегда, " + GetSexPhrase("пьян", "выпивши") + ".", ". И, как вижу, еще на свободе!") + LinkRandPhrase(" Ну, говори, чего тебе нужно?", " Чего тебе на этот раз?", " Опять тревожишь меня по пустякам?"),
													 LinkRandPhrase(TimeGreeting() + ", капитан ", "Здравствуйте " + GetAddress_Form(NPChar) + " ", "Приветствую вас, капитан ") + GetFullName(Pchar) + LinkRandPhrase(". Что может быть нужно " + GetSexPhrase("такому почтенному человеку", "бравой девушке") + ", как вы, от меня?", ". Зачем вы пришли?", ". Что на этот раз вы хотите узнать?")),

									  PCharRepPhrase(LinkRandPhrase("Здравствуйте капитан ", "Приветствую, ах, это вы " + GetAddress_Form(NPChar) + " ", "А, капитан ") + GetFullName(Pchar) + LinkRandPhrase(" я думал" + NPCharSexPhrase(NPChar, " ", "а ") + "мы больше с вами не встретимся,", " не скажу, что рад" + NPCharSexPhrase(NPChar, " ", "а ") + "вас видеть, но", " вижу, вы еще живы, как это ни прискорбно. А") + LinkRandPhrase(" что вам нужно?", " зачем вы пришли?", " чем могу быть " + NPCharSexPhrase(NPChar, "полезен?", "полезна?")),
	...
	case "SitySpy_5":
		...
		AddSimpleRumour(RandPhraseSimple("Вы знаете, губернатор " + sTemp + " поручил " + GetSexPhrase("некоему капитану", "некой девушке-капитану") + " " + GetMainCharacterNameGen() + " найти в городе вражеского лазутчика. И вы знаете, " + GetSexPhrase("он его нашел", "она его нашла") + " прямо на улицах!",
										 "Наш губернатор зал задание капитану " + GetMainCharacterNameGen() + " разыскать в городе вражеского агента. И " + GetSexPhrase("этот капитан нашел", "эта девушка нашла") + " лазутчика среди горожан!"),
