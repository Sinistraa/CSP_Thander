...
void ProcessCommonDialogEvent(ref NPChar, aref Link, aref NextDiag)
{
	if (sti(Pchar.Ship.Type) != SHIP_NOTUSED)
	{
		...
		int cannonMaxQ = 0;
		if (CheckAttribute(&RealShips[sti(Pchar.Ship.Type)], "CannonsQuantityMax"))
			cannonMaxQ = sti(RealShips[sti(Pchar.Ship.Type)].CannonsQuantityMax);
...
*/
		...
		if (CheckAttribute(&RealShips[sti(Pchar.Ship.Type)], "QuestShip"))
			fQuestShip = 1.2;
		...
		if (quantityMatherial2 < 1)
			quantityMatherial2 = 1;
		...
		if (quantityMatherial3 < 1)
			quantityMatherial3 = 1;
		...
		if (capacityMatherial3 < 1)
			capacityMatherial3 = 1;
		...
		if (SpeedRateMatherial3 < 1)
			SpeedRateMatherial3 = 1;
		...
		if (MastMultiplierMatherial3 < 1)
			MastMultiplierMatherial3 = 1;
		...
		if (TurnRateMatherial3 < 1)
			TurnRateMatherial3 = 1;
		...
		if (WindAgainstMatherial3 < 1)
			WindAgainstMatherial3 = 1;
	...
	case "ship_tunning_cannon_all_start":
		...
		if (makeint(Pchar.money) >= amount)
		{
			...
			AddQuestUserData("ShipTuning", "sText", "За свою работу по увеличению калибра и количества орудий на корабле " + XI_ConvertString(RealShips[sti(Pchar.Ship.Type)].BaseName) + " мастер-корабел затребовал: черного дерева - " + NPChar.Tuning.Matherial1 + ", сокровищ - " + NPChar.Tuning.Matherial2 + ", сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". В качестве задатка было уплачено " + NPChar.Tuning.Money + " золотых. Видимо, старик без ума от драгоценностей. Что ж - у каждого свои слабости...");
		}
		else
		{
	...
	case "ship_c_quantity_start":
		...
		if (makeint(Pchar.money) >= amount)
		{
			...
			ReOpenQuestHeader("ShipTuning");
			AddQuestRecord("ShipTuning", "t1");
			AddQuestUserData("ShipTuning", "sText", "За свою работу по увеличению количества орудий на корабле " + XI_ConvertString(RealShips[sti(Pchar.Ship.Type)].BaseName) + " мастер-корабел требует: черного дерева - " + NPChar.Tuning.Matherial1 + ", сокровищ - " + NPChar.Tuning.Matherial2 + ", сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". В качестве задатка было уплачено " + NPChar.Tuning.Money + " золотых. Видимо, старик без ума от драгоценностей. Что ж - у каждого свои слабости...");
		}
		else
		{
			NextDiag.TempNode = "ship_tunning_not_now";
	...
	case "ship_tunning_cannon_start":
		...
		if (makeint(Pchar.money) >= amount)
		{
			...
			ReOpenQuestHeader("ShipTuning");
			AddQuestRecord("ShipTuning", "t1");
			AddQuestUserData("ShipTuning", "sText", "За свою работу по увеличению калибра орудий на корабле " + XI_ConvertString(RealShips[sti(Pchar.Ship.Type)].BaseName) + " мастер-корабел требует: черного дерева - " + NPChar.Tuning.Matherial1 + ", сокровищ - " + NPChar.Tuning.Matherial2 + ", сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". В качестве задатка было уплачено " + NPChar.Tuning.Money + " золотых. Видимо, старик без ума от драгоценностей. Что ж - у каждого свои слабости...");
		}
		else
		{
			NextDiag.TempNode = "ship_tunning_not_now";
	...
	case "ship_tunning_Capacity_start":
		...
		if (makeint(Pchar.money) >= amount)
		{
			...
			ReOpenQuestHeader("ShipTuning");
			AddQuestRecord("ShipTuning", "t1");
			AddQuestUserData("ShipTuning", "sText", "За свою работу по увеличению дедвейта на корабле " + XI_ConvertString(RealShips[sti(Pchar.Ship.Type)].BaseName) + " мастер-корабел требует: сандала - " + NPChar.Tuning.Matherial1 + ", золотых слитков - " + NPChar.Tuning.Matherial2 + ", сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". В качестве задатка было уплачено " + NPChar.Tuning.Money + " золотых. Видимо, старик без ума от драгоценностей. Что ж - у каждого свои слабости...");
		}
		else
		{
			NextDiag.TempNode = "ship_tunning_not_now";
	...
	case "ship_tunning_SpeedRate_start":
		...
		if (makeint(Pchar.money) >= amount)
		{
			...
			ReOpenQuestHeader("ShipTuning");
			AddQuestRecord("ShipTuning", "t1");
			AddQuestUserData("ShipTuning", "sText", "За свою работу по увеличению скорости корабля " + XI_ConvertString(RealShips[sti(Pchar.Ship.Type)].BaseName) + " мастер-корабел требует: шелка - " + NPChar.Tuning.Matherial1 + ", алмазов - " + NPChar.Tuning.Matherial2 + ", сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". В качестве задатка было уплачено " + NPChar.Tuning.Money + " золотых. Видимо, старик без ума от драгоценностей. Что ж - у каждого свои слабости...");
		}
		else
		{
			NextDiag.TempNode = "ship_tunning_not_now";
	...
	case "ship_tunning_MastMultiplier_start":
		...
		if (makeint(Pchar.money) >= amount)
		{
			...
			ReOpenQuestHeader("ShipTuning");
			AddQuestRecord("ShipTuning", "t1");
			AddQuestUserData("ShipTuning", "sText", "За свою работу по укреплению мачт корабля " + XI_ConvertString(RealShips[sti(Pchar.Ship.Type)].BaseName) + " мастер-корабел требует: досок - " + NPChar.Tuning.Matherial1 + ", крестиков - " + NPChar.Tuning.Matherial2 + ", сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". В качестве задатка было уплачено " + NPChar.Tuning.Money + " золотых. Видимо, старик без ума от драгоценностей. Что ж - у каждого свои слабости...");
		}
		else
		{
			NextDiag.TempNode = "ship_tunning_not_now";
	...
	case "ship_tunning_MaxCrew_start":
		...
		if (makeint(Pchar.money) >= amount)
		{
			...
			ReOpenQuestHeader("ShipTuning");
			AddQuestRecord("ShipTuning", "t1");
			AddQuestUserData("ShipTuning", "sText", "За свою работу по увеличению экипажа на корабле " + XI_ConvertString(RealShips[sti(Pchar.Ship.Type)].BaseName) + " мастер-корабел требует: кожи - " + NPChar.Tuning.Matherial1 + ", сапфиров - " + NPChar.Tuning.Matherial2 + ", сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". В качестве задатка было уплачено " + NPChar.Tuning.Money + " золотых. Видимо, старик без ума от драгоценностей. Что ж - у каждого свои слабости...");
		}
		else
		{
			NextDiag.TempNode = "ship_tunning_not_now";
	...
	case "ship_tunning_TurnRate_start":
		...
		if (makeint(Pchar.money) >= amount)
		{
			...
			ReOpenQuestHeader("ShipTuning");
			AddQuestRecord("ShipTuning", "t1");
			AddQuestUserData("ShipTuning", "sText", "За свою работу по увеличению маневренности на корабле " + XI_ConvertString(RealShips[sti(Pchar.Ship.Type)].BaseName) + " мастер-корабел требует: полотна - " + NPChar.Tuning.Matherial1 + ", рубинов - " + NPChar.Tuning.Matherial2 + ", сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". В качестве задатка было уплачено " + NPChar.Tuning.Money + " золотых. Видимо, старик без ума от драгоценностей. Что ж - у каждого свои слабости...");
		}
		else
		{
			NextDiag.TempNode = "ship_tunning_not_now";
	...
	case "ship_tunning_HP_start":
		...
		if (makeint(Pchar.money) >= amount)
		{
			...
			ReOpenQuestHeader("ShipTuning");
			AddQuestRecord("ShipTuning", "t1");
			AddQuestUserData("ShipTuning", "sText", "За свою работу по увеличению прочности корпуса корабля " + XI_ConvertString(RealShips[sti(Pchar.Ship.Type)].BaseName) + " мастер-корабел требует: красного дерева - " + NPChar.Tuning.Matherial1 + ", серебряных слитков - " + NPChar.Tuning.Matherial2 + ", сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". В качестве задатка было уплачено " + NPChar.Tuning.Money + " золотых... Говорит, что в Европу собрался - врёт, наверное, старый черт.");
		}
		else
		{
			NextDiag.TempNode = "ship_tunning_not_now";
	...
	case "ship_tunning_WindAgainst_start":
		...
		if (makeint(Pchar.money) >= amount)
		{
			...
			ReOpenQuestHeader("ShipTuning");
			AddQuestRecord("ShipTuning", "t1");
			AddQuestUserData("ShipTuning", "sText", "За свою работу по увеличению хода в бейдевинд на корабле " + XI_ConvertString(RealShips[sti(Pchar.Ship.Type)].BaseName) + " мастер-корабел требует: хлопка - " + NPChar.Tuning.Matherial1 + ", изумрудов - " + NPChar.Tuning.Matherial2 + ", сундуков с золотом - " + NPChar.Tuning.Matherial3 + ". В качестве задатка было уплачено " + NPChar.Tuning.Money + " золотых. Совсем старик с ума сбрендил от этих побрякушек.");
		}
		else
		{
			NextDiag.TempNode = "ship_tunning_not_now";
	...
	case "quests":
		...
		if (CheckCharacterItem(pchar, "Ship_Print_1"))
			NPChar.print1 = 0;
		if (CheckCharacterItem(pchar, "Ship_Print_2"))
			NPChar.print2 = 0;
		if (CheckCharacterItem(pchar, "Ship_Print_3"))
			NPChar.print3 = 0;
		if (CheckCharacterItem(pchar, "Ship_Print_4"))
			NPChar.print4 = 0;
		if (CheckCharacterItem(pchar, "Ship_Print_5"))
			NPChar.print5 = 0;
		if (CheckCharacterItem(pchar, "Ship_Print_6"))
			NPChar.print6 = 0;
	...
	case "Step_S2_6":
		...
		break;
	case "PayToRapair1":
		if (pchar.paytorepair != pchar.dayRandom && CheckAttribute(PChar, "paytorepair"))
			DeleteAttribute(PChar, "paytorepair");
		...
		link.l1 = "Я вас поня" + GetSexPhrase("л", "ла") + ". Надеюсь 15000 тысяч пиастров уладит этот вопрос?" link.l1.go = "PayToRapair2";
		...
		if (CheckAttribute(PChar, "paytorepair"))
		{
			...
			link.l1 = "Мне нужно починить корабль." link.l1.go = "shipyard";
