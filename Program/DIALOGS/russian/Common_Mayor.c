		...
		if (sti(NPChar.nation) == PIRATE)
		{
			...
			else
			{
				...
				else
				{
					...
					{
						dialog.text = "Вижу ты с новостями?" link.l1 = "Да, завалил" + GetSexPhrase("", "а") + " таки северянина, ох и лютый же был. Обычно все разговоры заводят как смерть почуят, а этот рычал только аж пена изо рта шла.";
					...
					if (attrLoc == "Mein")
					{
						dialog.text = "Его " + RandPhraseSimple("Милость", "Светлость") + " генерал-губернатор " + NationNameGenitive(sti(NPChar.nation)) + " " + GetFullName(offref) +
									  LinkRandPhrase(" находится сейчас на материке ",
													 ...
													 " находится с визитом на материке ") +
									  " в городе " + GetConvertStr(locations[cn].fastreload + " Town", "LocLables.txt") + ".";
					}
					else
					{
						dialog.text = "Его " + RandPhraseSimple("Милость", "Светлость") + " генерал-губернатор " + NationNameGenitive(sti(NPChar.nation)) + " " + GetFullName(offref) +
									  LinkRandPhrase(" находится сейчас на острове ",
													 ...
													 " находится с визитом на острове ") +
									  GetConvertStr(attrLoc, "LocLables.txt") + " в городе " + GetConvertStr(locations[cn].fastreload + " Town", "LocLables.txt") + ".";
					}
				}
	...
	case "work":
		...
		if (CheckAttribute(NPChar, "from_sea") && Pchar.location.from_sea == NPChar.from_sea)
		{
			if (!CheckAttribute(pchar, "GenQuest.GhostShip.LastBattle") &&
				...
				dRand(2) == 1)
			{
				dialog.text = "Да, " + GetAddress_Form(NPChar) + RandPhraseSimple(", скорее, скорее спасите наш город!! Живые мертвецы в порту бьются с защитниками. Я не знаю, что делать!", ", вы вовремя! Мне сообщили, что в порту идет бой. Город штурмуют пираты. Помогите нам уничтожить их.");
		...
		if (GetCharacterSPECIALSimple(PChar, SPECIAL_C) >= dRand(11))
			i = -1; //WW   харизма героя перебивает работу у ПГГ
		if (i != -1)
		{
				...
				case 4:
					...
					if (sti(pchar.rank) > 6)
					{
						if (sti(pchar.rank) > 20)
							iRank = sti(pchar.rank) + sti(MOD_SKILL_ENEMY_RATE);
						else
...
*/
	...
	case "DestroyGang_agree":
		...
		else
		{
			...
			if (!CheckAttribute(pchar, "Kamikazi") && rand(99) == 0)
				pchar.quest.DestroyGang.win_condition = "Kamikazi";
			else
