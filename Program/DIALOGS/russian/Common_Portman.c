...
void ProcessDialogEvent()
{
	...
	case "BurntShip19":
		...
		if (sti(rRealShip.Stolen))
			cn *= 3; // "ворованные" компенсируются
			...
			for (i = 1; i < COMPANION_MAX; i++) //почему нельзя свой единственный корабль сдать?
			{
				...
				if (cn > 0)
				{
					...
					if (!GetRemovable(chref))
						continue;
					if (RealShips[sti(chref.Ship.Type)].BaseName == SHIP_SOLEYRU)
						continue; //Отрубаем хитрость на получение Солея
...
void BurntShipQuest_FillStartParams(ref _npchar)
{
	...
	if (rank < 5)
		shipType = SHIP_BERMSLOOP + rand(11);
	if (rank >= 5 && rank < 10)
		shipType = SHIP_POLACCA + rand(12);
	if (rank >= 10 && rank < 15)
		shipType = SHIP_BRIG + rand(23);
	if (rank >= 15 && rank < 20)
		shipType = SHIP_PINNACE + rand(31);
	if (rank >= 20 && rank < 30)
		shipType = SHIP_GALEON50 + rand(20);
	if (rank >= 30)
		shipType = SHIP_POSEIDON + rand(19);
