void ProcessDialogEvent()
{
	...
	case "ShipWreck_40":
		pchar.quest.ShipWreck_DeliveToCity.over = "yes";
		if (CheckAttribute(pchar, "GenQuest.CaptainComission.CrazyRec"))
			DeleteAttribute(pchar, "GenQuest.CaptainComission.CrazyRec");
	...
	case "CaptainComission_404":
		...
		if (NPChar.alignment == "good" && sti(pchar.reputation) > 50)
			bTemp = true;
		if (NPChar.alignment == "bad" && sti(pchar.reputation) <= 50)
			bTemp = true;
	...
	case "Greedy_4":
		if (!CheckAttribute(pchar, "GemGiven"))
			pchar.GemGiven = 1;
		else
			pchar.GemGiven = sti(pchar.GemGiven) + 1;
...
int PiratesOnUninhabited_GenerateShipType()
{
	...
	if (rank >= 5 && rank < 20)
	{
		...
		case 0:
			...
			break;
		case 1:
			retShipType = SHIP_BARQUE;
			break;
		case 2:
			retShipType = SHIP_FLEUT;
			break;
		case 3:
