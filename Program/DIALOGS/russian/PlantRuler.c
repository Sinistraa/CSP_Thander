...
void ProcessDialogEvent()
{
	...
	case "Meet_2":
		if (!CheckAttribute(npchar, "met"))
			dialog.text = "Приятно познакомиться. По какому поводу зашли?";
		else
			dialog.text = "По какому поводу зашли, капитан?";
	...
	case "trade_good_3":
		if (sti(pchar.Ship.Cargo.Goods.Slaves) > sti(npchar.SlavesLimit))
			dialog.text = "Сейчас я могу купить у вас до " + npchar.SlavesLimit + " рабов ценой по 215 за единицу. Итого, " + FindRussianMoneyString(sti(npchar.SlavesLimit) * 215) + ". Устраивает?";
		else
			dialog.text = "Сейчас я могу купить у вас до " + npchar.SlavesLimit + " рабов ценой по 215 за единицу. Итого, " + FindRussianMoneyString(sti(pchar.Ship.Cargo.Goods.Slaves) * 215) + ". Устраивает?";
		Link.l1 = "Вполне.";
