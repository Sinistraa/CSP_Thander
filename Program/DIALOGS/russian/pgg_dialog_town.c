...
void ProcessDialogEvent()
{
		...
		if (PGG_ChangeRelation2MainCharacter(NPChar, 0) > 49)
		{
			Dialog.Text = LinkRandPhrase("... Кошки впились в борт нам! По местам стоять! И морскому дьяволу тут не устоять!..",
						  ...
						  PCharRepPhrase(LinkRandPhrase(" Редко встретишь порядочного капитана в этих местах.",
														...
														" А вот и честный капитан, не то что мы, темнота! Хи-хи!") +
											 " Какие вести?",
										 LinkRandPhrase(" А вот и странствующ" + GetSexPhrase("ий покоритель морей!", "ая морская волчица!") + " Хотели что-то спросить?",
														...
														" Что-то запахло потом и кровью прямо по курсу!") +
	...
	case "officer":
		...
		Dialog.Text = NPCharRepPhrase(NPChar, RandSwear() + RandPhraseSimple("Каррамба! Я не прочь погеройствовать! Одна проблемка, мне нужно " + NPChar.Quest.Officer.Price + " пиастров, чтобы расплатиться с долгами.", "Ты кажешься мне не бедн" + GetSexPhrase("ым человеком", "ой женщиной") + ". И если не прочь заплатить мне " + NPChar.Quest.Officer.Price + " пиастров, то, пожалуй, мы сработаемся."),
		...
		MakeRandomLinkOrderTwo(link, PCharRepPhrase(RandSwear() + RandPhraseSimple("Меня это устраивает. Но, надеюсь, ты не каждый день будешь требовать столько.", "Это чертовски дорого, но, думаю, ты того стоишь."), LinkRandPhrase("Отлично! Считай себя зачисленной в команду.", "Хм... Пожалуй, я заплачу тебе.", "Похоже, ты стоишь этих денег. Держи!")), "officer_hired",
							   PCharRepPhrase(RandSwear() + RandPhraseSimple("Ты не стоишь столько. Держи нос по ветру...", "Это чертовски дорого, я не готов" + GetSexPhrase("", "а") + " к таким тратам."),
											  LinkRandPhrase("Да... жадность не порок...", "Ты смеешься надо мной? За такие деньги я могу купить десяток таких, как ты.", "Это грабеж... Слишком дорого.")),
							   "exit");
	...
	case "officer_hired":
		...
		if (NPChar.model.animation == "man" || NPChar.model.animation == "YokoDias" || NPChar.model.animation == "Milenace")
			Npchar.CanTakeMushket = true;
		if (npchar.model == "PGG_Whisper_6")
			EquipCharacterByItem(NPChar, "suit_1");
		if (!CheckAttribute(npchar, "HiringMoneyPaid"))
	...
	case "companion":
		...
		{
			Dialog.Text = NPCharRepPhrase(NPChar, RandSwear() + RandPhraseSimple("Заманчивое предложение! Хорошо! Я составлю компанию!", "А ты не промах, капитан... Думаю, мы сработаемся!"),
	...
	case "companion_pay":
		...
		{
			Dialog.Text = RandSwear() + " И правда не могу." PGG_ChangeRelation2MainCharacter(NPChar, 100);
		...
		{
			Dialog.Text = RandSwear() + " Это была шутка? У тебя же в карманах ветер свищет!" link.l1 = "И правда... Пойду я.";
			link.l1.go = "exit";
		}
		break;
	...
	case "companion_leave":
		Dialog.Text = RandSwear() + " " + NPCharRepPhrase(NPChar, RandPhraseSimple("Разорви тебя акула!!! Я выхожу из состава эскадры!", "Мое терпенье лопнуло! Я ухожу."), RandPhraseSimple("С меня достаточно, 'адмирал'!... Я иду своим курсом!", "С меня хватит! Я выхожу из состава эскадры."));
	...
	case "companion_stay":
		...
		Dialog.Text = RandSwear() + " " + NPCharRepPhrase(NPChar, RandPhraseSimple("Премия? Отлично! Сумма в " + NPChar.Quest.Companion.Price + " пиастров, меня устроит.", "Пожалуй, если ты выплатишь моей команде " + NPChar.Quest.Companion.Price + " пиастров, я уговорю их остаться."), RandPhraseSimple("Хорошо! Я готова остаться за скромную премию в размере " + NPChar.Quest.Companion.Price + " пиастров!", NPChar.Quest.Companion.Price + " пиастров устроят на некоторое время меня и мою команду."));
	...
	case "Yes_or_no":
		...
		MakeRandomLinkOrderTwo(link, PCharRepPhrase(RandSwear() + RandPhraseSimple("Меня это устраивает. Но, надеюсь, ты не каждый день будешь требовать столько.", "Это чертовски дорого, но, думаю, ты того стоишь."), LinkRandPhrase("Я соглас" + GetSexPhrase("ен", "на") + "!", "Хм... Пожалуй, я готов" + GetSexPhrase("", "а") + " расстаться с этой суммой.", "Похоже, ты стоишь этих денег. Держи!")), "officer_hired",
							   PCharRepPhrase(RandSwear() + RandPhraseSimple("Ты не стоишь столько. Бывай...", "Это чертовски дорого! Слишком дорого."),
											  LinkRandPhrase("Да... жадность не порок...", "Ты издеваешься? За такие деньги я могу купить десять таких.", "Это грабеж... умерь свою хотелку.")),
							   "exit_PIRATES");
		break;
	...
	case "quest_onStay": // ПГГ сам подходит
		...
		{
			link.l1 = PCharRepPhrase(RandSwear() + RandPhraseSimple("Я капитан " + GetFullName(pchar) + ".", "Я капитан посудины '" + PChar.Ship.Name + "' и пары сотен настоящих чертей!!! Мое имя " + GetFullName(pchar) + ".") + " А тебя я что-то не припомню.",
									 RandPhraseSimple("Позвольте представиться. Я капитан " + GetFullName(pchar) + ".",
													  "Не имел чести быть вам представленным. Я капитан корабля '" + PChar.Ship.Name + "' под флагом " + NationNameGenitive(sti(PChar.nation)) + ". Мое имя " + GetFullName(pchar) + ".") +
										 " А Вас я что-то не припоминаю.");
	...
	case "Quest_1_Work":
		...
		if (PChar.GenQuest.PGG_Quest.Template == "1")
		...
		if (CheckAttribute(pchar, "BSStart") && CheckAttribute(pchar, "BSInProgress"))
			PChar.GenQuest.PGG_Quest.Goods = GOOD_GOLD;
	...
	case "Exit_Quest_1_AfterTavernTalk":
		...
		if (NPChar.Chr_Ai.Type == "actor")
			LAi_ActorGoToLocation(NPchar, "reload", "reload1_back", "none", "", "", "", 20);
		break;
