void ProcessDialogEvent()
{
	...
	case "First time":
		...
		{
			string sTemp = "" DeleteAttribute(pchar, "questTemp.Whisper.HoldMonologue");
	...
	case "SmugglingFlag":
		if (CheckAttribute(pchar, "SmugglingFlag"))
			DeleteAttribute(pchar, "SmugglingFlag");
		else
			pchar.SmugglingFlag = true;
		Dialog.Text = "Готово.";
	...
	case "WorldmapTime_Change":
		if (stf(GetStrSmallRegister(dialogEditStrings[1])) > 0.0 && stf(GetStrSmallRegister(dialogEditStrings[1])) < 12.1)
			pchar.wmtimechange = stf(GetStrSmallRegister(dialogEditStrings[1]));
		else
			pchar.wmtimechange = 0.5;
		if (stf(pchar.wmtimechange) != 0.5)
			Dialog.Text = "Установлена скорость глобальной карты " + pchar.wmtimechange + ".";
		else
			Dialog.Text = "Установлена стандартное значение в 0.5.";
	...
	case "HellSpawn_Ritual": //перерождение
		...
		break
			...
			case "autofood" : Dialog.Text = "Введите процент, до которого должна опуститься энергия, прежде чем начать использовать еду (10-90).";
		//PChar.autofood_use
		link.l1 = "";
		Link.l1.edit = 3;
		...
		break
			...
			case "autofood_percentage" : iTemp = sti(dialogEditStrings[3]);
	...
	case "ReadBook":
		...
		break
			...
			case "TalkSelf_ChangeFlag" : Dialog.Text = "Какой флаг мне необходим на данный момент? Его смена займёт 1 час.";
		if (IsCharacterPerkOn(pchar, "FlagPir") && sti(pchar.nation) != 4)
		{
			Link.l1 = "Поднять пиратский флаг.";
			Link.l1.go = "TalkSelf_ChangeFlagPir";
		}
		if (IsCharacterPerkOn(pchar, "FlagEng") && sti(pchar.nation) != 0)
		{
			Link.l2 = "Поднять английский флаг.";
			Link.l2.go = "TalkSelf_ChangeFlagEng";
		}
		if (IsCharacterPerkOn(pchar, "FlagFra") && sti(pchar.nation) != 1)
		{
			Link.l3 = "Поднять французский флаг.";
			Link.l3.go = "TalkSelf_ChangeFlagFra";
		}
		if (IsCharacterPerkOn(pchar, "FlagSpa") && sti(pchar.nation) != 2)
		{
			Link.l4 = "Поднять испанский флаг.";
			Link.l4.go = "TalkSelf_ChangeFlagSpa";
		}
		if (IsCharacterPerkOn(pchar, "FlagHol") && sti(pchar.nation) != 3)
		{
			Link.l5 = "Поднять голландский флаг.";
			Link.l5.go = "TalkSelf_ChangeFlagHol";
		}
