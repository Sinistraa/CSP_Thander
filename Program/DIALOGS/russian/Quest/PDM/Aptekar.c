...
void ProcessDialogEvent()
{
	...
	case "CheckForConfuse":
		...
		for (i = 0; i < GetPassengersQuantity(pchar); i++)
		{
			...
			if (!GetRemovable(sld))
				continue;
			if (!CheckAttribute(sld, "HPminusDaysNeedtoRestore") || CheckAttribute(sld, "Doctored"))
				continue;
			attrL = "l" + i;
	...
	case "OfficerConfused":
		...
		if (!CheckAttribute(chref, "quest.officertype"))
			...
			offtype = XI_ConvertString(chref.quest.officertype);
	...
	case "OfficerConfused_2":
		chref = GetCharacter(sti(NPChar.OfficerToHealConfuse));
		if (CheckAttribute(chref, "chr_ai.max_hp"))
			npchar.HealPrice = (makeint(chref.chr_ai.max_hp) * 100) + makeint((sti(chref.rank) * 500) * (MOD_SKILL_ENEMY_RATE / 2));
		else
			npchar.HealPrice = (makeint(chref.health.maxhp) * 100) + makeint((sti(chref.rank) * 500) * (MOD_SKILL_ENEMY_RATE / 2));
