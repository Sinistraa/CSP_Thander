...
void ProcessDialogEvent()
{
	...
	case "Viktor_Bitva":
		...
		sld = CharacterFromID("PDM_NK_Viktor") if (MOD_SKILL_ENEMY_RATE >= 1 && MOD_SKILL_ENEMY_RATE <= 3) sld.equip.blade = "blade3";
		if (MOD_SKILL_ENEMY_RATE >= 4 && MOD_SKILL_ENEMY_RATE <= 6)
			sld.equip.blade = "blade18";
		if (MOD_SKILL_ENEMY_RATE >= 7 && MOD_SKILL_ENEMY_RATE <= 10)
			sld.equip.blade = "blade39";
		if (MOD_SKILL_ENEMY_RATE <= 6)
			FantomMakeCoolFighter(sld, Rank, Sila, Sila, "", "Pistol1", DopHP);
