...
void ProcessDialogEvent()
{
	...
	case "Antonio_8_1":
		...
		if (LAi_IsDead(CharacterFromID("PDM_Octavio_Lambrini")))
			OctavioDead = "Пусть мы сегодня и победили, но старший сын Ламбрини займёт его место и продолжит дело отца. Боюсь, контрабандисткая секта будет вечно приносить вред Великой Испании.";
		...
		if (!LAi_IsDead(CharacterFromID("PDM_Octavio_Lambrini")))
			OctavioDead = "Октавио Ламбрини арестован, ";
	...
	case "LambriniPGG_2_7":
		...
		string finalWords = ".";
		if (CheckAttribute(npchar, "bonusEnergy"))
			finalWords = " и несколько незабываемых приятных воспоминаний.";
