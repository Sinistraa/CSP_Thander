...
void ProcessDialogEvent()
{
	...
	case "First time":
		...
		if (npchar.quest.meeting == "0")
		{
			...
			{
				if (npchar.location == "FurieShipInside2")
					dialog.text = "Боже мой, какой шторм!.. Здравствуйте, меня зовут " + GetFullName(npchar) + ", я хозяйка этого дома.";
				else
					dialog.text = "Боже мой, как шторм!.. Здравствуйте, меня зовут " + GetFullName(npchar) + ".";
			...
			else
			{
				if (npchar.location == "FurieShipInside2")
					...
					dialog.text = "Здравствуйте, меня зовут " + GetFullName(npchar) + ".";
