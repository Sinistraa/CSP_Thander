void ProcessDialogEvent()
{
	...
	case "chard_waiting_quest_result":
		...
		if (CheckAttribute(pchar, "Whisper.FoundPortmanJournal"))
		{
			...
			Link.l1.go = "chard_check_journal" break;
		}
		if (CheckAttribute(pchar, "Whisper.BetrayChard"))
		{
			...
			Link.l1.go = "chard_betrayed" break;
		}
	...
	case "OrderHunter":
		...
		else
		{
			...
			if (sti(PChar.OrderHunter) == 2)
			{
				AddQuestRecord("TheLastMass", "2");
			}
	...
	case "CapitolCaptured_3":
		...
		Link.l2.go = "CapitolCaptured_4";
		break case "CapitolCaptured_4" : dialog.text = "Отличная мысль, капитан!";
