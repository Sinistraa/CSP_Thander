...
void ProcessDialogEvent()
{
	...
	case "Step_H3_5":
		...
		if (CheckAttribute(pchar, "questTemp.Talks.Tavern"))
			sTemp = sTemp + " Пьер Пикардиец снял комнату в таверне Тортуги на год вперед.";
		if (CheckAttribute(pchar, "questTemp.Talks.Store"))
			sTemp = sTemp + " Скупает драгоценности в магазине Тортуги чуть ли не оптом.";
		if (CheckAttribute(pchar, "questTemp.Talks.Shipyard"))
			sTemp = sTemp + " Поставил на свой фрегат новейшие дорогущие кулеврины.";
		if (CheckAttribute(pchar, "questTemp.Talks.Usurer"))
			sTemp = sTemp + " Ростовщику дал в рост такую сумму, что тот теперь ни у кого не принимает деньги.";
		if (CheckAttribute(pchar, "questTemp.Talks.Brothel"))
			sTemp = sTemp + " В борделе он завсегдатай и желанный гость.";
	...
	case "PL_Q1_3":
		...
		;
		link.l1 = "" + GetSexPhrase("Хорошо, буду иметь в виду", "Cпасибо") + "." link.l1.go = "exit";
	...
	case "PL_Q8_PBResidence_9":
		...
		link.l1 = "Что ты предлагаешь конкретно?" link.l1.go = "PL_Q8_PBResidence_10";
		break;
	...
	case "PL_Q8_Panama2":
		...
		if (CheckAttribute(sld, "private1.money"))
			sld.private1.money = 0;
		if (CheckAttribute(sld, "private2.money"))
			sld.private2.money = 0;
	...
	case "PL_Q8_Panama2_2":
		...
		if (CheckAttribute(sld, "private1.items.gold"))
			...
			AddMoneyToCharacter(pchar, -50000000);
		break;
