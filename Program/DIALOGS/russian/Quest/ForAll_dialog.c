...
void ProcessDialogEvent()
{
	...
	case "PGG_hired":
		...
		if (sld.model.animation == "man")
			sld.CanTakeMushket = true;
		if (HasSubStr(sld.model, "Whisper"))
		{
			sld.CanTakeMushket = true;
			if (sld.model == "PGG_Whisper_6")
	...
	case "DeckSharp_6":
		...
		sTemp = "";
		if (CheckAttribute(pchar, "questTemp.Sharp.PearlAreal.add"))
			sTemp = pchar.questTemp.Sharp.PearlAreal.add;
	...
	case "SCQ_exit":
		...
		if (!CheckAttribute(sld, "questSeekCap"))
			sld.questSeekCap = 0;
