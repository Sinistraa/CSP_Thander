...
void ProcessDialogEvent()
{
	...
	if (CheckAttribute(npchar, "angry") && !CheckAttribute(npchar, "angry.ok"))
	{
		...
		else
		{
			...
			case "repeat":
				...
				if (npchar.angry.name == "NewLife_Budget_Add")
					Dialog.CurrentNode = "AngryRepeat_Budget_Add";
				//когда подарками достал, а сам не давал
				if (npchar.angry.name == "NewLife_Present")
					Dialog.CurrentNode = "AngryRepeat_Present";
				break;
			case "quest":
				//когда долги достали Изабеллу
				if (npchar.angry.name == "NewLife_Budget_Pay")
					Dialog.CurrentNode = "AngryRepeat_Budget_Pay";
				//когда достало Изабеллу пиратсво ГГ
				if (npchar.angry.name == "NewLife_KnowPirates")
					Dialog.CurrentNode = "AngryRepeat_KnowPirates";
				break;
			}
		}
	}
	...
	case "Good_1":
		...
		if (!CheckAttribute(pchar, "quest.VisitStep"))
			pchar.quest.VisitStep = 1;
		else
			pchar.quest.VisitStep = makeint(pchar.quest.VisitStep) + 1;
		pchar.quest.wasInBeedroom = true;
		bDisableFastReload = true;
		pchar.quest.RP_afterVisitBedroom.win_condition.l1 = "ExitFromLocation";
		pchar.quest.RP_afterVisitBedroom.win_condition.l1.location = pchar.location;
