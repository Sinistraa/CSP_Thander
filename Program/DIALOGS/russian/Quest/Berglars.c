...
void ProcessDialogEvent()
{
    ...
    if (!CheckAttribute(NPChar, "refreshanim"))
        NPChar.refreshanim = "yes";
    if (NPChar.refreshanim == "yes")
    {
        if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
            NPChar.model.animation = "spy"; //LEO: Превозмогаторам страдать 08.12.2021
        Characters_RefreshModel(NPChar);
        if (bHardBoss)
            NPChar.AlwaysReload = true; //перезарядка независимо от Дозарядки
    ...
    if (Dialog.CurrentNode == "First time")
    {
        ...
        case "2":
            ...
            break;
        case "3":
            Dialog.CurrentNode = "Third time";
            break;
        case "4":
            Dialog.CurrentNode = "Fourth time";
            break;
        case "5":
            Dialog.CurrentNode = "Fifth time";
            break;
        case "6":
            Dialog.CurrentNode = "Sixth time";
            break;
        }
    }
    ...
    case "FirstFight_overAll":
        ...
        if (sti(pchar.questTemp.tugs.berglarState) == 2)
        {
            AddSimpleRumourCityTip("Ох, в неспокойные мы с вами, " + GetAddress_Form(pchar) + ", времена живём. У нас вот давеча судно встало на рейд, капитан его на берег сошёл, а на борт так и не поднялся. Только через неделю, в пещерах за городом, стража нашла его труп... с перерезанной от уха до уха глоткой!.. " +
                                       "Во время дознания местные отвечали, что видели того капитана по пути к пещере с каким-то ничем не примечательным на первый взгляд проходимцем. Так что, у нас нужно глядеть в оба...",
        ...
        if (sti(pchar.questTemp.tugs.berglarState) == 4)
        {
            AddSimpleRumourCityTip("Была тут у нас история - капитан один пропал... сошел с корабля и ни слуху, ни духу. Нашли его почитай через месяц, в пещере за городом. Голова прострелена, а в карманах - ни гроша! Да то, " + GetAddress_Form(pchar) + ", скажу я вам, не нечисть никак! Нечисть по карманам не шарит. Тут дело в другом, это точно!.. " +
                                       "Слыхал я, в Сент-Джонсе капитаны так же пропадали, и не единожды... Ну, вздрогнем...",
                                   npchar.city, 3, 1, "habitue", "");
        }
        ...
        if (npchar.id == "BerglarFortFrance")
        {
            ...
            if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
                sld = GetCharacter(NPC_GenerateCharacter("Berglars_Helper", "pirate_" + (rand(9) + 1), "man", "spy", 18, PIRATE, 0, true)); //LEO: Превозмогаторам страдать 08.12.2021
            else
                sld = GetCharacter(NPC_GenerateCharacter("Berglars_Helper", "pirate_" + (rand(9) + 1), "man", "man_fast", 18, PIRATE, 0, true));
            if (bHardBoss)
        ...
        if (npchar.id == "BerglarPanama")
        {
            GetCharacterPos(pchar, &locx, &locy, &locz);
            if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
                sld = GetCharacter(NPC_GenerateCharacter("Berglars_Helper", "pirate_" + (rand(9) + 1), "man", "spy", 21, PIRATE, 0, true)); //LEO: Превозмогаторам страдать 08.12.2021
            else
                sld = GetCharacter(NPC_GenerateCharacter("Berglars_Helper", "pirate_" + (rand(9) + 1), "man", "man_fast", 21, PIRATE, 0, true));
            if (bHardBoss)
                sld.AlwaysReload = true; //перезарядка независимо от Дозарядки
        ...
        if (npchar.id == "BerglarSantaCatalina")
        {
            GetCharacterPos(pchar, &locx, &locy, &locz);
            if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
                sld = GetCharacter(NPC_GenerateCharacter("Berglars_Helper", "pirate_" + (rand(9) + 1), "man", "spy", 20, PIRATE, 0, true)); //LEO: Превозмогаторам страдать 08.12.2021
            else
                sld = GetCharacter(NPC_GenerateCharacter("Berglars_Helper", "pirate_" + (rand(9) + 1), "man", "man_fast", 20, PIRATE, 0, true));
            if (bHardBoss)
                sld.AlwaysReload = true; //перезарядка независимо от Дозарядки
