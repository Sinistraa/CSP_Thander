...
void ProcessDialogEvent()
{
	...
	if (CheckAttribute(npchar, "angry") && !CheckAttribute(npchar, "angry.ok"))
	{
		...
		else
		{
			...
			case "repeat":
				...
				if (npchar.angry.name == "BlueBird_1")
					Dialog.CurrentNode = "AngryRepeat_1";
				break;
			}
		}
	...
	case "CapComission2":
		...
		else
		{
			...
			{
				link.l2 = "Хорошо, что не продал. Вот деньги - 150000 золотых. Где я могу его забрать?" link.l2.go = "CapComission2_3";
			}
		}
		break;
...
void SetMushketCapitainInWorld()
{
	...
	if (MOD_SKILL_ENEMY_RATE == 10 && bHardAnimations)
		ref sld = GetCharacter(NPC_GenerateCharacter("MushketCap", "officer_17", "man", "spy", Rank, PIRATE, -1, true));
	else
		sld = GetCharacter(NPC_GenerateCharacter("MushketCap", "officer_17", "man", "man_fast", Rank, PIRATE, -1, true));
	sld.name = "Альберт";
	sld.lastname = "Зиверт";
