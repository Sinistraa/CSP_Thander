...
void SetSpyGlassData()
{
	...
	if (InterfaceStates.HUDStyle)
		sTextureName = "battle_interface\ship_icons2.tga";
	else
		sTextureName = "battle_interface\ship_icons2_johnny88hud.tga";
	float uvLeft = 0;
	float uvTop = 0;
	float uvRight = 0.125;
		...
		if (iShip != SHIP_NOTUSED)
		{
			...
			if (ShipsTypes[iShip].name == "fort")
			{
				...
				if (InterfaceStates.AltIntIcons)
					sTextureName = "battle_interface\List_icon2_Konshud.tga";
				else
	...
	{
		if (CheckCharacterPerk(chref, "MusketsShoot"))
			sCaptainName = "  " + XI_ConvertString("MushketShot") + "   " + sCaptainName;
		if (CheckAttribute(RealShips[sti(chref.Ship.Type)], "Tuning.HighBort"))
			sCaptainName = "  " + XI_ConvertString("EXTRABIGSIDESON") + "  " + sCaptainName;
	}
...
void FillISpyGlassParameters()
{
	...
	if (InterfaceStates.SpyglassTextures)
		objISpyGlass.lens.texture = "battle_interface\\spyglassEHO\\" + sTexture + ".tga";
	else
	...
	if (InterfaceStates.HUDStyle)
		...
		objISpyGlass.info.nation.pos = RecalculateHIconScaled(136) + "," + (ntop + RecalculateVIconScaled(8)) + "," + RecalculateHIconScaled(200) + "," + (ntop + RecalculateVIconScaled(40));
	objISpyGlass.info.nation.uv = "0,0,0.125,1.0";
	//
	if (InterfaceStates.AltIntIcons)
		objISpyGlass.info.cannon.texture = "battle_interface\List_icons_Konshud.tga";
	else
	...
	//
	if (InterfaceStates.AltIntIcons)
		objISpyGlass.info.speed.texture = "battle_interface\List_icon2_Konshud.tga";
	else
	...
	if (InterfaceStates.AltIntIcons)
		objISpyGlass.info.sailto.texture = "battle_interface\List_icons_Konshud.tga";
	else
		objISpyGlass.info.sailto.texture = "battle_interface\list_icons.tga";
	...
	//
	if (InterfaceStates.AltIntIcons)
		objISpyGlass.info.charge.texture = "battle_interface\List_icon2_Konshud.tga";
	else
