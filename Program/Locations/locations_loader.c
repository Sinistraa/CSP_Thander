...
void SetCamShuttle(ref loc) // boal вынес в метод
{
	...
	else
	{
		if (CheckAttribute(pchar, "GenQuest.CamShuttle"))
		{
			...
			{
				if (Pchar.Drunked.Today != Pchar.dayRandom)
					Log_Info("Ох, что-то штормит меня сегодня...");
			}
...
bool LoadLocation(ref loc)
{
	...
	if (loc.id == "Bridgetown_Plantation")
		DeleteAttribute(loc, "fastreload");
	if (loc.id == "Marigo_ExitTown")
	{
		loc.townsack = "Marigo";
		loc.fastreload = "Marigo";
	}
	...
	if (isNoBoarding)
	{
		...
		else
		{
			if (nLocConst != LOC_TOWN)
			{
				...
				if (CheckAttribute(loc, "id.label") && loc.id.label == "Alcove")
					dropSea = false;
				if (dropSea)
					CreateSea(EXECUTE, REALIZE);
			}
	...
	for (i = 0; i < num; i++)
	{
		...
		if (sat != "grassPatch")
		{
			//#20180920-01
			if (bUpLoc && CheckFP(loc))
			{ //locator models typically precede
				nAmbient = amb_tavern;
				//   SendMessage(loc, "ll", MSG_LOCATION_SET_AMBIENT_LIGHT, nAmbient);
				bUpLoc = false;
			}
...
void LocLoadShips(ref Location)
{
	...
	if (bMainCharacterHere)
	{
		...
		{
			if (locNumShips >= iNumShips)
				break;
			if (locNumShips >= maxShipsLoc)
				break;
	...
	for (i = 1; i < MAX_CHARACTERS; i++) // начало не с 0
	{
		...
		if (!Character_LocIsEntryLocation(&Characters[i], Location))
			continue;
		//#20200226-02
		if (CheckAttribute(&Characters[i], "ShipInStockMan"))
			continue;
		bool bExist = false;
	...
	if (Location.id == "Marigo_town")
	{
		...
		for (i = 1; i <= 2; i++)
		{
			...
			{
				sLocShip = "3";
				rCharacter.ship.type = GenerateShip(4 + drand(10), false);
			}
			rShip = GetRealShip(sti(rCharacter.Ship.Type));
			Ship_SetLightsAndFlares(rCharacter);
			Ship_PrepareShipForLocation(rCharacter);
		...
		for (i = 1; i <= 2; i++)
		{
			...
			{
				sLocShip = "4";
				rCharacter.ship.type = GenerateShip(3 + drand1(24), false);
			}
			else
			{
				sLocShip = "5";
				rCharacter.ship.type = GenerateShip(3 + drand2(24), false);
			}
		...
		for (i = 1; i <= 2; i++)
		{
			...
			{
				sLocShip = "7";
				rCharacter.ship.type = GenerateShip(3 + drand2(52), false);
			}
			SetBaseShipData(rCharacter);
			rShip = GetRealShip(sti(rCharacter.Ship.Type));
			Ship_SetLightsAndFlares(rCharacter);
